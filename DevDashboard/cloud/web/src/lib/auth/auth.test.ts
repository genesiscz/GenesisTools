...
/**
 * Open signup must fail closed in production. No mail provider is wired, so
 * `requireEmailVerification` cannot be turned on yet — the guard is what stops that gap reaching a
 * deploy silently. Both controls are pinned: production without verification throws, and every
 * other environment still boots (otherwise the guard would break local dev and CI instead).
 */
describe("assertEmailVerificationSafe", () => {
    it("refuses to boot production while email verification is off", async () => {
        const { assertEmailVerificationSafe } = await import("./auth.server");
        expect(() => assertEmailVerificationSafe("production", false)).toThrow(/Email verification is disabled/);
    });

    it("allows production once verification is on", async () => {
        const { assertEmailVerificationSafe } = await import("./auth.server");
        expect(() => assertEmailVerificationSafe("production", true)).not.toThrow();
    });

    it("never blocks development, test or an unset NODE_ENV", async () => {
        const { assertEmailVerificationSafe } = await import("./auth.server");

        for (const env of ["development", "test", ""]) {
            expect(() => assertEmailVerificationSafe(env, false)).not.toThrow();
        }
    });

    it("ships with verification still off, so the guard is load-bearing rather than decorative", async () => {
        const { REQUIRE_EMAIL_VERIFICATION } = await import("./auth.server");
