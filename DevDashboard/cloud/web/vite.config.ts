...
const config: ConfigWithTest = {
    ...
    test: {
        ...
        environment: "node",
        include: ["src/**/*.test.ts"],
        // `cloud-store.test.ts` and `auth.test.ts` build a real in-memory SQLite in `beforeAll`,
        // which first has to transform the drizzle + better-auth + better-sqlite3 graph. On a cold
        // cache that alone runs past vitest's 10s default, so both suites reported a hook timeout
        // and SKIPPED every test in them — the cross-account-device and one-subscription-per-account
        // guarantees were never actually asserted. Budget for the transform instead.
        hookTimeout: 60_000,
        server: {
            deps: {
