...
| §4 watch | `tools grok tail --name <NAME>` follows the running turn and exits when it ends; `tools grok status --name <NAME>` (whether a turn is running; `--json` for the full metadata); `tools grok read --name <NAME> [--turn N] [--format compact]` re-prints any finished turn |
