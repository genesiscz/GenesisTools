...
describe("research config migration", () => {
    ...
    it("never overwrites an earlier archive from the same day", async () => {
        const home = sandbox();
        const p = paths(home);
        const now = new Date();
        const day = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
        const earlier = `${p.legacyResearch}.migrated-${day}`;

        write(earlier, "the first backup");
        write(p.legacyResearch, { defaultPath: "/vault/Research" });
        await run(RESEARCH, home, ["config"]);

        expect(readFileSync(earlier, "utf8")).toBe("the first backup");
        expect(existsSync(`${earlier}-2`)).toBe(true);
        expect(existsSync(p.legacyResearch)).toBe(false);
    });

    it("writes nothing when the legacy file cannot be archived", async () => {
        const home = sandbox();
        const p = paths(home);

        write(p.legacyResearch, { version: 1, defaultPath: "/legacy/path" });
        // A read-only parent refuses the rename, so the legacy file cannot be archived.
        chmodSync(dirname(p.legacyResearch), 0o555);

        try {
            await run(RESEARCH, home, ["config"]);
        } finally {
            chmodSync(dirname(p.legacyResearch), 0o755);
        }

        // Nothing half-migrated: no research key was written, and the source is still there to
        // retry from on the next run.
        expect(existsSync(p.config) ? read(p.config).research : undefined).toBeUndefined();
        expect(existsSync(p.legacyResearch)).toBe(true);
    });

    it("never replaces a shared config it cannot read", async () => {
        const home = sandbox();
        const p = paths(home);

        write(p.legacyResearch, { version: 1, defaultPath: "/legacy/path" });
        write(p.config, "{ other plugins' sections, but broken");
        await run(RESEARCH, home, ["config"]);

        // Every other plugin's section lives in this file. Writing `{ research: … }` over it
        // would erase them, so the migration is abandoned and both files stay as they were.
        expect(readFileSync(p.config, "utf8")).toBe("{ other plugins' sections, but broken");
        expect(existsSync(p.legacyResearch)).toBe(true);
    });
