...
- If the result carries `capture.failed: true` with "peekaboo produced NO output", the peekaboo BINARY is crashing (verify standalone: `peekaboo capture live --mode screen --duration 2s --json`; 3.0.0-beta3 did this — upgrading to 3.9.4 fixed it). Element control, screenshots, annotate, and OCR don't use that path and keep working.
