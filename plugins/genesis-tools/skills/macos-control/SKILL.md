...
## The verified family: `see` → `act --refresh`
...
tools control act --app com.apple.calculator --refresh \
    --snapshot "$(jq -r .snapshot /tmp/calc.json)" --element N --action press \
    --path /tmp/after.png > /tmp/after.json

tools json /tmp/after.json         # the settled tree is under .after
```
...
`after` is a full `see` result — elements, screenshot and a fresh snapshot token — so the next
`act` reads its token straight from `/tmp/after.json` and no separate `see` is needed. When the
UI never settled, `after.ok` is `false` and `refreshRequired` stays `true`; run `see` then.
...
The plan's `duration` field is seconds on both; only a raw `peekaboo capture live` command
typed by hand reads it as milliseconds. See references/capture.md Step 1.
...
The full note lives in [references/capture.md](references/capture.md), which is its only copy.
