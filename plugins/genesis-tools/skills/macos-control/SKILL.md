...
the `peekaboo list` command, which `tools control capture preflight` still calls, so it exits
1 with `undefined is not an object (evaluating 'activeScreen.scaleFactor')`. This is broken on
master too. The affected functions, the v4 replacements and what still works are in the note
at the top of [references/capture.md](references/capture.md), which is the only copy of it.
Element control, screenshots, annotate and OCR do not use that path and keep working.
