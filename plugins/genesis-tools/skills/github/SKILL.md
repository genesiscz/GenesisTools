...
5. **Delegate replies to a cheap background agent** — thread replies are independent shell commands that don't need main context. Claude Code: a `Bash` agent with `model: "haiku"` and `run_in_background: true` holding every `tools github review --respond` command; don't wait for it. Codex: `spawn_agent`. Grok: `spawn_subagent`. (`references/harness-tools.md` in this plugin.) Running them inline also works — the point is only to keep them out of the reasoning context.
