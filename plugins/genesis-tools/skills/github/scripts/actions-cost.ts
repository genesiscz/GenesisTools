...
// --- Runner rates ($/min) — GitHub Actions pricing effective Jan 1, 2026 ---
...
async function ghJson<T>(args: string[]): Promise<T> {
    ...
    // biome-ignore lint/style/noRestrictedGlobals: standalone script, no access to SafeJSON
...
function detectRunnerType(labels: string[]): string {
    for (const label of labels) {
        ...
        if (l.includes("ubuntu") || l.includes("linux")) {
            const coreMatch = l.match(/(\d+)-cores?/);
...
async function listRuns(
    ...
    const args = [
        ...
        "1000",
        "--json",
        "databaseId,workflowName,conclusion,createdAt,headBranch,event",
    ];
...
async function getRunJobs(repo: string, runId: number): Promise<JobInfo[]> {
    const result = await ghJson<{ jobs: JobInfo[] }>(["run", "view", String(runId), "--repo", repo, "--json", "jobs"]);
