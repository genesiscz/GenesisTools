...
- **It resolves the user's real `claude` install**, not whatever `claude` happens to be first on PATH. This repo used to vendor `@anthropic-ai/claude-code`, so `node_modules/.bin` shadowed the real one under `bun run` with a pinned CLI that rejected current flags; that dependency is gone, but resolving deliberately is still the right behaviour.
