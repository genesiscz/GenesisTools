...
import { importedHooksConfig, legacyGuardConfigPath, readLegacyGuardConfig } from "@app/agents/lib/hooks/import-config";
...
// Merged exactly as the hooks load it: each harness map over the shipped one, so a legacy file
// naming only `harnesses.codex` still gets grok's shipped `zsh-glob-qualifier: allow`. The raw
// importer output lacked that and showed a false divergence.
const imported = importedHooksConfig(legacy);
