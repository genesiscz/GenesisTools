...
#    `src/utils/logger/client-isolation.test.ts` (it owns the per-Vite-app client
...
if [ ! -f src/utils/logger/client-isolation.test.ts ]; then
    echo "::error:: src/utils/logger/client-isolation.test.ts missing — the browser-client @app/utils/logger isolation guard (Phase-2 deviation, handoff §5) must exist and run in the suite."
    fail=1
fi
