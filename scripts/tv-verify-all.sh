...
if rg -n "import \{[^}]*\bout\b[^}]*\} from \"@(app|genesiscz)/utils/logger\"" src/tradingview/lib/ >/dev/null 2>&1 || rg -n "\bout\.|console\.|process\.exit" src/tradingview/lib/ >/dev/null 2>&1; then
