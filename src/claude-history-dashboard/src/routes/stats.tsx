		...
		<div className="min-h-screen bg-background">
			...
			<header className="border-b border-border">
				<div className="max-w-7xl mx-auto px-6 py-6">
					...
					<h1 className="text-3xl font-black tracking-tight text-foreground">Statistics</h1>
					<p className="text-muted-foreground mt-1">
						Overview of your Claude Code usage
						{quickStats.isCached && <span className="text-xs ml-2 text-cyan-400">(cached)</span>}
					</p>
				</div>
			</header>
				...
				<Card className="mb-6">
					<CardContent className="py-4">
						<div className="flex items-center gap-4">
							...
							{isLoadingRange && <span className="text-xs text-cyan-400 animate-pulse">Loading...</span>}
						</div>
					</CardContent>
				</Card>
			</div>
			...
			<main className="max-w-7xl mx-auto px-6 pb-8">
				...
				{filteredStats ? (
					...
					<Suspense fallback={<ActivityChartSkeleton />}>
						<Await promise={fullStats}>{(stats) => <ActivityChart stats={stats} />}</Await>
					</Suspense>
				)}
...
function FilteredStatCards({ stats, isFiltered }: { stats: SerializableStats; isFiltered: boolean }) {
	const totalTokens =
		...
		stats.tokenUsage.cacheCreateTokens +
		...
		<Card>
			<CardContent>
				<div className="flex items-center gap-4">
					...
					<div>
						<p className="text-2xl font-bold text-foreground">{formatted || value.toLocaleString()}</p>
						...
						<div key={project} className="flex items-center gap-3">
							<div className="flex-1">
								<div className="flex items-center justify-between mb-1">
									<span className="text-sm text-foreground truncate">{project}</span>
