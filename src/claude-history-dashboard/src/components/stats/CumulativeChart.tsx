...
export function CumulativeChart({ dailyActivity, dailyTokens, title = 'Cumulative Growth' }: CumulativeChartProps) {
  ...
  const sortedDates = Object.entries(dailyActivity)
    .sort(([a], [b]) => a.localeCompare(b))

  const cumulativeData = sortedDates.reduce((acc, [date, count]) => {
    ...
    return acc
  }, [] as Array<{ date: string; messages: number; tokens: number; cumulativeMessages: number; cumulativeTokens: number }>)
