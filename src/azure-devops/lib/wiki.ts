...
export function rewriteAttachmentLinks(markdown: string, localPaths: Map<string, string>): string {
    ...
    for (const [original, localPath] of localPaths) {
        rewritten = rewritten.split(original).join(encodeURI(localPath));
    }
...
export async function downloadWikiAttachments({
    ...
    await concurrentMap({
        ...
        fn: async (ref) => {
            ...
            const localPath = join(outputDir, safeFileName(ref.name));
...
/** Match a full or abbreviated commit id against the page's history, newest first. */
export function findCommit(commits: GitCommitRefApi[], ref: string): GitCommitRefApi | undefined {
    const wanted = ref.trim().toLowerCase();

    return commits.find((commit) => commit.commitId.toLowerCase().startsWith(wanted));
}
