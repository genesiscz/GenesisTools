...
import { storage, CACHE_TTL, loadGlobalCache, saveGlobalCache, formatJSON } from "@app/azure-devops/cache";
...
export async function handleQuery(
  ...
  if (filters) logger.debug(`[query] Filters: states=${filters.states?.join(",")}, severities=${filters.severities?.join(",")}`);
  ...
  logger.debug(`[query] Cache: ${rawCache ? `found with ${oldCache?.length || 0} items` : "not found or forced refresh"}`);
...
export function registerQueryCommand(program: Command): void {
  program
    ...
    .action(async (input: string, options: QueryOptions) => {
      ...
      if (options.state) {
        filters.states = options.state.split(",").map(s => s.trim()).filter(Boolean);
      }
      if (options.severity) {
        filters.severities = options.severity.split(",").map(s => s.trim()).filter(Boolean);
      }
