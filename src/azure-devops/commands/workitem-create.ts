    ...
    const steps: Array<{
        ...
        // Step 0: Project selection
        {
            ...
            run: async () => {
                ...
                const sortedProjects = configuredProject ? [configuredProject, ...otherProjects] : projects;
...
async function handleCreate(options: {
    ...
    const hasValidMode =
        options.interactive || options.fromFile || options.sourceInput || (options.type && options.title);
...
`);
    ...
    if (
        options.sourceInput &&
        (options.sourceInput.includes("/_queries/") ||
            /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(options.sourceInput))
    ) {
