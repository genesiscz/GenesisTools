...
import type { OutputFormat, WorkItemFull, WorkItemCache, WorkItemSettings, QueryItemMetadata } from "@app/azure-devops/types";
import { CACHE_TTL, WORKITEM_FRESHNESS_MINUTES, loadGlobalCache, saveGlobalCache, formatJSON } from "@app/azure-devops/cache";
...
export async function handleWorkItem(
  ...
  logger.debug(`[workitem] Starting with input: ${input}, force=${forceRefresh}, category=${categoryArg}, taskFolders=${taskFoldersArg}`);
...
export function registerWorkitemCommand(program: Command): void {
  program
    ...
    .action(async (input: string, options: { format: OutputFormat; force?: boolean; category?: string; taskFolders?: boolean }) => {
      await handleWorkItem(input, options.format, options.force ?? false, options.category, options.taskFolders);
    });
}
