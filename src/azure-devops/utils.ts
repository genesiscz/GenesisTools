...
export function getTaskFilePath(id: number, title: string, ext: string, category?: string, useTaskFolder?: boolean): string {
...
): Map<string, { name: string; required: boolean; allowedValues?: string[]; defaultValue?: string; helpText?: string }> {
  const schema = new Map<string, { name: string; required: boolean; allowedValues?: string[]; defaultValue?: string; helpText?: string }>();
 ...
 */
export function extractUsedValuesFromFull(
  items: WorkItemFull[],
  project: string,
  queryId?: string
): UsedValuesCache {
...
): Record<string, { description: string; required?: boolean; allowedValues?: string[]; usedValues?: string[]; defaultValue?: string; examples?: string[] }> {
  const hints: Record<string, { description: string; required?: boolean; allowedValues?: string[]; usedValues?: string[]; defaultValue?: string; examples?: string[] }> = {};
...
function transformHintsForTemplate(
  hints: Record<string, { description: string; required?: boolean; allowedValues?: string[]; usedValues?: string[]; defaultValue?: string; examples?: string[] }>
...
export function generateTemplateFromWorkItem(
  ...
  const parsed = sourceItem.relations ? parseRelations(sourceItem.relations) : { parent: undefined, children: [], related: [] };
  ...
  const hints: Record<string, { description: string; required?: boolean; usedValues?: string[]; allowedValues?: string[] }> = {
    title: {
  ...
  for (const [refName, rawValue] of Object.entries(rawFields)) {
    ...
    if (refName === "System.Tags" && typeof value === "string") {
      fields[fieldName] = value.split(";").map(t => t.trim()).filter(Boolean);
    } else {
