...
export interface SkeletonSymbol {
    ...
    startLine: number;
    endLine: number;
    /** The head as displayed: one line, cut at 160 characters with a `…`. */
    signature: string;
    /**
     * The whole head, set only when `signature` was cut. Parameter counts, types and the return
     * type are read from this: a cut inside the parameter list dropped parameters and the `)`.
     */
...
/** Every `kind` `extractSkeleton` emits, so a `--kinds` filter can reject a misspelled one. */
...
/** One line, without the dangling `{`, `=>` or `=` a cut-away body leaves. Never truncated. */
...
function clip(single: string): { text: string; truncated: boolean } {
 ...
 * Returned whole; `push` clips it for display and keeps the whole head as `fullSignature`.
 */
function signatureOf(node: ts.Node, source: ts.SourceFile, body?: ts.Node): string {
    ...
    return flatten(source.text.slice(start, end));
}
...
function headOf(node: ts.Node, source: ts.SourceFile, members: ts.NodeArray<ts.Node>): string {
    return flatten(source.text.slice(node.getStart(source), members.pos));
}
...
export function extractSkeleton(source: ts.SourceFile, options: ExtractOptions = {}): SkeletonSymbol[] {
    ...
    const push = (
        ...
        symbols.push({
            ...
            signature: shown.text,
            ...(shown.truncated ? { fullSignature: signature } : {}),
