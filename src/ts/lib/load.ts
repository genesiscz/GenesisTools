...
export async function loadFiles(paths: string[], options: LoadOptions = {}): Promise<Loaded> {
    ...
    for (const absolute of files) {
        ...
        // Forward slashes on every platform: the analysers split and match `file` on `/`, so a
        // Windows `\` path saw no directories, no shared home and no aliased import.
        const file = (relative(process.cwd(), absolute) || absolute).replace(/\\/g, "/");
