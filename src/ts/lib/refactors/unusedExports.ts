...
export function resolveSpecifier(fromFile: string, specifier: string, files: readonly string[]): string[] {
    ...
    let tail = segments.slice(specifier.startsWith("@") ? 2 : 1).join("/");

    // `@app/logger` may be a tsconfig alias, not a scoped package, and an empty tail matched
    // nothing, so every export of the logger read as unused. Over-matching is the safe side.
    if (tail === "" && specifier.startsWith("@") && segments.length === 2) {
        tail = segments[1] ?? "";
    }
