import { analysedSignature, parameterCount, parameterTypes } from "../signature";
    ...
    run: ({ entries, options }): Recommendation[] => {
        ...
        for (const entry of entries) {
            for (const symbol of entry.symbols) {
                ...
                const count = parameterCount(analysedSignature(symbol));

                if (count <= maxParams) {
                    continue;
                }

                const types = parameterTypes(analysedSignature(symbol));
                let swappable = 0;
