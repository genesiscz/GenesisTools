...
export const godFilesAnalyser: Analyser = {
    ...
    run: ({ entries, options }): Recommendation[] => {
        ...
        // Nearest-rank: `ceil(n * 0.9) - 1`. `floor(n * 0.9)` picked the LARGEST count for ten files,
        // so the outlier set its own cut-off and could never exceed it. Below ten files even the
        // nearest rank IS the largest count, so there the budget alone is the bar.
        const p90 = counts.length >= 10 ? (counts[Math.ceil(counts.length * 0.9) - 1] ?? 0) : 0;
