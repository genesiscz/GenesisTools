...
import { analysedSignature, parameterCount, returnType } from "../signature";
import { DECLARATION_KINDS, enrichSymbols, extractSkeleton, parseSource } from "../skeleton";
...
describe("unused-exports", () => {
    ...
    test("a one-segment alias such as @app/logger matches the scanned file of that name", () => {
        const files = ["src/logger.ts", "src/lib/client.ts"];

        expect(resolveSpecifier("src/feature/use.ts", "@app/logger", files)).toEqual(["src/logger.ts"]);
    });
});
...
describe("long-functions, param-bloat, god-files", () => {
    ...
    test("parameters and the return type are read from the whole head, not the 160-character display", () => {
        const params = Array.from({ length: 7 }, (_, index) => `someRatherLongParameterName${index}: string`).join(
            ", "
        );
        const text = `function wide(${params}): number {\n\treturn 1;\n}\n`;
        const [symbol] = extractSkeleton(parseSource("src/a.ts", text));

        expect(symbol?.signature.endsWith("…")).toBe(true);
        expect(parameterCount(analysedSignature(symbol ?? { signature: "" }))).toBe(7);
        expect(returnType(analysedSignature(symbol ?? { signature: "" }))).toBe("number");
    });
    ...
    test("god-files reports an oversized file in a scan of fewer than ten files", () => {
        // Below ten files the nearest-rank p90 is the largest count itself, which hid every file.
        const many = Array.from({ length: 12 }, (_, index) => `export const c${index} = ${index};`).join("\n");
        const alone = run(godFilesAnalyser, [{ file: "src/big.ts", text: many }], { maxDeclarations: 5 });
        const few = run(
            godFilesAnalyser,
            [
                { file: "src/big.ts", text: many },
                { file: "src/small.ts", text: "export const x = 1;\n" },
            ],
            { maxDeclarations: 5 }
        );

        expect(alone.map((recommendation) => recommendation.sites[0]?.file)).toEqual(["src/big.ts"]);
        expect(few.map((recommendation) => recommendation.sites[0]?.file)).toEqual(["src/big.ts"]);
    });

    test("every kind the skeleton emits is a valid --kinds value, accessors included", () => {
        const text =
            "export class Box {\n\tget size(): number {\n\t\treturn 1;\n\t}\n\tset size(v: number) {\n\t\tvoid v;\n\t}\n}\n";
        const kinds = extractSkeleton(parseSource("src/box.ts", text)).map((symbol) => symbol.kind);
    ...
    test("god-files still reports the outlier among exactly ten files", () => {
        // With ten files `floor(10 * 0.9)` indexed the largest count, so the outlier set its own bar.
        const many = Array.from({ length: 12 }, (_, index) => `export const c${index} = ${index};`).join("\n");
        const found = run(
            godFilesAnalyser,
            [
...
describe("collectFiles --ignore", () => {
    ...
    test("never matches the directories above the scan, only the paths inside it", () => {
        // The scan root's own name contains the needle, the way a repo under
        // `/Users/me/test-projects/app` does. Matching the absolute path skipped every file.
        const root = mkdtempSync(join(tmpdir(), "ts-collect-test-"));

        mkdirSync(join(root, "test-fixtures"));
        writeFileSync(join(root, "keep.ts"), "export const a = 1;\n");
        writeFileSync(join(root, "test-fixtures", "copy.ts"), "export const b = 1;\n");

        const found = collectFiles(root, { ignore: ["test"] });
