...
/**
 * The directory of the tsconfig that governs `from`: the nearest one at or above it, never above
 * `root`. The repository root's tsconfig is only the fallback. A nested workspace (a mobile app, a
 * web app) declares its own aliases, like `@/*` or `@dd/*`, and reading only the root file left
 * every such import unresolved.
 */
function tsconfigDirFor(from: string, root: string): string {
    let dir = from;
...
export function resolveSpecifier(fromFile: string, specifier: string, root: string): string | null {
    ...
    const { baseUrl, paths } = tsconfigPaths(tsconfigDirFor(dirname(fromFile), root));
...
/**
 * The module a local name is imported from, and the name it is DECLARED under there. They
 * differ for `import type { Account as User }`: the target file declares `Account`, so looking
 * `User` up there found nothing and the type was silently left out.
 */
function importSpecifierFor(source: ts.SourceFile, name: string): { specifier: string; importedName: string } | null {
    for (const statement of source.statements) {
            ...
            for (const element of bindings.elements) {
                if (element.name.text === name) {
                    return {
                        ...
                        importedName: element.propertyName?.text ?? name,
                    };
                }
            }
        }
        ...
        if (statement.importClause.name?.text === name) {
            return { specifier: (statement.moduleSpecifier as ts.StringLiteral).text, importedName: name };
        }
    }
...
export function expandTypes(
    ...
    // Keyed by FILE and name, never the bare name: two `Options` in two files are two types,
    // and a name-only key printed the first one and silently dropped the other.
    const key = (at: string, name: string): string => `${at}::${name}`;
    // A lookup already made (this name, as seen from this file).
    const seen = new Set<string>();
    // A declaration already printed, however many files reached it.
    ...
    while (queue.length > 0 && found.length < MAX_TYPES) {
        ...
        if (!job || seen.has(key(job.file, job.name))) {
            continue;
        }

        seen.add(key(job.file, job.name));

        let declaration = declarationFor(job.source, job.name);
        let declarationSource = job.source;
        let declarationFile = job.file;
        let declaredName = job.name;

        if (!declaration) {
            const imported = importSpecifierFor(job.source, job.name);
            ...
            declaredName = imported.importedName;
            ...
            if (!target) {
                ...
                if (printed.has(key(specifier, declaredName))) {
                    continue;
                }

                printed.add(key(specifier, declaredName));
            ...
            try {
                ...
                declaration = declarationFor(declarationSource, declaredName);
            } catch {
                continue;
            }
        }

        if (!declaration || printed.has(key(declarationFile, declaredName))) {
            continue;
        }

        printed.add(key(declarationFile, declaredName));
        ...
        if (nextDepth <= maxDepth) {
            for (const nested of collectTypeNamesIn(declaration)) {
                if (!seen.has(key(declarationFile, nested))) {
                    queue.push({
                        name: nested,
