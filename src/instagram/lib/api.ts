 ...
 */
async function fetchReels(reelIds: string[], sessionId: string | undefined, label: string): Promise<StoryReel[]> {
    if (!sessionId) {
    ...
    const { data } = await getJson<RawReelsResponse>(`/api/v1/feed/reels_media/?${query}`, { sessionId, label });
...
export async function fetchStories(userId: string, sessionId: string | undefined): Promise<StoryReel[]> {
    return fetchReels([userId], sessionId, `stories:${userId}`);
}

export async function fetchHighlightMedia(highlightIds: string[], sessionId: string | undefined): Promise<StoryReel[]> {
    return fetchReels(
 ...
 */
export async function fetchHighlightTray(userId: string, sessionId: string | undefined): Promise<HighlightRef[]> {
    if (!sessionId) {
