...
program
    ...
    .action(async (username: string, options: SessionOptions & { json?: boolean }) => {
        try {
            ...
            if (session) {
                const tray = await fetchHighlightTray(profile.id, session.sessionId);
...
program
    ...
    .action(async (username: string, options: SessionOptions & { download?: string | boolean; json?: boolean }) => {
        try {
            const session = await resolveOrExplain(options);
            const profile = await fetchProfile(username);
            const reels = await fetchStories(profile.id, session?.sessionId);
...
program
    ...
    .action(
        async (
            ...
            try {
                const session = await resolveOrExplain(options);
                const reels = await fetchHighlightMedia([highlightId], session?.sessionId);
