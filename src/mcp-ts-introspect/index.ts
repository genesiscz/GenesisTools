...
async function getMode(argv: Args): Promise<string> {
    ...
    if (argv.package) return "package";
    if (argv.source) return "source";
    if (argv.project !== undefined) return "project";
...
async function getPackageName(argv: Args): Promise<string> {
    if (argv.package) return argv.package;
...
async function getSourceCode(argv: Args): Promise<string> {
    if (argv.source) return argv.source;
...
async function getOutputDestination(argv: Args): Promise<string> {
    if (argv.output) return argv.output;
...
async function main() {
    ...
    try {
        ...
        const options: IntrospectOptions = {
            searchPaths: Array.isArray(argv.searchPaths) ? argv.searchPaths : 
                        argv.searchPaths ? [argv.searchPaths] : [],
        ...
        switch (mode) {
            case "package": {
                ...
                if (argv.verbose) logger.info(`Introspecting package: ${packageName}`);
            ...
            case "source": {
                ...
                if (argv.verbose) logger.info(`Introspecting source code...`);
            ...
            case "project": {
                ...
                if (argv.verbose) logger.info(`Introspecting project: ${projectPath}`);
