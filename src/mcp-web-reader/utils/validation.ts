...
export function validateMarkdown(markdown: string): ValidationResult {
    ...
    if (htmlTags > 0) issues.push(`${htmlTags} HTML tags remaining in output`);

    // Check for empty links
    const emptyLinks = (markdown.match(/\[\s*\]\(\s*\)/g) || []).length;
    if (emptyLinks > 0) issues.push(`${emptyLinks} empty links found`);
    ...
    if (brokenImages > 0) issues.push(`${brokenImages} images with empty src`);
