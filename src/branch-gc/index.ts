...
import { gitOk, runGit } from "./lib/git";
...
async function deleteBranch(cwd: string, name: string): Promise<boolean> {
    const shaRes = await runGit(cwd, ["rev-parse", "--short", name]);
    ...
    log.out.error(`Failed to delete ${name}: ${res.stderr.trim() || "unknown git error"}`);
    return false;
}
...
await runTool(program, { tool: "branch-gc" });
...
async function main(): Promise<void> {
    ...
    if (opts.dryRun || !isInteractive()) {
        if (safe.length > 0) {
            out.log.info(`Run ${pc.bold(suggestCommand("tools branch-gc", { add: ["--yes"] }))} to delete them.`);
        }
