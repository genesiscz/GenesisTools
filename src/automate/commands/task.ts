...
export function registerTaskCommand(parent: Command): void {
  ...
  parent
    ...
    .action((arg: string) => {
      ...
      if (!schedule) { p.log.error(`Task "${arg}" not found`); return; }
      ...
      const scheduleRuns = runs.filter(r => r.preset_name === schedule.preset_name && r.trigger_type === "schedule");
  ...
  parent
    ...
    .action((name: string) => {
      ...
      if (!existing) { p.log.error(`Task "${name}" not found`); return; }
  ...
  parent
    ...
    .action((name: string) => {
      const db = getDb();
      if (!db.getSchedule(name)) { p.log.error(`Task "${name}" not found`); return; }
  ...
  parent
    ...
    .action(async (name: string, opts: { verbose?: boolean }) => {
      ...
      if (!schedule) { p.log.error(`Task "${name}" not found`); return; }
