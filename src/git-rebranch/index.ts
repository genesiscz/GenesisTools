...
async function detectForkPoint(
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (
                count < bestCommitCount ||
                (count === bestCommitCount && isCommon && !commonBases.includes(bestBase!))
            ) {
                bestBase = candidate;
                bestForkSha = mb;
...
async function executeBranches(git: GitInstance, groups: CommitGroup[], forkPointSha: string): Promise<BranchResult[]> {
