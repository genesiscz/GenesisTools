...
import type { HarSession, HarFile, EntryFilter } from "@app/har-analyzer/types";
...
export async function startMcpServer(): Promise<void> {
	...
	const server = new Server(
		{ name: "har-analyzer", version: "1.0.0" },
		{ capabilities: { tools: {} } },
	);
		...
		try {
			switch (name) {
				...
				case "har_overview": {
					...
					return { content: [{ type: "text", text: formatDashboard(ctx.session.stats, ctx.session.sourceFile) }] };
				}
				...
				case "har_detail": {
					...
					if (a.raw) {
						...
						if (!section || section === "body") {
							...
							if (entry.request.postData?.text) {
								lines.push(await ctx.refStore.formatValue(entry.request.postData.text, `e${idx}.rq.body`, { full }));
							} else {
								lines.push("(none)");
							}
					...
					lines.push(`Request Headers: ${entry.request.headers.length} | Response Headers: ${entry.response.headers.length}`);
				...
				case "har_expand": {
					...
					switch (path) {
						...
						case "rq.body":
							...
							break;
						case "rs.body":
							content = entry.response.content.encoding === "base64"
				...
				case "har_search": {
					...
					for (const ie of ctx.session.entries) {
						...
						if (scope === "header" || scope === "all") {
							...
							if (allH.some((h) => h.name.toLowerCase().includes(query) || h.value.toLowerCase().includes(query))) {
								results.push(`[e${ie.index}] ${ie.method} ${truncatePath(ie.path, 40)} ${ie.status} (header match)`);
								continue;
							}
						}
						...
						if (scope === "body" || scope === "all") {
							...
							if (body.toLowerCase().includes(query)) {
								results.push(`[e${ie.index}] ${ie.method} ${truncatePath(ie.path, 40)} ${ie.status} (body match)`);
							}
						}
					}
				...
				case "har_export": {
					...
					return { content: [{ type: "text", text: `Would export ${filtered.length} entries. Use CLI: tools har-analyzer export [--domain X] [--sanitize] [-o file]` }] };
				}
