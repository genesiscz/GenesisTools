...
export class SessionManager {
    ...
    async createSession(harFilePath: string): Promise<HarSession> {
        ...
        await Bun.write(join(this.storage.getCacheDir(), "last-session.txt"), sourceHash);
    ...
    async loadSession(hashOrPath?: string): Promise<HarSession | null> {
        ...
        const session = await this.storage.getCacheFile<HarSession>(`sessions/${hash}.json`, SESSION_TTL);
