...
export function formatDashboard(stats: SessionStats, sourceFile: string): string {
	...
	const statusBuckets = Object.entries(stats.statusDistribution)
	...
	const domainRows = domainEntries.map(([domain, count]) => [
		domain,
		String(count),
	]);
	...
	const mimeEntries = Object.entries(stats.mimeTypeDistribution)
		.sort(([, a], [, b]) => b - a);

	const mimeHeaders = ["Type", "Count"];
	const mimeRows = mimeEntries.map(([mime, count]) => [
		mime,
		String(count),
	]);
