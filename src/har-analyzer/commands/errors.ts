...
async function formatErrorEntry(entry: IndexedEntry, rawEntry: HarEntry, refStore: RefStoreManager): Promise<string> {
    const parts: string[] = [];
    ...
    if (bodyText && isInterestingMimeType(entry.mimeType)) {
        const preview = await refStore.formatValue(bodyText.slice(0, 120), `e${entry.index}-err-body`);
