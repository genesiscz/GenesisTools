...
describe.skipIf(skip.integration)("tools say", () => {
    ...
    describe("default-profile mute (silences no-app calls)", () => {
        it("--mute --save --app default exits 0", async () => {
            const r = await execTool(["say", "--mute", "--save", "--app", "default"]);
            expect(r.exitCode).toBe(0);
        });
        ...
        it("speech without --app reports muted when default is muted", async () => {
            ...
            const setup = await execTool(["say", "--mute", "--save", "--app", "default"]);
            expect(setup.exitCode).toBe(0);

            const r = await execTool(["say", "should be muted"]);
            expect(r.exitCode).toBe(0);
            expect(getOutput(r).toLowerCase()).toContain("muted");
        });

        it("--unmute --save --app default exits 0", async () => {
            const r = await execTool(["say", "--unmute", "--save", "--app", "default"]);
            expect(r.exitCode).toBe(0);
        });
    });
    ...
    describe("per-app mute", () => {
        ...
        it("speech with muted app reports muted", async () => {
            const setup = await execTool(["say", "--mute", "--save", "--app", "e2e-test"]);
            expect(setup.exitCode).toBe(0);

            const r = await execTool(["say", "should be muted", "--app", "e2e-test"]);
            expect(r.exitCode).toBe(0);
            expect(getOutput(r).toLowerCase()).toContain("muted");
        });

        it("--unmute --save --app e2e-test exits 0", async () => {
            const r = await execTool(["say", "--unmute", "--save", "--app", "e2e-test"]);
            expect(r.exitCode).toBe(0);
        });
        ...
        it("--unmute --save through a muted profile (PR #157 t3)", async () => {
            ...
            const muted = await execTool(["say", "should be muted", "--app", "e2e-test"]);
            ...
            const remute = await execTool(["say", "--mute", "--save", "--app", "e2e-test"]);
            expect(remute.exitCode).toBe(0);

            const reMuted = await execTool(["say", "should be muted", "--app", "e2e-test"]);
