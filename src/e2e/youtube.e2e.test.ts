...
describe("tools youtube", () => {
    ...
    describe.skipIf(!process.env.RUN_NETWORK_TESTS)("transcribe with captions", () => {
        ...
        it("fetches captions as SRT", async () => {
            const r = await execTool(["youtube", "transcribe", "dQw4w9WgXcQ", "--format", "srt"]);
            expect(r.exitCode).toBe(0);
        ...
        it("fetches captions as VTT", async () => {
            const r = await execTool(["youtube", "transcribe", "dQw4w9WgXcQ", "--format", "vtt"]);
            expect(r.exitCode).toBe(0);
            expect(r.stdout).toContain("WEBVTT");
        }, 30_000);

        it("fetches captions as JSON", async () => {
            const r = await execTool(["youtube", "transcribe", "dQw4w9WgXcQ", "--format", "json"]);
            expect(r.exitCode).toBe(0);
        ...
        it("accepts --lang flag", async () => {
            const r = await execTool(["youtube", "transcribe", "dQw4w9WgXcQ", "--lang", "en"]);
            expect(r.exitCode).toBe(0);
        }, 30_000);
    });
