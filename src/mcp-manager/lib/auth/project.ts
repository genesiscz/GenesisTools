...
/**
 * An IPv6 host has to be bracketed inside a URL, or `http://::1:8318/` is not parseable
 * and the port reads as part of the address. Every gateway URL in the codebase is built
 * from here, so bracketing at this one spot covers the health probe, the projections
 * written into harness configs, and the handle's own `url`.
 */
export function gatewayBaseUrl(listen: GatewayListen): string {
    const host = listen.host.includes(":") && !listen.host.startsWith("[") ? `[${listen.host}]` : listen.host;
...
export function isGatewayProjectedStdio(config: UnifiedMCPServerConfig, server: string): boolean {
    ...
    // Exact length, not a prefix match. `tools mcp-manager gateway stdio --server rohlik
    // --anything` is a config a USER wrote, not one projectServerForHarness emitted, and
    // treating it as ours means sync-from-providers overwrites it with the stored
    // definition before conflict detection ever sees it.
