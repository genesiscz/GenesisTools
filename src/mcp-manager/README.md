...
The tool uses a unified configuration file at `~/mcp.json` with the following schema:
...
The server doesn't exist in any provider configuration. Use `install` command to add it from `~/mcp.json`.
