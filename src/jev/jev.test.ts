...
} from "@genesiscz/utils/ai/evaluation/providers";
...
describe("Jev evaluation", () => {
    test("a long-lived evaluator honors each call's provider and caches both adapters", async () => {
        const created: string[] = [];
        const calls: EvaluationCall[] = [];
        const evaluate = await createEvaluatorWithProviderFactory({ provider: "vercel" }, async (provider) => {
            created.push(provider);
            const adapter: EvaluationProvider = {
                id: provider,
                async evaluate(call) {
                    calls.push(call);
                    return {
            ...
            return { adapter, apiKey: `${provider}-key` };
        });

        await evaluate({ input: demoInput });
        await evaluate({ input: demoInput, provider: "typesafe" });
        await evaluate({ input: demoInput, provider: "typesafe" });

        expect(created).toEqual(["vercel", "typesafe"]);
        expect(calls.map((call) => call.provider)).toEqual(["vercel", "typesafe", "typesafe"]);
    });
...
describe("Jev authentication", () => {
    test("keeps the key in the vault, never in the config file, with explicit precedence", async () => {
        ...
        try {
            ...
            env.testing.unset("TYPESAFE_API_KEY");
...
describe("Fly arena and lazy circuit cache", () => {
    ...
    test("Jev gets reduced state, validates choices, and uses an explicit low-confidence fallback", async () => {
        ...
        evaluate.mockImplementation(async () => ({
            model: JEV_MODEL,
            answers: {
                action: { type: "choice", choice: "right" },
                threat: { type: "boolean", probability: 0.1 },
                survival: { type: "score", score: 3 },
            },
            usage: { inputTokens: 10, outputTokens: 3, totalTokens: 13 },
        ...
        expect(await decideArena({ observation, evaluate })).toMatchObject({
            action: "right",
            confidence: null,
            fallback: false,
        });
