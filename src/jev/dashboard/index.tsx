...
function Zdr({
    ...
    return (
        <label className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span className="text-primary">{unavailable ? "Vercel AI Gateway only" : "Pro / Enterprise"}</span>
        </label>
    );
}
...
function Playground({ advanced = false, provider }: { advanced?: boolean; provider: EvaluationProviderId }) {
        ...
        <>
            ...
            <div className="jev-columns">
                <Panel title={advanced ? "Request editor" : "Your question"}>
                    ...
                    <Zdr checked={zdr} onChange={setZdr} disabled={busy} unavailable={provider === "typesafe"} />
...
function TypeScriptLab({ provider }: { provider: EvaluationProviderId }) {
                        ...
                        <Input
                            ...
                            onChange={(event) => {
                                if (Number.isFinite(event.target.valueAsNumber)) {
                                    setMaxSteps(Math.min(2048, Math.max(1, event.target.valueAsNumber)));
                                }
                            }}
                        />
                    </Field>
        ...
        >
            <div className="jev-page" key={provider}>
                ...
                {tab === "control" ? (
                    ...
                    <TypeScriptLab provider={provider} />
                ) : (
