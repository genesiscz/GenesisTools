...
export function ControlLab() {
    ...
    return (
        <div className="space-y-6">
            <nav aria-label="Control experiments" className="flex flex-wrap gap-2">
                {areas.map((label) => (
                    <Button key={label} variant={area === label ? "brand" : "outline"} onClick={() => setArea(label)}>
                        {label}
                    </Button>
                ))}
            </nav>
            {area === "Targets" ? (
                ...
                <ResilienceLab key="recovery" kind="recovery" />
            ) : area === "Workflows" ? (
