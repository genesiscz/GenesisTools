...
export async function decideArena({
    ...
    const selectedProbability = action.probabilities?.[action.choice];
    const fallback = selectedProbability !== undefined && selectedProbability < 0.45;
    return {
        action: fallback
        ...
        confidence: selectedProbability ?? null,
