...
export function sandboxCommand({
    ...
    const privateRoots = [homedir(), tmpdir(), "/tmp"].map((item) => realpathSync(item));
    ...
    const profile = [
        ...
        `(deny file-read-data (require-all (require-any ${privateRoots.map((item) => `(subpath ${quote(item)})`).join(" ")}) (require-not (require-any ${allowed.map((item) => `(subpath ${quote(item)})`).join(" ")} (literal ${quote(executable)})))))`,
