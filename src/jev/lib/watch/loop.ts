...
export async function runWatch(options: {
    ...
    while ((options.now ?? Date.now)() < deadline) {
        ...
        const observation = await prof.measureAsync("observe", () =>
            options.driver.observe({ signal: options.signal })
        );
        observes += 1;
        ticks += 1;
        ...
        log.debug(
            { tick: ticks, changed: lastState.changed.length, unchanged: lastState.unchangedCount },
            "watch tick"
        );
        ...
        requests += 1;
        log.info(
            { tick: ticks, status: fanout.status, reason: fanout.reason, done: fanout.done, requests },
            "watch decision"
        );
    ...
    log.info({ ticks, observes, requests }, "watch time budget reached");
