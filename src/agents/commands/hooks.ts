...
export function registerHooksCommands(program: Command): void {
    ...
    hooks
        ...
        .action((options: { dist?: string; target?: string; write?: boolean }) => {
            ...
            if (options.write && managed) {
                pointDist(target, dist);
            }

            const result = installHooks({ dist, write: Boolean(options.write) });
        ...
        .action((options: { session?: string; decision?: string; lines?: string }) => {
            ...
            try {
                ...
                for (const line of readFileSync(config.logPath, "utf8").split("\n")) {
                    ...
                    try {
                        records.push(SafeJSON.parse(line) as Record<string, unknown>);
                    } catch {
                        skipped += 1;
                    }
                }
            ...
            const filtered = records
                ...
                .slice(-Number.parseInt(options.lines ?? "20", 10));
    ...
    config
        ...
        .action((options: { from?: string; write?: boolean }) => {
            ...
            try {
                ...
                ui.err(`Cannot read the legacy guard config: ${err instanceof Error ? err.message : String(err)}`);
                process.exitCode = 1;
                return;
            }
