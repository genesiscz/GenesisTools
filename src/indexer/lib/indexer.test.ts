...
describe("Indexer", () => {
    it("sync() indexes new files and produces correct stats", async () => {
        writeFixtureFile(tempDir, "math.ts", `
...
`.trim());
        ...
        writeFixtureFile(tempDir, "greet.ts", `
...
`.trim());
    ...
    it("sync() skips unchanged files on second run", async () => {
        writeFixtureFile(tempDir, "stable.ts", `
...
`.trim());
    ...
    it("reindex() processes everything regardless", async () => {
        writeFixtureFile(tempDir, "always.ts", `
...
`.trim());
    ...
    it("search() returns results", async () => {
        writeFixtureFile(tempDir, "searchable.ts", `
...
`.trim());
    ...
    it("events fire in correct order", async () => {
        writeFixtureFile(tempDir, "events.ts", `
...
`.trim());
    ...
    it("close() cleans up without errors", async () => {
        writeFixtureFile(tempDir, "cleanup.ts", `
export const x = 1;
`.trim());
    ...
    it("name and stats getters work", async () => {
        writeFixtureFile(tempDir, "getters.ts", `
