...
if (!JENKINS_URL || !JENKINS_USER || !JENKINS_TOKEN) {
    const missing = [!JENKINS_URL && "JENKINS_URL", !JENKINS_USER && "JENKINS_USER", !JENKINS_TOKEN && "JENKINS_TOKEN"]
        .filter(Boolean)
        .join(", ");
...
class JenkinsServer {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            folderPath: {
                                type: "string",
                                description:
                                    'Path to folder or view (optional, empty for root level). Use "job/FolderName" format for folders',
                            },
                        },
                        required: [],
                    },
    ...
    private async getBuildStatus(args: GetBuildStatusArgs) {
        ...
        const response = await this.axiosInstance.get<BuildStatus>(`/${args.jobPath}/${buildNumber}/api/json`);
    ...
    private async triggerBuild(args: TriggerBuildArgs) {
        ...
        const endpoint = args.parameters ? `/${args.jobPath}/buildWithParameters` : `/${args.jobPath}/build`;
    ...
    private async getBuildHistory(args: GetBuildHistoryArgs) {
        ...
        const response = await this.axiosInstance.get(`/${args.jobPath}/api/json?tree=${treeQuery}`);
    ...
    private async stopBuild(args: StopBuildArgs) {
        ...
        await this.axiosInstance.post(`/${args.jobPath}/${buildNumber}/stop`);
