...
describe("thread names in the Codex state database", () => {
    ...
    test("a state database a live process holds is SKIPPED, and the rollout copy still runs", async () => {
        // The busy probe was extended to cover these databases precisely because the name write
        // opens them read-write, and nothing consumed that answer for the destination. It is a
        // skip rather than a refusal on purpose: the destination is the user's primary home, Codex
        // is open on it much of the time, and the name carry is not opt-in — refusing the whole
        // migration over it would cost more than the bug.
        const root = scratch();
        const shared = { date: "2026-09-01", uuid: uuid("0051") };
        const destination = makeHome(root, ".codex", [shared]);
        const source = makeHome(root, ".codex-named", [shared, { date: "2026-09-02", uuid: uuid("0052") }]);
        stateDatabase(destination, [{ id: uuid("0051") }]);
        stateDatabase(source, [{ id: uuid("0051"), name: "astra-pricing" }]);
        ...
        const applied = await migrateHome({
            ...
            backupRoot: join(root, "backups"),
            apply: true,
            stamp: "20260914-190000",
            inspectOpenFiles: (query) =>
                (query.files ?? []).includes(join(destination, "state_5.sqlite"))
                    ? [{ pid: 4242, command: "codex", path: join(destination, "state_5.sqlite") }]
                    : [],
        });

        // Skipped, not refused: the run completed and the rollout the source alone held was copied.
        expect(applied.refusals).toEqual([]);
        expect(applied.totals.copied).toBe(1);
        expect(applied.sessionNames[0]).toMatchObject({ stateSkippedBusy: 1, stateAdded: 0, stateFailed: 0 });
        // Nothing was written under the live process.
        expect(threadName(destination, uuid("0051"))).toBeNull();
    });

    test("NEGATIVE CONTROL: the same migration writes the name when nothing holds the database", async () => {
        // Without this, a skip that fired unconditionally would look exactly like the fix working.
        const root = scratch();
        const shared = { date: "2026-09-01", uuid: uuid("0051") };
        const destination = makeHome(root, ".codex", [shared]);
        const source = makeHome(root, ".codex-named", [shared]);
        stateDatabase(destination, [{ id: uuid("0051") }]);
        stateDatabase(source, [{ id: uuid("0051"), name: "astra-pricing" }]);
        ...
        const applied = await migrateHome({
            ...
            backupRoot: join(root, "backups"),
            apply: true,
            stamp: "20260914-190100",
            inspectOpenFiles: clear,
        });

        expect(applied.sessionNames[0]).toMatchObject({ stateSkippedBusy: 0, stateAdded: 1, stateFailed: 0 });
        expect(threadName(destination, uuid("0051"))).toBe("astra-pricing");
    });

    test("a DRY RUN predicts the busy skip instead of counting a name the apply will not carry", async () => {
        // The skip used to be gated on `apply`, so the plan promised a name the apply then skipped.
        // `tools codex migrate-home` turned that count into a danger confirm and a full backup, and
        // a `--json` consumer read `stateSkippedBusy: 0` on every dry run.
        const root = scratch();
        const shared = { date: "2026-09-01", uuid: uuid("0061") };
        const destination = makeHome(root, ".codex", [shared]);
        const source = makeHome(root, ".codex-named", [shared]);
        stateDatabase(destination, [{ id: uuid("0061") }]);
        stateDatabase(source, [{ id: uuid("0061"), name: "astra-pricing" }]);

        const planned = await migrateHome({
            from: [source],
            to: destination,
            backupRoot: join(root, "backups"),
            stamp: "20260914-190200",
            inspectOpenFiles: (query) =>
                (query.files ?? []).includes(join(destination, "state_5.sqlite"))
                    ? [{ pid: 4242, command: "codex", path: join(destination, "state_5.sqlite") }]
                    : [],
        ...
        expect(planned.sessionNames[0]).toMatchObject({ stateSkippedBusy: 1, stateAdded: 0, stateFailed: 0 });
        // Diagnostics do not mutate: the plan still wrote nothing into the held database.
        expect(threadName(destination, uuid("0061"))).toBeNull();
    });
});
