    ...
    try {
        ...
        server.on("upgrade", (request, connection, head) => {
            ...
            sockets.handleUpgrade(request, connection, head, (ws) => {
                ...
                // Both `close` and `error` land here, and one peer can emit both. `peers.delete`
                // returning false is what makes the teardown idempotent, so the last-peer check
                // below runs once per peer rather than once per event.
                const forget = () => {
                    if (!peers.delete(ws)) {
                        return;
                    }
