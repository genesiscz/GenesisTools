...
async function fetchGitHubGraphQL(token: string, query: string, variables: Record<string, unknown>): Promise<unknown> {
...
async function fetchAdditionalComments(token: string, threadId: string, startCursor: string): Promise<Comment[]> {
...
async function replyToThread(token: string, pullRequestReviewThreadId: string, body: string): Promise<string> {
    const query = `
...
function formatThread(thread: ParsedThread): string {
    ...
    output +=
        c(`[THREAD #${thread.threadNumber}] `, "bold") +
        `${severityIcon} ${c(severityText, "bold")} - ${thread.title}\n`;
    output += c("=".repeat(90), "cyan") + "\n";
    ...
    if (thread.replies.length > 0) {
        ...
        for (const reply of thread.replies) {
            output +=
                c(`  > ${reply.author} (${c(reply.id, "dim")}): `, "dim") +
                reply.body.split("\n")[0].substring(0, 60) +
                "\n";
        }
    }
...
function formatSummary(
    ...
    output +=
        c("|", "cyan") + c(`  PR #${prNumber}: `, "bold") + title.substring(0, 70).padEnd(78) + c("|", "cyan") + "\n";
...
Examples:
  ...
  $ tools github-pr 137 -r "fixed" --resolve-thread -t <thread-id>   # Reply AND resolve in one command`
    ...
    // Terminal output
    console.log(
        formatSummary(input.owner, input.repo, input.prNumber, prInfo.title, prInfo.state, parsedThreads, fullStats)
    );
