...
/** Resolve a URL-relative path safely inside `dir`; null when it escapes or is missing. */
function safeResolve(dir: string, relUrl: string): string | null {
    const decoded = decodeURIComponent(relUrl);
 ...
 */
function resolveCleanUrl(dir: string, urlPath: string): CleanResolution | null {
    ...
    if ((full === dir || full.startsWith(dir + sep)) && existsSync(full) && statSync(full).isDirectory()) {
    ...
    return {
        ...
        configureServer(server) {
            server.middlewares.use((req, res, next) => {
                ...
                const serveMdPage = async (file: string): Promise<void> => {
                    const rendered = await marked.parse(readFileSync(file, "utf8"));
                    respond(
                ...
                const handle = async (): Promise<void> => {
                    ...
                    if (path === "/__catalog" || (path === "/" && !existsSync(join(dir, "index.html")))) {
                        respond(renderCatalogHtml(dir, scanArtifacts(dir), templateDir, urlBase));
                    ...
                    if (tsxMatch) {
                        ...
                        if (sub && lastSegment.includes(".")) {
                            req.url = `${urlBase}${sub}${url.search}`;
                            next();
                    ...
                    if (viewerAsset && !/\.(tsx|jsx|md)$/.test(path)) {
                        req.url = `${urlBase}/${viewerAsset[1]}${url.search}`;
                        next();
                    ...
                    if (!path.startsWith("/__") && !lastSegment.includes(".")) {
                        ...
                        if (clean?.kind === "html") {
                            req.url = `${urlBase}/${clean.rel}${url.search}`;
                            next();
                        ...
                        if (clean?.kind === "dir") {
                            ...
                            const sub = scanArtifacts(subDir);
