...
function ChatPage() {
    ...
    useEffect(() => {
        fetch("/demo/api/available-providers")
            ...
            .then((data) => {
                ...
                if (storedPref && availableOptions.some((o) => o.model === storedPref.model)) {
    ...
    const canPlayTTS = selectedModel && hasCapability(selectedModel.provider, "tts");
    const canRecordAudio = selectedModel && hasCapability(selectedModel.provider, "transcription");
    ...
    const handleModelChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const selected = availableModelOptions.find((m) => `${m.provider}:${m.model}` === e.target.value);
        if (selected) {
            ...
            <div className="relative flex h-[calc(100vh-80px)] bg-gray-900">
                <div className="flex-1 flex items-center justify-center px-4">
                    <div className="text-center max-w-xl">
                        <h1 className="text-2xl font-bold text-white mb-4">No AI Providers Available</h1>
                        <p className="text-gray-400 mb-4">
                            Please configure at least one AI provider in your{" "}
                            <code className="text-orange-400">.env.local</code> file, or ensure Ollama is running
                            locally.
                        </p>
                    </div>
                </div>
