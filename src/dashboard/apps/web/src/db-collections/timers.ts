...
export async function getTimers(userId: string): Promise<TimerRecord[]> {
    const results = await db.getAll<TimerRow>("SELECT * FROM timers WHERE user_id = ? ORDER BY created_at DESC", [
        userId,
    ]);
 ...
 */
export function watchTimers(userId: string, callback: (timers: TimerRecord[]) => void): () => void {
    ...
    const subscription = query.subscribe({
        next: (results) => {
            const timers = (results.rows?._array ?? []).map((row: TimerRow) => timerSchema.parse(row));
            callback(timers);
        },
        error: (err) => {
