...
export function hasCapability(provider: Provider, capability: keyof VendorCapabilities): boolean {
    return VENDOR_CAPABILITIES[provider]?.[capability] ?? false;
}

export function getProvidersWithCapability(capability: keyof VendorCapabilities): Provider[] {
