...
function GuitarRecommendation({ id }: { id: string }) {
    return (
        <div className="rounded-lg border border-orange-500/20 bg-gray-800/50 p-4">
            ...
            <p className="text-sm text-gray-400 mb-3">This is a placeholder for the guitar recommendation feature.</p>
            <div className="text-xs text-gray-500">Recommendation ID: {id}</div>
        </div>
    );
}
        ...
        <div ref={messagesContainerRef} className="flex-1 overflow-y-auto">
            {messages.map(({ id, role, parts }) => (
                <div
                    key={id}
                    className={`py-3 ${
                        role === "assistant" ? "bg-linear-to-r from-orange-500/5 to-red-600/5" : "bg-transparent"
                    }`}
                >
