...
export function useCelebrations(userId: string | null) {
    ...
    const pendingCelebrations: Celebration[] = useMemo(() => {
        if (useFallback) {
            return fallbackCelebrations.filter((c) => !c.shownAt && !c.dismissed);
        }
    ...
    async function showNextCelebration(): Promise<Celebration | null> {
        ...
        if (useFallback) {
            try {
                ...
                setFallbackCelebrations((prev) => prev.map((c) => (c.id === pending.id ? shown : c)));
    ...
    async function dismissActiveCelebration(): Promise<void> {
        ...
        if (useFallback) {
            ...
            setFallbackCelebrations((prev) => prev.map((c) => (c.id === active.id ? { ...c, dismissed: true } : c)));
        } else {
            try {
    ...
    async function determineTier(completionType: CompletionType): Promise<CelebrationTier> {
        ...
        try {
            const adapter = useFallback ? getAssistantStorageAdapter() : await initializeAssistantStorage();
