...
import { generateTaskId, generateCompletionId, generateBadgeId, BADGE_DEFINITIONS } from '@/lib/assistant/types'
...
export function useTaskStore(userId: string | null) {
  ...
  const tasks: Task[] = useMemo(() => {
    if (useFallback) return state.fallbackTasks
  ...
  const streak: Streak | null = useMemo(() => {
    if (useFallback) return state.fallbackStreak
    if (!streakQuery.data) return null
    ...
    return {
      ...
      streakResetDate: streakQuery.data.streakResetDate
        ? new Date(streakQuery.data.streakResetDate)
        : undefined,
    }
  ...
  const badges: Badge[] = useMemo(() => {
    if (useFallback) return state.fallbackBadges
  ...
  async function updateTask(id: string, updates: TaskUpdate): Promise<Task | null> {
    ...
    if (updates.projectId !== undefined) serverUpdates.projectId = updates.projectId
    if (updates.deadline !== undefined)
      serverUpdates.deadline = updates.deadline?.toISOString() ?? null
    if (updates.urgencyLevel !== undefined) serverUpdates.urgencyLevel = updates.urgencyLevel
    ...
    if (updates.linkedGitHub !== undefined) serverUpdates.linkedGitHub = updates.linkedGitHub
    if (updates.blockedBy !== undefined) serverUpdates.blockedBy = updates.blockedBy
    if (updates.blocks !== undefined) serverUpdates.blocks = updates.blocks
    ...
    if (updates.focusTimeLogged !== undefined) serverUpdates.focusTimeLogged = updates.focusTimeLogged
  ...
  async function completeTask(
      ...
      try {
        await upsertStreakMutation.mutateAsync({
          ...
          longestStreakDays: Math.max(
            newStreakData.currentStreakDays,
    ...
    for (const def of BADGE_DEFINITIONS) {
      if (earnedBadgeTypes.includes(def.type)) continue
...
function calculateStreak(
  completedTasks: Task[],
  now: Date
): { currentStreakDays: number; streakResetDate?: Date } {
