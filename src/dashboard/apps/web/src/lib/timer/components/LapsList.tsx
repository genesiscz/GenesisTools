...
import { ChevronDown, ChevronUp, Trash2, Trophy, Zap } from "lucide-react";
 ...
 */
export const LapsList = memo(function LapsList({ laps, onClear, maxVisible = 3, className }: LapsListProps) {
    const [expanded, setExpanded] = useState(false);

    // Find best and worst laps (excluding first lap)
    const { bestLapIndex, worstLapIndex } = useMemo(() => {
        if (laps.length <= 1) {
            return { bestLapIndex: -1, worstLapIndex: -1 };
        }
        ...
        comparableLaps.forEach((lap, i) => {
            if (lap.lapTime < comparableLaps[best].lapTime) {
                best = i;
            }
            if (lap.lapTime > comparableLaps[worst].lapTime) {
                worst = i;
            }
        });
