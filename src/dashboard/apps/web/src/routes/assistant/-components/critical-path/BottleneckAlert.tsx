 ...
 */
export function BottleneckAlert({ task, blockedTasks, className, onViewGraph }: BottleneckAlertProps) {
    ...
    return (
        <div
            className={cn(
                ...
                isUrgent ? "bg-red-500/10 border-red-500/30" : "bg-orange-500/10 border-orange-500/30",
                className
            )}
        >
            ...
            <div className="relative">
                ...
                <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                        <div className={cn("p-1.5 rounded-lg", isUrgent ? "bg-red-500/20" : "bg-orange-500/20")}>
                            <AlertTriangle className={cn("h-5 w-5", isUrgent ? "text-red-400" : "text-orange-400")} />
                        </div>
                        <div>
                            <h4 className={cn("font-semibold text-sm", isUrgent ? "text-red-300" : "text-orange-300")}>
                                Bottleneck Alert
                            </h4>
                ...
                >
                    <Zap className={cn("h-4 w-4", isUrgent ? "text-red-400" : "text-orange-400")} />
                    <p className={cn("text-xs font-medium", isUrgent ? "text-red-300" : "text-orange-300")}>
                        {isUrgent
 ...
 */
export function BottleneckBadge({ count, className }: { count: number; className?: string }) {
    if (count === 0) {
        return null;
    }
