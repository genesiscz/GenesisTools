...
export function DependencySelector({
    ...
    const blockedTasks = allTasks.filter((t) => t.blockedBy?.includes(taskId) && t.id !== taskId);

    // Available tasks for selection (not self, not already selected, not completed)
    const availableTasks = allTasks.filter(
        (t) => t.id !== taskId && !currentDependencies.includes(t.id) && t.status !== "completed"
    );
        ...
        <div className={cn("space-y-4", className)}>
            ...
            <div>
                        ...
                        <DialogContent className="sm:max-w-md">
                            ...
                            <div className="max-h-64 overflow-y-auto space-y-1">
                                {filteredTasks.length === 0 ? (
                                    <p className="text-sm text-muted-foreground text-center py-4">No available tasks</p>
                                ) : (
                ...
                {dependencyTasks.length === 0 ? (
                    <p className="text-xs text-muted-foreground">No dependencies. This task can start anytime.</p>
                ) : (
            ...
            <div>
                ...
                {blockedTasks.length === 0 ? (
                    <p className="text-xs text-muted-foreground">This task doesn't block any other tasks.</p>
                ) : (
                    <div className="space-y-1.5">
