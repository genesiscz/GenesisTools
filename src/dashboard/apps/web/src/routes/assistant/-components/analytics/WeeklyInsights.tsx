import { Terminal, TrendingUp, TrendingDown, Clock, Zap, AlertTriangle } from 'lucide-react'
...
function generateInsights(
  ...
  if (dayEntries.length >= 3) {
    const weakestDay = dayEntries.reduce((min, [day, val]) =>
      val < (energyByDay[min] ?? 5) ? day : min
    , dayEntries[0][0])
    ...
    if (weakestDayEnergy < 3) {
      insights.push({
        ...
        text: `${dayNames[parseInt(weakestDay)]} is your weak spot. Try lighter tasks or take a break.`,
        type: 'warning',
      })
    }
  }
  ...
  const deepFocusPercent = review.totalMinutes > 0
    ? (review.deepFocusMinutes / review.totalMinutes) * 100
    : 0
