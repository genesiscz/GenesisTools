...
import type { Badge, Streak, Task, UrgencyLevel } from "@/lib/assistant/types";
...
import { CELEBRATION_MESSAGES, DEFAULT_CELEBRATION_SETTINGS, getRandomMessage } from "./types";
...
interface CelebrationContextValue {
    ...
    celebrateTaskCompletion: (task: Task, streak: Streak | null, newBadges: Badge[], totalCompleted: number) => void;
...
export function CelebrationManagerProvider({ children }: CelebrationManagerProviderProps) {
    ...
    function celebrateTaskCompletion(task: Task, streak: Streak | null, newBadges: Badge[], totalCompleted: number) {
        ...
        if (shouldShowBadge && settings.mode === "full-party") {
            ...
            const celebration: BadgeCelebrationData = {
                ...
                message: badgeName ? `You unlocked: ${badgeName}` : `${totalCompleted} tasks completed!`,
                trigger: "task-complete",
                badgeName,
        ...
        const celebration: MicroCelebrationData = {
            ...
            icon: "check",
            accent:
                task.urgencyLevel === "critical" ? "amber" : task.urgencyLevel === "important" ? "purple" : "emerald",
        };
    ...
    return (
        <CelebrationContext.Provider value={contextValue}>
            ...
            {state.activeMicroCelebrations.map((celebration, index) => (
                <div key={celebration.id} style={{ transform: `translateY(-${index * 80}px)` }}>
                    <MicroCelebration
