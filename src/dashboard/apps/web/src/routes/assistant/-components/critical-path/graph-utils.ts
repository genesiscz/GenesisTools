...
import type { CriticalPathAnalysis, GraphEdge, GraphNode, NodePosition } from "./types";
...
export function calculateLayout(
    ...
    const horizontalSpacing = Math.max(nodeSize + 60, (width - 200) / Math.max(levelCount, 1));

    // Position nodes
    for (let level = 0; level <= maxLevel; level++) {
        const tasksAtLevel = levelGroups.get(level) ?? [];
        const verticalSpacing = Math.max(nodeSize + 40, (height - 200) / Math.max(tasksAtLevel.length, 1));
        ...
        tasksAtLevel.sort((a, b) => (priorityOrder[b.urgencyLevel] ?? 0) - (priorityOrder[a.urgencyLevel] ?? 0));

        tasksAtLevel.forEach((task, i) => {
            const x = 100 + level * horizontalSpacing;
            const y = 100 + i * verticalSpacing + (height - 200 - tasksAtLevel.length * verticalSpacing) / 2;
 ...
 */
export function buildGraphNodes(tasks: Task[], width: number, height: number): GraphNode[] {
            ...
            for (const depId of task.blockedBy) {
                if (taskIds.has(depId)) {
                    edges.push({
                        from: depId,
                        to: task.id,
                        isOnCriticalPath: criticalPathIds.has(depId) && criticalPathIds.has(task.id),
                    });
                }
            }
        }
...
export function analyzeCriticalPath(tasks: Task[]): CriticalPathAnalysis {
    ...
    for (const task of incompleteTasks) {
        const deps = task.blockedBy?.filter((d) => incompleteTasks.some((t) => t.id === d));
