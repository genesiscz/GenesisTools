...
export function BadgeCard({
    ...
    const badgeElement = (
        <div
            className={cn("relative flex flex-col items-center gap-1", isClickable && "cursor-pointer", className)}
            onClick={onClick}
        >
            ...
            <div
                className={cn(
                    ...
                    isLegendary && earned && ["shadow-lg", config.shadowStyle]
                )}
                style={
                    isLegendary && earned
                        ? {
            ...
            >
                ...
                {(rarity === "rare" || rarity === "legendary") && earned && (
                    <>
                        <div
                            className={cn(
                                "absolute top-0 left-0 w-2 h-2 border-l border-t rounded-tl",
                                config.borderColor
                            )}
                        />
                        <div
                            className={cn(
                                "absolute top-0 right-0 w-2 h-2 border-r border-t rounded-tr",
                                config.borderColor
                            )}
                        />
                        <div
                            className={cn(
                                "absolute bottom-0 left-0 w-2 h-2 border-l border-b rounded-bl",
                                config.borderColor
                            )}
                        />
                        <div
                            className={cn(
                                "absolute bottom-0 right-0 w-2 h-2 border-r border-b rounded-br",
                                config.borderColor
                            )}
                        />
                    </>
                ...
                {/* Icon */}
                <IconComponent
                    className={cn("relative z-10", sizeClasses.icon, earned ? config.iconColor : "text-gray-600")}
                />
    ...
    return (
        <Tooltip>
            ...
            <TooltipContent className={cn("max-w-xs p-3 space-y-2", "bg-[#0a0a14]/95 border", config.borderColor)}>
                <div className="flex items-center gap-2">
                    <IconComponent className={cn("h-4 w-4", config.iconColor)} />
                    <span className={cn("font-semibold", config.labelColor)}>{displayName}</span>
                </div>
