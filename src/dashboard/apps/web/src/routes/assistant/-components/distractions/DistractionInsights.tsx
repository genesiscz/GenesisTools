...
function generateRecommendations(
  ...
  if (mostCommonSource === 'slack' || (bySource.slack && bySource.slack.count >= 3)) {
    recommendations.push({
      ...
      description:
        'Try enabling Do Not Disturb from 9-11am. Slack is your biggest distraction source.',
  ...
  if (mostCommonSource === 'email' || (bySource.email && bySource.email.count >= 3)) {
    recommendations.push({
      ...
      description:
        'Check email only at 9am, 1pm, and 5pm instead of continuously throughout the day.',
  ...
  if (bySource.meeting && bySource.meeting.count >= 2) {
    recommendations.push({
      ...
      description:
        'Block 2-3 hours each morning for deep work. Move meetings to afternoon slots.',
  ...
  if (bySource.hunger && bySource.hunger.count >= 2) {
    recommendations.push({
      ...
      description:
        'Set reminders for snacks at 10:30am and 3pm to prevent hunger-driven interruptions.',
  ...
  if (bySource.coworker && bySource.coworker.count >= 2) {
    recommendations.push({
      ...
      description:
        'Let colleagues know your availability windows. Use a "focus" status when deep working.',
...
export function DistractionInsights({
  ...
  distractions,
  className,
}: DistractionInsightsProps) {
  const [activeExperiment, setActiveExperiment] = useState<Experiment | null>(null)

  const recommendations = useMemo(
    () => generateRecommendations(stats, distractions),
    [stats, distractions]
  )
        ...
        {activeExperiment?.completed && (
          <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20">
            ...
            <p className="text-sm mb-2">
              Great job trying &quot;{activeExperiment.title}&quot;
            </p>
            <Button
              size="sm"
              variant="ghost"
