...
function getCellColor(quality: number, hasData: boolean): string {
  if (!hasData) return 'bg-slate-800/30'

  if (quality >= 4.5) return 'bg-cyan-400'
  if (quality >= 4) return 'bg-cyan-500/80'
  if (quality >= 3.5) return 'bg-cyan-600/60'
  if (quality >= 3) return 'bg-cyan-700/50'
  if (quality >= 2.5) return 'bg-amber-600/40'
  if (quality >= 2) return 'bg-amber-700/30'
  if (quality >= 1.5) return 'bg-red-800/40'
...
function getCellBorder(quality: number, hasData: boolean, isPeak: boolean, isLow: boolean): string {
  if (isPeak) return 'border-amber-400/60 shadow-amber-400/30'
  if (isLow) return 'border-red-500/40'
  if (!hasData) return 'border-slate-700/20'
  if (quality >= 4) return 'border-cyan-500/40'
...
export function HeatmapCell({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        side="top"
        className="bg-slate-900/95 border border-cyan-500/30 text-slate-100 px-3 py-2"
      >
        <div className="flex flex-col gap-1">
          ...
          {hasData ? (
            <>
              <span>
                Focus: <span className={cn(
                  'font-bold',
                  focusQuality >= 4 ? 'text-cyan-400' :
                  focusQuality >= 3 ? 'text-amber-400' : 'text-red-400'
                )}>{displayQuality}/5</span>
              </span>
              <span className="text-slate-400 text-xs">
                {contextSwitches} context {contextSwitches === 1 ? 'switch' : 'switches'} | {tasksCompleted} {tasksCompleted === 1 ? 'task' : 'tasks'}
              </span>
              {isPeak && (
                <span className="text-amber-400 text-xs font-medium">Peak focus time</span>
              )}
              {isLow && (
                <span className="text-red-400 text-xs font-medium">Low energy time</span>
              )}
            </>
          ) : (
...
export function EmptyCell() {
  return (
    <div className="w-full aspect-square rounded-sm bg-slate-900/20 border border-slate-800/10" />
  )
}
