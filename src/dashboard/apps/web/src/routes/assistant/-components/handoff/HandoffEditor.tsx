...
export function HandoffEditor({
    ...
    useEffect(() => {
        if (open) {
            ...
            const taskDecisions = availableDecisions.filter((d) => d.relatedTaskIds.includes(task.id));
            setSelectedDecisionIds(new Set(taskDecisions.map((d) => d.id)));

            // Pre-select active blockers
            const activeBlockers = availableBlockers.filter((b) => b.taskId === task.id && !b.unblockedAt);
    ...
    function handleMoveStep(index: number, direction: "up" | "down") {
        ...
        if (newIndex < 0 || newIndex >= nextSteps.length) {
            return;
        }
