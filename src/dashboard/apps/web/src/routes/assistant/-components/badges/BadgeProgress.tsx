      ...
      <div className="relative flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="absolute inset-0 flex justify-between px-[1px]">
              {[25, 50, 75].map((milestone) => (
                <div
                  ...
                  className={cn(
                    'w-px h-2',
                    percent >= milestone ? 'bg-white/20' : 'bg-gray-700/50'
                  )}
          ...
          <div className="flex items-center justify-between mt-1.5 text-[10px]">
            ...
            <span className={cn('uppercase tracking-wider', config.textColor)}>
              {progress.rarity}
            </span>
          </div>
        </div>
    ...
    return (
      <div className={cn('flex flex-col items-center justify-center py-8', className)}>
        ...
        <p className="text-sm text-muted-foreground text-center">
          All badges earned or no progress yet!
        </p>
      </div>
    )
  }
...
export function BadgeProgressCompact({
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      <span className={cn('text-xs font-mono min-w-[3ch]', config.textColor)}>
        {Math.round(percent)}%
      </span>
    </div>
  )
 ...
 */
export function NextBadgePreview({
  progress,
  className,
}: {
