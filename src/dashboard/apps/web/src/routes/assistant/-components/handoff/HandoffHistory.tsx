...
import type { HandoffDocument, Decision, TaskBlocker } from '@/lib/assistant/types'
...
export function HandoffHistory({
  ...
  const filteredHandoffs = handoffs.filter((h) => {
    if (filter === 'pending') return !h.reviewed
    if (filter === 'reviewed') return h.reviewed
    return true
  })

  // Get decisions and blockers for selected handoff
  const selectedDecisions = selectedHandoff
    ? decisions.filter((d) => selectedHandoff.decisions.includes(d.id))
    : []
  const selectedBlockers = selectedHandoff
    ? blockers.filter((b) => selectedHandoff.blockers.includes(b.id))
    : []
    ...
    return (
      <div className={cn('p-6 text-center', className)}>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Create a handoff when passing work to a teammate.
        </p>
      </div>
    )
  }
        ...
        {filteredHandoffs.length === 0 ? (
          ...
          filteredHandoffs.map((handoff) => (
            <HandoffCard
              key={handoff.id}
              handoff={handoff}
              onClick={() => handleViewHandoff(handoff)}
            />
          ))
        )}
      </div>
