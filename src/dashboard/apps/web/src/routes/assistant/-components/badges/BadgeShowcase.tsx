 ...
 */
export function BadgeShowcase({ badges, loading = false, filterRarity, onBadgeClick, className }: BadgeShowcaseProps) {
        ...
        <div className={cn("space-y-4", className)}>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-semibold">Earned Badges ({badges.length})</h3>
                </div>
...
export function BadgeShowcaseCompact({
    ...
    const sortedBadges = [...badges].sort((a, b) => raritySortOrder[a.rarity] - raritySortOrder[b.rarity]);
    ...
    return (
        <div className={cn("flex items-center gap-1", className)}>
            {displayBadges.map((badge) => (
                <BadgeCard key={badge.id} badge={badge} earned={true} size="sm" showName={false} />
            ))}
            {remaining > 0 && (
                <button
                    onClick={onViewAll}
