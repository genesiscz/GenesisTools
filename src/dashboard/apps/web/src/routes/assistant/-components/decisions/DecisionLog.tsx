...
export function DecisionLog({
  ...
  const chain = chainViewDecision && getDecisionChain
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
          {sortedDecisions.map((decision, index) => (
            ...
            >
              <DecisionCard
                decision={decision}
                onSupersede={onSupersedeDecision ? (id) => {
                  const d = decisions.find((dec) => dec.id === id)
                  if (d) setSupersedingDecision(d)
                } : undefined}
                onReverse={onReverseDecision ? (id) => {
                  const d = decisions.find((dec) => dec.id === id)
                  if (d) setReversingDecision(d)
                } : undefined}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setReversalReason(e.target.value)}
