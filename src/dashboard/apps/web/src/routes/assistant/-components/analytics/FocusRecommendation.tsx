...
import { Zap, X, ArrowRight, Brain, Coffee } from 'lucide-react'
...
function isInPeakWindow(heatmapData: EnergyHeatmapData | null): boolean {
  if (!heatmapData?.peakTime) return false
...
function isInLowWindow(heatmapData: EnergyHeatmapData | null): boolean {
  if (!heatmapData?.lowTime) return false
...
function getRecommendedTask(
  ...
  if (isHighEnergy) {
    ...
    if (importantTask) return importantTask
  } else {
...
export function FocusRecommendation({
  ...
  if (!heatmapData || (!isPeakTime && !isLowTime)) return null
 ...
 */
export function FocusRecommendationCompact({
  heatmapData,
  className,
}: Omit<FocusRecommendationProps, 'tasks'>) {
  const isPeakTime = isInPeakWindow(heatmapData)
  const isLowTime = isInLowWindow(heatmapData)
