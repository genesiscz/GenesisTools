...
import { Scale, Plus, X, Calendar, User, Tag } from 'lucide-react'
...
import type { DecisionInput, DecisionImpactArea, Task } from '@/lib/assistant/types'
...
export function DecisionForm({
  ...
  const [alternatives, setAlternatives] = useState<string[]>(
    initialValues?.alternativesConsidered ?? []
  )
  const [newAlternative, setNewAlternative] = useState('')
  const [impactArea, setImpactArea] = useState<DecisionImpactArea>(
    initialValues?.impactArea ?? 'frontend'
  )
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!title.trim() || !reasoning.trim() || !impactArea) return
