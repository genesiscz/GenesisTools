...
export function EnergyByDay({ review, loading }: EnergyByDayProps) {
  ...
  const weakSpot = nonZeroDays.length > 0
    ? nonZeroDays.reduce((min, d) => d.energy < min.energy ? d : min)
    : null
  const strongSpot = nonZeroDays.length > 0
    ? nonZeroDays.reduce((max, d) => d.energy > max.energy ? d : max)
    : null
