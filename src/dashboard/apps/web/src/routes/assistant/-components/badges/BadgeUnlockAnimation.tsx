 ...
 */
function ConfettiCanvas({ active, colors, className }: { active: boolean; colors: string[]; className?: string }) {
...
export function BadgeUnlockAnimation({ badge, open, onClose, onShare }: BadgeUnlockAnimationProps) {
    ...
    const definition = badge ? BADGE_DEFINITIONS.find((b) => b.type === badge.badgeType) : null;
    ...
    return (
        <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
            <DialogContent className={cn("sm:max-w-md overflow-hidden", "bg-[#0a0a14]/95 border-gray-800")}>
                {/* Confetti */}
                ...
                <DialogHeader className="relative text-center pt-4">
                    <DialogTitle
                        className={cn("text-2xl font-bold tracking-wide", rarity === "legendary" && "animate-pulse")}
                        style={{ color: config.primary }}
                    >
                        {config.title}
                    </DialogTitle>
                    <DialogDescription className="text-muted-foreground">You've earned a new badge</DialogDescription>
                </DialogHeader>
                ...
                <div className="relative flex flex-col items-center py-8">
                    ...
                    <div
                        className={cn(
                            ...
                            showBadge ? "scale-100 opacity-100 translate-y-0" : "scale-50 opacity-0 translate-y-8"
                        )}
                    >
                        {/* Outer glow ring */}
                        <div
                        ...
                        >
                            ...
                            {/* Icon */}
                            <IconComponent className="h-14 w-14 relative z-10" style={{ color: config.primary }} />
                        </div>
                    </div>
                ...
                <div className="relative flex gap-3 justify-center pb-4">
                    <Button variant="outline" size="sm" onClick={handleCopyShare} className="gap-2">
