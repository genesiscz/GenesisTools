...
export function LogForm({
  ...
  const [discussedAt, setDiscussedAt] = useState(
    initialValues?.discussedAt ? formatDateForInput(new Date(initialValues.discussedAt)) : formatDateForInput(new Date())
  )
  const [sentiment, setSentiment] = useState<CommunicationSentiment>(
    initialValues?.sentiment ?? 'context'
  )
      ...
      <DialogContent className="sm:max-w-[550px] bg-card border-border/50 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl">
            {isEdit ? 'Edit Entry' : 'Log Communication'}
          </DialogTitle>
        </DialogHeader>
