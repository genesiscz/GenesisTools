import { CheckCircle, Clock, Flame, Minus, TrendingDown, TrendingUp } from "lucide-react";
...
export function WeekStats({ review, streak, comparison, loading }: WeekStatsProps) {
    ...
    const deepFocusPercent =
        review && review.totalMinutes > 0 ? Math.round((review.deepFocusMinutes / review.totalMinutes) * 100) : 0;
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-3">
            ...
            <StatCard
                ...
                label="Focus Time"
                value={`${focusHours}h`}
                subValue={<span className="text-xs text-muted-foreground">{deepFocusPercent}% deep work</span>}
            />
            ...
            <StatCard
                ...
                subValue={
                    streak && streak.longestStreakDays > 0 ? (
                        <span className="text-xs text-muted-foreground">Best: {streak.longestStreakDays} days</span>
                    ) : null
                }
...
function StatCard({ icon: Icon, iconColor, iconBg, label, value, subValue, valueClassName }: StatCardProps) {
    return (
