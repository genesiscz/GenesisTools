  ...
  if (authLoading || (!initialized && loading)) {
    return (
      <DashboardLayout
        ...
        description="Capture important messages and decisions"
      >
...
function EmptyState({ onAddEntry }: { onAddEntry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 px-6">
      ...
      {/* Text */}
      <h2 className="text-xl font-semibold text-foreground/70 mb-2">
        No communications logged yet
      </h2>
      <p className="text-muted-foreground text-center max-w-md mb-8">
        Start capturing important messages, decisions, and context from your daily communications.
        Never lose track of key discussions again.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mb-8">
        <FeatureItem
          ...
          description="Save important threads"
        />
        <FeatureItem
          icon="github"
          title="GitHub Discussions"
          description="PR comments & issues"
        />
        <FeatureItem
          icon="email"
          title="Email Threads"
          description="Key email exchanges"
        />
        <FeatureItem
          icon="meeting"
          title="Meeting Notes"
          description="Decisions & action items"
        />
      </div>
