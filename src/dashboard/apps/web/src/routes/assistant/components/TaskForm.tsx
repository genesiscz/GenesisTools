...
import { AlertTriangle, AlertCircle, Sparkles, X, Calendar } from 'lucide-react'
...
export function TaskForm({
  ...
  const [urgency, setUrgency] = useState<UrgencyLevel>(
    initialValues?.urgencyLevel ?? 'nice-to-have'
  )
  const [deadline, setDeadline] = useState(
    initialValues?.deadline ? formatDateForInput(initialValues.deadline) : ''
  )
  const [isShippingBlocker, setIsShippingBlocker] = useState(
    initialValues?.isShippingBlocker ?? false
  )
          ...
          {urgency === 'critical' && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-red-500/10 border border-red-500/20">
              <input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIsShippingBlocker(e.target.checked)}
