    ...
    return (
        <div data-testid="key-result-row" className="rounded-lg border border-border/60 bg-muted/20 p-3">
            ...
            <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-border/50">
                <div
                    className={cn("h-full rounded-full transition-[width] duration-500 ease-out", "bg-current", colorClassName)}
                    style={{ width: `${pct}%` }}
                />
            </div>
            ...
            <div className="mt-2.5 flex items-center gap-1.5">
                ...
                <span className={cn("ml-auto font-mono text-xs font-semibold tabular-nums", colorClassName)}>{pct}%</span>
            </div>
        </div>
    );
}
