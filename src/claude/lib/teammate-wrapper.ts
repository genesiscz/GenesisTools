...
export function claudeBinaryCandidates(): string[] {
    ...
    return [
        ...
        // PATH is passed explicitly: Bun.which() with no options reads the environment bun
        // snapshotted at process start, not process.env.PATH, so it answers from the real machine
        // no matter what PATH says now. That made the suite's PATH isolation a no-op and hid the
        // last-resort branch behind whichever `claude` the developer happened to have installed.
        Bun.which("claude", { PATH: env.get("PATH") ?? "" }) ?? undefined,
...
export function buildTeammateWrapperScript(input: { claudeBin: string; env: TeammateAuthEnv }): string {
    ...
    const exportLines = exports.map(([k, v]) => `export ${k}=${shellQuote(v)}`).join("\n");
