...
function formatResultsAsMarkdown(results: SearchResult[], filters: SearchFilters): string {
	...
	const modeDesc = filters.summaryOnly
		? " (summary-only)"
	...
	for (let i = 0; i < results.length; i++) {
		...
		const relevanceStr =
			filters.sortByRelevance && result.relevanceScore !== undefined
		...
		lines.push(
			`**Date:** ${date}${result.gitBranch ? ` | **Branch:** ${result.gitBranch}` : ""}`,
		);
			...
			try {
				...
				if (options.interactive) {
					...
					if (!project && !options.all) {
						...
						const projectIndex = cwdParts.findIndex(
							(p: string) => p === "Projects" || p === "projects",
						);
						if (projectIndex !== -1 && cwdParts[projectIndex + 1]) {
							project = cwdParts[projectIndex + 1];
							console.log(
								chalk.dim(
									`Auto-detected project: ${project} (use --all to search all projects)`,
								),
							);
						}
					}
					...
					filters = {
						...
						conversationDateUntil: options.convDateUntil
							? parseDate(options.convDateUntil)
							: undefined,
					};
				}
