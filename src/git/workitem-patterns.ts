...
const SUGGEST_TEMPLATES: WorkitemPattern[] = [
	...
	{ regex: "(\\w+)-(\\d+)", source: "commit-message", captureGroup: 2, description: "PREFIX-NUMBER (commit message)" },
	{ regex: "feat\\(.*?(\\d{5,6})", source: "commit-message", captureGroup: 1, description: "feat(...ID) conventional commit" },
	{ regex: "fix\\(.*?(\\d{5,6})", source: "commit-message", captureGroup: 1, description: "fix(...ID) conventional commit" },
	{ regex: "COL-(\\d+)-", source: "branch-name", captureGroup: 1, description: "COL-XXXXX branch pattern" },
	{ regex: "(\\w+)-(\\d+)-", source: "branch-name", captureGroup: 2, description: "PREFIX-NUMBER-desc branch pattern" },
];
