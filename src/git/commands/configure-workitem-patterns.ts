...
async function getCommitMessages(repoPath: string, limit: number): Promise<{ messages: string[]; branches: string[] }> {
	...
	const messages = logResult.success
		? logResult.stdout.split("\n").filter((l) => l.trim())
		: [];

	const branchResult = await executor.exec([
		"for-each-ref",
	...
	while (running) {
		const action = await p.select({
			...
			options: [
				...
				{ value: "remove", label: "Remove an existing pattern", hint: currentPatterns.length === 0 ? "no patterns to remove" : undefined },
...
async function handleConfigureWorkitemPatterns(
	storage: Storage,
	options: ConfigurePatternsOptions,
): Promise<void> {
	if (options.list) {
