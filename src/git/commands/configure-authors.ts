...
async function getRepoAuthors(limit: number = 500): Promise<string[]> {
    ...
    const result = await executor.exec(["log", "--all", `-${limit}`, "--format=%an"]);
...
async function handleConfigureAuthors(storage: Storage, options: ConfigureAuthorsOptions): Promise<void> {
