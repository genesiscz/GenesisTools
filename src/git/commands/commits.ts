...
function outputTable(
	...
	for (const day of sortedDays) {
		...
		for (const commit of dayCommits) {
			...
			console.log(
				`  ${chalk.dim(commit.shortHash)} ${commit.message}${workitemTag}${statTag}${authorTag}`,
			);
		}
	}
	...
	if (workitemMap.size > 0) {
		...
		for (const [id, stats] of workitemMap) {
			const statPart =
				includeStat ? chalk.dim(` (+${stats.totalInsertions}/-${stats.totalDeletions})`) : "";
...
async function handleCommits(options: CommitsOptions): Promise<void> {
	...
	if (options.format !== "json") {
		...
		console.log(
			chalk.bold(`Finding git commits from ${options.from} until ${options.to} from ${authorDisplay}`),
		);
	}
