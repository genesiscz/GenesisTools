...
function formatIssueSummary(data: IssueData, options: FormatOptions): string {
  ...
  if (stats.dateRange.start && stats.dateRange.end) {
    lines.push(`| Date Range | ${formatDateShort(stats.dateRange.start)} → ${formatDateShort(stats.dateRange.end)} |`);
  }
  lines.push('');
...
function formatIssueMarkdown(data: IssueData, options: FormatOptions): string {
  ...
  if (data.comments.length > 0) {
    ...
    for (let i = 0; i < data.comments.length; i++) {
      ...
      const commentLink = comment.htmlUrl || `https://github.com/${data.owner}/${data.repo}/issues/${data.issue.number}#issuecomment-${comment.id}`;
      lines.push(`### Comment ${i + 1} — @${comment.author} · [${formatDate(comment.createdAt)}](${commentLink})`);
...
function buildIndex(data: IssueData): IndexEntry[] {
  ...
  if (data.events.length > 0) {
    ...
    index.push({
      ...
      dateRange: firstEventDate && lastEventDate
  ...
  if (data.comments.length > 0) {
    ...
    for (let i = 0; i < data.comments.length; i += COMMENTS_PER_INDEX_GROUP) {
      ...
      const label = data.comments.length <= COMMENTS_PER_INDEX_GROUP
        ? 'Comments'
        : `Comments ${i + 1}-${groupEnd}`;
