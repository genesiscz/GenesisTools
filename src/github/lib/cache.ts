...
export function upsertIssue(data: Omit<IssueRecord, 'id'>): IssueRecord {
  ...
  if (existing) {
    db.query(`
      ...
      WHERE id = ?
    `).run(
      data.type, data.title, data.body, data.state, data.author,
      data.created_at, data.updated_at, data.closed_at,
      data.last_fetched, data.last_comment_cursor,
      existing.id
    );
  ...
  const result = stmt.get(
    data.repo_id, data.number, data.type, data.title, data.body,
    data.state, data.author, data.created_at, data.updated_at,
    data.closed_at, data.last_fetched, data.last_comment_cursor
  ) as { id: number };
...
export function getComments(issueId: number, options: {
  limit?: number;
  offset?: number;
  since?: string;
  before?: string;
...
export function getLastNComments(issueId: number, n: number, options: {
  excludeBots?: boolean;
  minReactions?: number;
  author?: string;
} = {}): CommentRecord[] {
...
export function upsertComment(data: CommentRecord): void {
  ...
  `).run(
    data.id, data.issue_id, data.author, data.body,
    data.created_at, data.updated_at, data.reaction_count,
    data.reactions_json, data.is_bot
  );
}
...
export function upsertComments(comments: CommentRecord[]): void {
  ...
  const transaction = db.transaction((items: CommentRecord[]) => {
    for (const c of items) {
      stmt.run(
        c.id, c.issue_id, c.author, c.body,
        c.created_at, c.updated_at, c.reaction_count,
        c.reactions_json, c.is_bot
      );
    }
  });
...
export function getTimelineEvents(issueId: number): TimelineEventRecord[] {
  const db = getDatabase();
  return db.query('SELECT * FROM timeline_events WHERE issue_id = ? ORDER BY created_at ASC')
