 ...
 */
async function fetchTimelineEvents(owner: string, repo: string, number: number): Promise<GitHubTimelineEvent[]> {
    const octokit = getOctokit();
 ...
 */
export async function issueCommand(input: string, options: IssueCommandOptions): Promise<void> {
...
async function issueSingleCommand(input: string, options: IssueCommandOptions): Promise<void> {
    // Initialize database
    getDatabase();
