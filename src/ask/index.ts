...
import { commandHandler, CommandResult } from "./chat/CommandHandler";
...
class ASKTool {
  ...
  private async startInteractiveChat(argv: Args): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.blue(`> Starting with ${colorizeProvider(modelChoice.provider.name)}/${modelChoice.model.name}`));
        ...
        try {
          ...
          const { message } = await this.prompter.prompt({
            ...
            validate: (input: string) => {
              if (input.startsWith("/")) {
                return commandHandler.isValidCommand(input) || "Unknown command. Type /help for available commands.";
              }
          ...
          if (chatEngine.getConversationLength() % 10 === 0) { // Every 5 pairs (user + assistant)
      ...
      console.log(chalk.gray(`Duration: ${formatElapsedTime(Date.now() - new Date(session.startTime).getTime())}`));
  ...
  private getAvailableTools(): Record<string, {
    description: string;
    parameters: {
      [key: string]: {
        type: string;
  ...
  private async handleCommandResult(
    ...
    if (result.transcriptionFile) {
      try {
        ...
        await chatEngine.sendMessage(`Transcription of "${result.transcriptionFile}":\n\n${transcriptionResult.text}`);

        console.log(chalk.green(` Transcription completed in ${formatElapsedTime(transcriptionResult.processingTime)}`));
