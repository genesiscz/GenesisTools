...
export class ModelSelector {
  ...
  async selectProvider(providers: DetectedProvider[]): Promise<DetectedProvider | null> {
    const choices = providers.map((provider) => ({
      name: provider.name,
      message: `${chalk.cyan(provider.name)} - ${provider.config.description || `${provider.models.length} models available`}`,
      value: provider,
    }));
    ...
    try {
      ...
      if (typeof response.model === 'string') {
        try {
          ...
          const parsed = JSON.parse(response.model);
          modelName = parsed.id;
        } catch {
        ...
        modelName = response.model.id || response.model;
      }

      const choice = choices.find(c => c.name === modelName);
      const selectedModel = choice ? choice.value : null;
  ...
  private formatModelChoice(model: ModelInfo): string {
    const parts = [
      ...
      chalk.gray(`(${this.formatTokens(model.contextWindow)} tokens)`),
    ];

    if (model.pricing) {
      const costStr = model.pricing.input && model.pricing.output
        ? chalk.yellow(`$${(model.pricing.input / 1000).toFixed(4)}/${(model.pricing.output / 1000).toFixed(4)} per 1K tokens`)
    ...
    if (model.capabilities.length > 0) {
      const caps = model.capabilities.map(cap => {
        switch (cap) {
          case "vision": return chalk.blue("👁️");
          case "function-calling": return chalk.magenta("🔧");
          case "reasoning": return chalk.red("🧠");
          default: return chalk.gray(cap);
        }
      }).join(" ");
      parts.push(caps);
    }
      ...
      if (modelName && !providerName) {
        for (const provider of providers) {
          ...
          if (model) {
            ...
            targetModel = model;
            logger.info(`Auto-selected provider ${chalk.cyan(provider.name)} for model ${chalk.yellow(modelName)}`);
            break;
          }
        }
        ...
        if (providerName) {
          ...
          if (!targetProvider) {
            logger.error(`Provider "${providerName}" not found. Available: ${providers.map(p => p.name).join(", ")}`);
            return null;
          }
        ...
        if (modelName) {
          targetModel = targetProvider.models.find(m =>
            m.id === modelName || m.name === modelName
          );
          if (!targetModel) {
  ...
  async selectTranscriptionModel(fileSize?: number): Promise<{ provider: string; model: string; providerInstance: ProviderV1 } | null> {
    const transcriptionProviders = [
      { name: "groq", envKey: "GROQ_API_KEY", model: "whisper-large-v3", maxFileSize: 25 * 1024 * 1024 },
      { name: "openrouter", envKey: "OPENROUTER_API_KEY", model: "openai/whisper-1", maxFileSize: 25 * 1024 * 1024 },
    ...
    if (availableProviders.length === 0) {
      ...
      logger.info("Supported: GROQ_API_KEY, OPENROUTER_API_KEY, OPENAI_API_KEY, ASSEMBLYAI_API_KEY, DEEPGRAM_API_KEY, GLADIA_API_KEY");
      return null;
    }
