...
export class TranscriptionManager {
  ...
  async transcribeAudio(
    ...
    try {
      ...
      logger.info(`Using ${chalk.green(transcriptionModel.provider)} with model ${chalk.yellow(transcriptionModel.model)}`);
      ...
      const result = await transcribe({
        model: transcriptionModel.providerInstance(transcriptionModel.model),
        audio: audioBuffer,
      ...
      if (result.duration) {
        transcriptionResult.duration = result.duration;
      }

      logger.info(
        `Transcription completed in ${chalk.green((processingTime / 1000).toFixed(1))}s`
      );
