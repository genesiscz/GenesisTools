...
describe("focusSessionPane", () => {
    ...
    test("an id that would be read as a CLI flag never spawns anything either", async () => {
        const calls: string[][] = [];
        // The route hands this straight through from an unauthenticated POST body.
        const result = await focusSessionPane("--json", { run: runner({}, calls) });
