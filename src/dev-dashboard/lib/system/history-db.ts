...
export class PulseHistoryDb {
    ...
    constructor(dbPath: string = DEFAULT_DB_PATH) {
        ...
        this.db.run(
            "CREATE TABLE IF NOT EXISTS pulse_kv (k TEXT PRIMARY KEY, v TEXT, updated_at TEXT)",
        );
    }
    ...
    recordAt(metric: string, value: number, ts: string): void {
        this.db
            .prepare("INSERT INTO pulse_points (metric, ts, value) VALUES (?, ?, ?)")
            .run(metric, ts, value);
    }
    ...
    series(metric: string, minutes: number): PulsePoint[] {
        ...
        const rows = this.db
            .prepare(
                "SELECT ts, value FROM pulse_points WHERE metric = ? AND ts >= ? ORDER BY ts ASC",
            )
    ...
    pruneOlderThan(hours: number): number {
        ...
        const before = this.db
            .prepare("SELECT COUNT(*) AS n FROM pulse_points WHERE ts < ?")
            .get(cutoff) as CountRow;
    ...
    getPublicIp(maxAgeMs: number): string | null {
        const row = this.db
            .prepare("SELECT v, updated_at FROM pulse_kv WHERE k = 'public_ip'")
            .get() as KvRow | null;
