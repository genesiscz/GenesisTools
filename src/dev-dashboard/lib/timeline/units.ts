...
/**
 * Platform-neutral timeline units, shared by the web dashboard and the mobile app. Both surfaces
 * render the same merged event stream, so the hour bucketing, the row time, the failed-run rule and
 * the pill labels have to be one implementation — they were duplicated byte-for-byte, and nothing
 * failed if only one copy was edited.
 *
 * What stays per platform is the DISPLAY payload alone: the web maps a kind to a CSS colour var,
 * mobile maps it to an icon name. `eventKind` carries everything else.
 */
...
/** The platform-neutral half of an event's presentation. */
...
/** Per-type tone + pill label. A failed run is the only one that flips to `danger`. */
export function eventKind(event: TimelineEvent): EventKind {
    if (event.type === "run") {
        const failed = event.exitCode !== null && event.exitCode !== 0;
        return { tone: failed ? "danger" : "accent", pillLabel: "RUN" };
    }

    if (event.type === "qa") {
        return { tone: "muted", pillLabel: "Q&A" };
    }

    return { tone: "muted", pillLabel: "TERM" };
}
