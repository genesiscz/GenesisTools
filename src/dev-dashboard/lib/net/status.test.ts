...
describe("deriveNetStatus", () => {
    it("low latency on a live transport = healthy, passes SSID + IP through", () => {
        ...
        expect(s).toEqual({ transport: "lan", latencyMs: 40, quality: "healthy", ssid: "Foltyn-5G", publicIp: "203.0.113.7" });
    });

    it("mid latency = degraded", () => {
        expect(deriveNetStatus({ pulse: PULSE, pingMs: LATENCY_HEALTHY_MS + 1, activeTransport: "tailscale" }).quality).toBe("degraded");
        expect(deriveNetStatus({ pulse: PULSE, pingMs: LATENCY_DEGRADED_MS, activeTransport: "tailscale" }).quality).toBe("degraded");
    });

    it("very high latency stays degraded (a slow link still works)", () => {
        expect(deriveNetStatus({ pulse: PULSE, pingMs: LATENCY_DEGRADED_MS + 500, activeTransport: "cloudflared-self" }).quality).toBe("degraded");
    });
