...
function QaFeed() {
    ...
    const deepLinkTried = useRef<Set<string>>(new Set());
    ...
    useEffect(() => {
        ...
        // Once per id. A cancelled or timed-out form has no `entryId`, so `pinnedId` never
        // changes, and this effect would then re-fire on every render: one GET per render.
        if (deepLinkTried.current.has(pinnedId)) {
            return;
        }

        deepLinkTried.current.add(pinnedId);
        let cancelled = false;
        qaPendingApi
            .get(pinnedId)
            .then(({ form }) => {
    ...
    useEffect(() => {
        ...
        // The value comes straight off the URL, so a quote in it would make querySelector
        // throw a DOMException and kill the whole effect.
        const safeId = CSS.escape(pinnedId);
        const el =
            document.querySelector(`[data-qa-pending-id="${safeId}"]`) ??
            document.querySelector(`[data-qa-id="${safeId}"]`);
    ...
    // With a pending form on screen the page is still useful, so a history failure must not
    // replace it. It gets a banner below instead of taking the whole route.
    ...
    return (
        <div className="relative flex flex-col gap-4">
            ...
            {logQuery.isError ? (
                <p className="dd-panel px-4 py-2 text-xs text-[var(--dd-danger)]" data-testid="qa-history-error">
                    The Q&amp;A history could not be loaded, so only the pending questions above are shown.
                </p>
            ) : null}

            {logQuery.isLoading ? (
                <div className="dd-panel py-8 text-center text-sm text-[var(--dd-text-muted)]">Loading Q&amp;A…</div>
            ) : logQuery.isError ? null : displayEntries.length === 0 ? (
                // The error banner above already said the history could not be loaded; an empty
                // list here is the FAILURE, not a real "nothing recorded yet".
