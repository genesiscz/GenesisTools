...
function tsImportSnippet(type: SnippetType, label: string): string {
    ...
    if (type === "assert") {
        return ["// #region @dbg", `dbg.assert(condition, '${label}');`, "// #endregion @dbg"].join("\n");
    }
    return ["// #region @dbg", `dbg.${type}('${label}', data);`, "// #endregion @dbg"].join("\n");
}
...
function phpImportSnippet(type: SnippetType, label: string): string {
    ...
    if (type === "assert") {
        return ["// #region @dbg", `${cls}::assert($condition, '${label}');`, "// #endregion @dbg"].join("\n");
    }
    return ["// #region @dbg", `${cls}::${type}('${label}', $data);`, "// #endregion @dbg"].join("\n");
}
