        ...
        <header className="sticky top-0 z-20 glass-card border-b border-white/8">
            <div className="px-3 sm:px-5 py-2.5 flex flex-wrap items-center gap-3">
                ...
                >
                    ...
                    {sessions.map((s) => {
                        const header = formatSessionHeaderParts(s);
                        const label = header.cwd
                            ? `[${header.badge}] ${header.name} · ${header.cwd}`
                            : `[${header.badge}] ${header.name}`;
            ...
            {meta ? <SessionHeaderLine session={meta} className="px-3 sm:px-5 pb-2 text-[11px]" /> : null}
        </header>
    );
}
