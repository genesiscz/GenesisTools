...
describe("formatSessionDatetimeSuffix", () => {
    it("formats YYYY-MM-dd_HH-mm-ss (no `:` — NTFS-safe)", () => {
...
describe("isRelatedSessionName", () => {
    it("matches base and collision-suffixed names only", () => {
        ...
        // Legacy colon-format suffixes from a prior build must NOT match —
        // they no longer occur in `formatSessionDatetimeSuffix` output and
        // colon chars are now rejected by SAFE_LOG_SESSION_NAME.
        expect(isRelatedSessionName("eval2-dup", "eval2-dup-2026-05-26_14:30:22")).toBe(false);
    });
});
