...
Pressing an `AXMenuButton` or `AXPopUpButton` opens an in-window menu, and the AX call returns before the menu exists. The press therefore waits for the menu and reports `menuOpened`; a swallowed press is retried exactly once, and only after a fresh read proves no menu is open, reported as `menuPressRetried`. `--refresh` waits up to three seconds for the menu rows and reports `after.awaited`. A press toggles, so pressing a control whose menu is already open is refused instead of silently closing it; close it with `--action perform --ax-action AXCancel` on that same control, which is delegated to the open `AXMenu` because that row carries no identifier of its own. A menu that is already closed answers `menuAlreadyClosed` rather than failing.
