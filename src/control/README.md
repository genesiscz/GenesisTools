...
When the app gives an element an `AXIdentifier`, `act --by-identifier <id>` skips the observation step entirely: the same process walks the app's windows, resolves the identifier and dispatches against that read, so there is no token and nothing in between for the world to move in. It searches every window, or only `--window-index N`, to `--depth N` (default 20). Zero matches and two matches are both refusals: the first names the windows it could not read and the SwiftUI identifier propagation that shadows per-control ids, the second lists every candidate. It is exclusive with `--snapshot`, `--element`, `--coords`, `--region`, `--target-key` and `--revalidate-scope`, because the identifier is both the selector and the identity that survives a repainting window.

```bash
tools control act --app Genesis --by-identifier focus-hud-primary --action press
```

Pressing an `AXMenuButton` or `AXPopUpButton` opens an in-window menu, and the AX call returns before the menu exists. The press (or a `perform` of `AXShowMenu`) therefore waits for the menu and reports `menuOpened`; a swallowed press is retried exactly once with the same action, and only after a fresh read proves no menu is open, reported as `menuPressRetried`. If no menu is seen even then, the action returns `ok: false` with an `error` saying the outcome is uncertain: the press was dispatched, so inspect rather than repeat. `--refresh` waits up to three seconds for the menu rows and reports `after.awaited`. A press toggles, so pressing a control whose menu is already open is refused instead of silently closing it; close it with `--action perform --ax-action AXCancel` on that same control, which is delegated to the open `AXMenu` because that row carries no identifier of its own. A menu that is already closed answers `menuAlreadyClosed` rather than failing.
