...
export function runCmd(cmd: string[], timeoutMs = 15_000): { ok: boolean; stdout: string; stderr: string } {
    const r = runCmdFull(cmd, timeoutMs);
    return { ok: r.ok, stdout: r.stdout.slice(0, 500), stderr: r.stderr.slice(0, 500) };
}

/**
 * 🛑 Use this, not `runCmd`, whenever the output is PARSED. `runCmd` clamps both streams to 500
 * characters so a diagnostic line stays readable, and JSON passes that mark quickly: `ax-tool
 * screens` is over 500 bytes at three displays and `ax-tool window` at three windows. The strict
 * parse then throws on the truncated tail and the native lookup falls back to Peekaboo, which
...
export function runCmdFull(cmd: string[], timeoutMs = 15_000): { ok: boolean; stdout: string; stderr: string } {
    ...
    return {
        ...
        stderr: r.stderr.toString().trim() || (r.exitCode !== 0 ? `timed out/killed after ${timeoutMs}ms` : ""),
    };
}
...
export function listScreens(): ScreenInfo[] {
    if (axToolAvailable()) {
        const native = runCmdFull([AX_TOOL_PATH, "screens"]);
...
function isV4Window(window: PeekabooV3Window | PeekabooV4Window): window is PeekabooV4Window {
    // `bounds` is the only field both versions always carry, and its SHAPE differs: v3 nests
    // tuples, v4 uses {x,y,width,height}. The label fields are optional, so an untitled v4
    // window used to fall into the v3 branch and throw on `w.bounds[0][0]` before it was added.
    if (typeof window.bounds === "object" && window.bounds !== null) {
        return !Array.isArray(window.bounds);
    }

    return "window_id" in window || "window_title" in window || "window_index" in window;
}
...
export function listWindowBounds(app: string): WindowBounds[] {
    if (axToolAvailable()) {
        const native = runCmdFull([AX_TOOL_PATH, "window", "--app", app]);
...
export interface CaptureAttempt {
    ...
    failDiag: string;
    /**
     * Basename of the binary this attempt actually spawned. The native backend falls back to
     * Peekaboo by replacing the attempt, not by moving `backend`, so this is the only field
     * that names the process a later diagnostic is describing.
     */
    tool: string;
}
...
export async function startCapture(argv: string[]): Promise<CaptureAttempt> {
    ...
    return { proc: p, sessionDir: dir, stdoutText, stderrText, failDiag, tool };
}
