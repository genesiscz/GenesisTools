...
describe("parseWindowList", () => {
    ...
    // Both label fields are optional on a v4 row. Keying the version off them sent an untitled
    // window into the v3 reader, which indexes bounds as a nested tuple and threw on
    // `w.bounds[0][0]` before the row was ever added — taking the whole lookup down with it.
    it("reads a Peekaboo 4 window that carries neither a title nor an index", () => {
        const data = {
            windows: [
                { window_id: 5150, is_key: true, is_on_screen: true, bounds: { x: 1, y: 2, width: 30, height: 40 } },
            ],
        };
        expect(() => parseWindowList(data)).not.toThrow();
        expect(parseWindowList(data)).toEqual([
            { title: "", index: 0, id: 5150, isMainWindow: true, x: 1, y: 2, w: 30, h: 40 },
        ]);
    });
