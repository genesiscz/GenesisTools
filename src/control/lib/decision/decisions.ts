...
function confidenceFor(result: EvaluationResponse, id: string): number | undefined {
...
export const exactExpectationSchema = z.object({ identifier: z.string().min(1), value: z.string() }).strict();
...
export async function judgeOutcome(
    ...
    if (options.exact) {
        ...
        const rows = options.observation.elements.filter((row) => row.AXIdentifier === exact.identifier);
        const matched = rows.length === 1 && String(rows[0].AXValue ?? "") === exact.value;
        return {
