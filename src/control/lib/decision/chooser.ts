...
import { type DecisionPolicy, resolveIntent } from "./decisions";
...
export async function chooseCandidate(options: {
    ...
    if (options.hostDecision) {
        const host = readHostDecision(options.hostDecision);
        const packet = escalationPacket({
            observation: options.observation,
            intent,
            candidates,
            signals: host.packet.signals,
            budget,
        });
        const admitted = acceptHostChoice({ response: host.answer, packet: host.packet, currentPacket: packet });
        if (host.packet.signals.conflict !== null && host.packet.signals.conflict >= 0.8) {
            return { ...base, packet, status: "escalated" as const, reason: "Observed conflict requires inspection." };
        }
        ...
        return {
            ...
            reason: selected
                ? "Host choice bound to current evidence without another model call."
                : "Host declined the action.",
...
export function readHostDecision(input: unknown): HostDecision {
    const parsed = hostDecisionSchema.parse(input);
