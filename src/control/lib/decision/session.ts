...
export class ControlSession {
    ...
    async observe(): Promise<Observation> {
        ...
        try {
            ...
            this.budget.remaining();
            this.options.driver.validateObservation?.(next);
    ...
    async dispatch(options: {
        ...
        if (
            ...
            !candidatesFor({ observation, action: candidate.action, parameters: options.parameters }).some(
        ...
        try {
            const refreshed = observationSchema.safeParse(result.after);
            if (refreshed.success && refreshed.data.snapshot !== observation.snapshot) {
                this.budget.remaining();
                if (!sameScope(observation, refreshed.data)) {
                    throw new Error("App instance or window changed in action readback; stopping.");
                }
                this.options.driver.validateObservation?.(refreshed.data);
