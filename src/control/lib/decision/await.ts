...
import { evidenceChoices, type Observation, observedEvidence, sameScope } from "./observation";
...
export async function awaitCondition(options: {
    ...
    const events: Array<{
        ...
        probability: number;
        confidence?: number;
        probabilities: Awaited<ReturnType<typeof classifyWait>>["probabilities"];
        evidenceDecision: Awaited<ReturnType<typeof classifyWait>>["evidenceDecision"];
    }> = [];
    ...
    try {
        ...
        while (true) {
            ...
            if (fingerprint !== previousFingerprint) {
                ...
                const classified = await classifyWait({ observation, condition, evaluate, signal: budget.signal });
