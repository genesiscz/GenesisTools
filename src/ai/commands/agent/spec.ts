...
export interface AgentLauncher {
     ...
     *
     * `account` is the name the user typed, unresolved. A compatibility route that consumes the
     * invocation itself has to see it: grok's worker door ran entirely inside `preflight` and
     * never learned the positional account existed, so it discarded it in silence.
     */
    preflight?(input: {
        flags: Record<string, unknown>;
        passthrough: string[];
