...
function emit(outcome: WorkerVerbOutcome | undefined): void {
    ...
    // `interruptTurn` on a signal-backed backend: nothing to print here, `endTurnOrSession`
    // reads the pids itself to decide which confirmation sentence is true.
...
export function registerWorkerVerbs<Meta extends WorkerMeta>(
    ...
    if (caps.accountUnsupported) {
        // Advertising a flag that can only throw is a promise the tool cannot keep: the user
        // learns it is unsupported by trying it. Hidden rather than removed, so `--account x`
        // still reaches the backend's refusal, which names the fix, instead of commander's
        // bare "unknown option".
        spawn.addOption(
            spawn.createOption("-a, --account <account>", `Refused: ${caps.accountUnsupported}`).hideHelp()
        );
    } else if (caps.accountRequired) {
    ...
    parent
        .command("status")
        .description(
            "Show whether a session's turn is running and its cwd; --json for the full metadata; omit --name to list every session"
        )
        ...
        .action(async (flags: { name?: string; json?: boolean }) => {
            ...
            // `--json` has to MEAN something here. Writing the payload unconditionally made the
            // declared flag a no-op on this branch while it worked on the other, and turned
            // `tools codex status --name x` — one human line on stderr before the shared core —
            // into JSON on stdout for every caller that never asked for it.
            if (flags.json === true) {
                out.result(payload);
                return;
            }
            ...
            out.printlnErr(
                `${pc.bold(meta.name)} ${live.running ? pc.green("running") : pc.dim("idle")}${
                    live.pids?.length ? pc.dim(` pid ${live.pids.join(", ")}`) : ""
                }${live.detail ? pc.dim(` — ${live.detail}`) : ""}`
            );
            out.printlnErr(pc.dim(`cwd ${meta.cwd}`));
        });
    ...
    async function endTurnOrSession(name: string, shutdown: boolean): Promise<void> {
        ...
        // 🛑 Only a backend whose turn IS the process may refuse here. On a daemon backend
        // `running` is DERIVED from the meta file — codex reports `stalled` after 120s with no
        // notification (store.ts) — and a stalled turn is the single most common reason to
        // interrupt one. `tools codex interrupt` sent the control request unconditionally before
        // the shared core, and refusing left `stop`, which tears the daemon down and loses the
        // session, as the only way out of a hung turn.
        if (!live.running && !driver.shutdown) {
            out.printlnErr(pc.dim(`No running turn for '${name}'. Nothing to stop.`));
            return;
        }

        const outcome = await driver.interruptTurn(meta);
        emit(outcome);

        // `liveness` and `interruptTurn` each read the process table separately, so a turn that
        // ended in the gap between the two signals nothing — the confirmation below would then
        // claim an action that never happened.
        if (outcome?.kind === "signalled" && outcome.pids.length === 0) {
            out.printlnErr(pc.dim(`The turn on '${name}' had already ended before the signal reached it.`));
            return;
        }
