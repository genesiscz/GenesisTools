...
describe("a case difference is a typo, not a different account", () => {
    // 🛑 Case-folding is gated on `fuzzy`, which only the LAUNCH picker passes. It is a
    // convenience for `tools codex run Shop`, and running it everywhere widened
    // `tools ai accounts logout`, whose resolved id goes straight to the irreversible
    // `clearCredentials` — see the non-fuzzy control at the end of this block.
    test("a differently-cased name resolves without the substring pass", async () => {
        const picked = await resolve("WORK", [account("acc_work", "work"), account("acc_shop", "shop")], true);

        expect(picked.status === "ok" && picked.account.id).toBe("acc_work");
    });

    test("a differently-cased id resolves too", async () => {
        const picked = await resolve("ACC_WORK", [account("acc_work", "work")], true);

        expect(picked.status === "ok" && picked.account.name).toBe("work");
    });

    test("an exact-only caller refuses a differently-cased name rather than resolving it", async () => {
        // `tools ai accounts logout WORK` must not wipe `work`. Destructive verbs do not opt in
        // to fuzziness, so for them a case difference stays a miss.
    ...
    test("two accounts differing only in case are refused, never guessed", async () => {
        const picked = await resolve("WORK", [account("acc_upper", "Work"), account("acc_lower", "work")], true);
