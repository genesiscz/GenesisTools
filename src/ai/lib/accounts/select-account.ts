...
export interface ResolveAccountInput {
    ...
    /**
     * Every convenience match beyond the literal spelling, for the launch pickers only.
     *
     * 🛑 It gates TWO things, not one. First, a case difference: with `fuzzy` unset,
     * `matchExactly` runs `id === requested` and `name === requested` and nothing else, so
     * `logout WORK` is "not found" rather than a hit on `work`. Second, once the exact passes
     * fail, a unique case-insensitive SUBSTRING of a name (`tools codex run shop` for
     * `cdx-shop`); several substring hits prompt on a TTY and are an error otherwise.
     *
     * The exact passes always run first, so an id or a full name can never be shadowed by a
     * longer name that contains it. `runLogout` hands the resolved id straight to the
     * irreversible `clearCredentials`, which is why it takes neither convenience.
     */
    fuzzy?: boolean;
}
 ...
 */
function matchExactly(accounts: AccountEntry[], requested: string, fuzzy = false): AccountEntry[] {
    const needle = requested.toLowerCase();
    const passes = [
        (entry: AccountEntry) => entry.id === requested,
        (entry: AccountEntry) => entry.name === requested,
        // 🛑 Case-folding is a CONVENIENCE for the launch picker, so it is gated on `fuzzy` like
        // the substring pass below. Running it for everyone widened `tools ai accounts logout`,
        // whose resolved id goes straight to the irreversible `clearCredentials`: `logout WORK`
        // used to be "not found" and would instead have wiped `work`.
        ...(fuzzy
            ? [
...
export async function resolveAccountName(input: ResolveAccountInput): Promise<AccountResolution> {
    ...
    if (input.requested) {
        const exact = matchExactly(input.accounts, input.requested, input.fuzzy);
