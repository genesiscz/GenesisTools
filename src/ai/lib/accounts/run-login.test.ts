...
/**
 * `login` without `--name` takes the identity's local part as the name. A hit on
 * an account of ANOTHER provider used to be treated as a re-login: the identity
 * guard has nothing to contradict on an API-key account, so `applyLoginOutcome`
 * switched its provider and deleted its vault secrets (PR #359 review t5).
 */
describe("writeLoginOutcome keeps a guessed name off another provider's account", () => {
    async function seedApiKeyWork(): Promise<void> {
        registerPlugin(fakePlugin("other-sub", { presentation, logoutTargets: ["oauth"] }));
        await seed({
            version: CONFIG_VERSION,
            accounts: [
                {
                    ...
                    enabled: true,
                    billing: { mode: "subscription" },
                    credentials: { authFile: join(home, "work", "auth.json") },
                    useEnvApiKey: false,
                },
            ],
            defaults: {},
    ...
    function otherLogin(rollback: () => Promise<void>): LoginOutcome {
        return { provider: "other-sub", credentials: { accessToken: "sk-other" }, rollback };
    }

    test("in a pipe a suggested name writes NOTHING, says why and rolls the flow back", async () => {
        await seedApiKeyWork();
        const before = readFileSync(configPath(), "utf8");
        let rolledBack = 0;
        ...
        expect(written).toBeNull();
        expect(readFileSync(configPath(), "utf8")).toBe(before);
        expect(storedAccount("work")?.provider).toBe("fake-sub");
        expect(rolledBack).toBe(1);
    });

    test("NEGATIVE CONTROL: an explicit --name still switches the provider on purpose", async () => {
        await seedApiKeyWork();
        let rolledBack = 0;
        ...
        expect(written).not.toBeNull();
        expect(storedAccount("work")?.provider).toBe("other-sub");
        expect(rolledBack).toBe(0);
    });

    test("NEGATIVE CONTROL: a suggested name that hits the SAME provider is a plain re-login", async () => {
        await seedApiKeyWork();
        ...
        const written = await writeLoginOutcome({
            ...
            autoNamed: true,
            account: (await AiConfigStore.load()).account("work"),
            outcome: { provider: "fake-sub", credentials: { accessToken: "sk-fake-fresh" } },
            interactive: false,
        });

        expect(written).not.toBeNull();
        expect(await resolveSecret(storedAccount("work")?.credentials.accessToken)).toBe("sk-fake-fresh");
    });
});
