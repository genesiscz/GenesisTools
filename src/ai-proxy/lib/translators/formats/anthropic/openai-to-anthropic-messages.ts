...
function mapToolChoice(toolChoice: unknown, forcedAllowed: boolean): AnthropicToolChoice | undefined {
    ...
    // Opus 5.5 and Fable 5.1 answer a forced choice with a 400, and so does any model once
    // thinking is on. `auto` keeps the tools callable; the forcing itself cannot be carried over.
...
export function openAiChatToAnthropicMessages(
    ...
    // Adaptive thinking rejects a pinned temperature/top_p just like a budget does, so on
    // Opus 4.6 / Sonnet 4.6 (the adaptive models that still take sampling) the client's
    // sampling wins and thinking is skipped. From 4.7 on no sampling survives to here.
    const samplingPinned = result.temperature !== undefined || result.top_p !== undefined;

    if (typeof body.reasoning_effort === "string" && rules.thinking === "adaptive" && !samplingPinned) {
    ...
    } else if (typeof body.reasoning_effort === "string" && rules.thinking === "budget" && !samplingPinned) {
    ...
    if (tools) {
        ...
        // Thinking (adaptive or budget) accepts only `auto` / `none`, so a request this function
        // turned thinking on for loses the forcing the same way Opus 5.5 always does.
        const toolChoice = mapToolChoice(body.tool_choice, rules.forcedToolChoice && result.thinking === undefined);
