        ...
        .action(
            async (
                ...
                if (opts.deleteBranch) {
                    const users = branchUsers(mr.sourceBranch, mr, report.mrs);
                    if (users.length) {
                        throw new Error(
                            `Branch ${mr.sourceBranch} is still used by open ${users.map((u) => `!${u.iid}`).join(", ")}; close those first or drop --delete-branch.`
                        );
                    }
                ...
                if (opts.deleteBranch && (await branchProtected(api, mr.sourceBranch)) !== null) {
                    const deleted = await deleteBranch(api, mr.sourceBranch);
                    if (!deleted.ok) {
                        throw new Error(
