...
import { parseDay, shiftDay } from "@app/gitlab/lib/activity";
import {
    compareCommits,
    findUser,
    type GitLabCommit,
...
async function runAnalyzeUser(opts: Options): Promise<void> {
    ...
    // GitLab's `after` is exclusive, so passing --since itself skipped the pushes of that day.
    const events = await getUserEvents(api, user.id, shiftDay(parseDay(opts.since, "--since"), -1));
    ...
    // With no matching author the commits are every commit the user PUSHED, which can include
    // other people's work. That fallback used to show only in a stderr warning, so the report read
    // as the user's own commits. The report itself now says which one it is.
    const authorFilter = userEmails.size ? "author-email" : "none";
    ...
    emit(
        renderMarkdown({ user, since: opts.since, authorFiltered: authorFilter !== "none", enriched, byDay, days }),
        outputPath,
        "Markdown"
    );
}
...
interface RenderArgs {
    user: GitLabUser;
    since: string;
    /** False when no commit author matched the user, so the commits are everything they pushed. */
...
function renderMarkdown({ user, since, authorFiltered, enriched, byDay, days }: RenderArgs): string {
    const lines: string[] = [];
    ...
    if (!authorFiltered) {
        lines.push(
            `> ⚠️ No commit author matched @${user.username} or "${user.name}", so this lists every commit the user pushed, which can include other people's commits.`
        );
        lines.push("");
    }
