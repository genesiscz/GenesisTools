...
export async function writeAnchoredDraft(
    ...
    try {
        ...
        if (!created.position?.new_line) {
            // The POST already created a top-level draft. Take it back rather than leave a stray
            // note on the MR for the author to find and delete by hand.
            ...
            return {
                ...
                error: removed.ok
                    ? `GitLab dropped the anchor for ${draft.path}:${draft.line}; the top-level draft it created was deleted again.`
                    : `draft ${created.id} was created without an anchor and is now a top-level note, and deleting it failed (${removed.error}). Delete it by hand.`,
            };
        }
