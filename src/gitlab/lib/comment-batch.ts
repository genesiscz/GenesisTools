...
import { type ProjectApi, projectBase, restGet, restWrite } from "@app/gitlab/lib/client";
...
export async function fetchDraftNotes(api: ProjectApi, iid: string): Promise<Array<{ id: number; note: string }>> {
    return restGet<Array<{ id: number; note: string }>>(api, `${mrPath(api, iid)}/draft_notes`);
}
...
export async function publishDraftNote(
    ...
    try {
        await restWrite<void>(api, { method: "PUT", path: `${mrPath(api, iid)}/draft_notes/${draftId}/publish` });
