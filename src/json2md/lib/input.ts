...
function detectFormat(text: string, source: string): Exclude<InputFormat, "auto"> {
    ...
    // The name wins over the content: a `.json` file that opens with a comment is still JSON,
    // and SafeJSON reads the comment. Only extension-less input falls to the heuristics below.
    if (source.endsWith(".json") || source.endsWith(".jsonc")) {
        return "json";
    }

    const trimmed = text.trimStart();

    // A leading comment, or a top-level scalar (`42`, `"x"`, `true`), is JSON as SafeJSON reads
    // it. TOML never opens with either, so sending them to TOML only produced a syntax error.
