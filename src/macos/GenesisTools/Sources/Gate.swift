...
struct GateClientInfo: Decodable {
    ...
    /// Set by the gate when the pid is running and is an ancestor of the gate process.
    var verified: Bool?
}
...
private func describeClient(_ client: GateClientInfo) -> String {
    ...
    if client.verified != true {
        lines.append("⚠️ Unverified: this pid did not start the request. Allow once at most; it will not be remembered.")
    }
