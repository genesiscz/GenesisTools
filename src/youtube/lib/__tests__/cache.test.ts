...
describe("cache path helpers", () => {
    it("builds sanitized per-channel video directories", () => {
        expect(videoDir(layout, "@Some Channel!*" as `@${string}`, "abc123")).toBe(join(baseDir, "channels", "Some_Channel__", "videos", "abc123"));
    });

    it("builds audio, video, and thumbnail paths", () => {
        expect(audioPath(layout, "@mkbhd", "video123", "wav")).toBe(join(baseDir, "channels", "mkbhd", "videos", "video123", "audio", "video123.wav"));
        expect(videoFilePath(layout, "@mkbhd", "video123", "mp4")).toBe(join(baseDir, "channels", "mkbhd", "videos", "video123", "video", "video123.mp4"));
        expect(thumbPath(layout, "@mkbhd", "video123")).toBe(join(baseDir, "channels", "mkbhd", "videos", "video123", "thumbs", "video123.jpg"));
    });
