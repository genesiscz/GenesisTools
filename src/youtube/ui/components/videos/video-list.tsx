...
export function VideoList({ handle }: { handle: ChannelHandle }) {
    ...
    const videos = useVideos({ channel: handle, since: filters.since || undefined, limit: filters.limit, includeShorts: filters.includeShorts });
    const channels = useChannels();
    const sync = useSyncChannel();
    const channel = channels.data?.find((item) => item.handle === handle);

    async function onSync() {
        await sync.mutateAsync({ handle, since: filters.since || undefined, limit: filters.limit, includeShorts: filters.includeShorts });
    ...
    return (
        <div className="space-y-6">
            <header className="yt-panel rounded-3xl p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-4">
                        {channel?.thumbUrl ? <img src={channel.thumbUrl} alt="" className="size-16 rounded-full border border-primary/30 object-cover" /> : <div className="grid size-16 place-items-center rounded-full border border-primary/30 bg-primary/10 text-primary">{handle.slice(1, 3).toUpperCase()}</div>}
                        <div>
            ...
            {videos.data?.length === 0 ? (
                <EmptyState title="No videos cached" body="Run a sync to discover this channel's videos and transcripts." cta={<Button onClick={onSync}>Sync now</Button>} />
            ) : (
