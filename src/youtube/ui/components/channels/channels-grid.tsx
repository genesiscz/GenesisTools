...
export function ChannelsGrid() {
    ...
    const filtered = useMemo(() => {
        ...
        return (channels.data ?? []).filter((channel) => `${channel.handle} ${channel.title ?? ""}`.toLowerCase().includes(query));
    ...
    return (
        <div className="space-y-6">
            <header className="flex flex-col gap-4 rounded-3xl border border-primary/20 bg-black/25 p-5 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                    <div className="relative">
                        ...
                        <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search channels" className="pl-9 sm:w-72" />
                    </div>
            ...
            {channels.data?.length === 0 ? (
                <EmptyState title="No channels yet" body="Track a channel to start indexing its videos, transcripts, summaries, and jobs." cta={<Button onClick={() => setOpen(true)}>Add your first channel</Button>} />
            ) : (
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                    {filtered.map((channel) => <ChannelCard key={channel.handle} channel={channel} />)}
                </div>
            )}
