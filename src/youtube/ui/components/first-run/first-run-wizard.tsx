            ...
            <Card className="yt-panel neon-border w-full max-w-xl overflow-hidden">
                <CardHeader className="space-y-4">
                    ...
                    <CardDescription className="text-base leading-7">
                        The local dashboard talks to the YouTube pipeline over HTTP. Keep the default for a local server, or point at a remote tunnel.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-5">
                    <div className="space-y-2">
                        <label className="font-mono text-xs uppercase tracking-[0.22em] text-muted-foreground" htmlFor="api-url">
                            API Base URL
                        </label>
                        <Input id="api-url" value={apiUrl} onChange={(event) => setApiUrl(event.target.value)} placeholder="http://localhost:9876" />
                    </div>
                    <Button onClick={onSubmit} disabled={submitting || !apiUrl.trim()} className="btn-glow w-full gap-2">
