...
import { retry, debounce, throttle, withTimeout, concurrentMap } from "./async";
...
describe("retry", () => {
    ...
    it("rejects after exhausting all retries", async () => {
        ...
        }, { maxAttempts: 3, delay: 10 })).rejects.toThrow("always fails");
    ...
    it("calls onRetry callback", async () => {
        ...
        await retry(async () => {
            ...
            maxAttempts: 3, delay: 10, backoff: "fixed",
            onRetry: (a, d) => retries.push({ attempt: a, delay: d }),
        });
        expect(retries).toEqual([{ attempt: 1, delay: 10 }, { attempt: 2, delay: 10 }]);
    });
...
describe("debounce", () => {
    ...
    it("passes arguments to the original function", () => {
        let received: string[] = [];
        const debounced = debounce((...args: string[]) => { received = args; }, 100);
        debounced("a", "b");
        jest.advanceTimersByTime(100);
...
describe("concurrentMap", () => {
    ...
    it("handles errors per item via onError", async () => {
        ...
        const result = await concurrentMap({
            items: [1, 2, 3],
            fn: async (n) => { if (n === 2) throw new Error("fail"); return n * 10; },
            concurrency: 5,
    ...
    it("throws when concurrency < 1", async () => {
        await expect(concurrentMap({ items: [1], fn: async (n) => n, concurrency: 0 })).rejects.toThrow("concurrency must be >= 1");
    });
});
