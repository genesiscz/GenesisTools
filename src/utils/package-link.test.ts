...
describe("unlinkUtilsPackage", () => {
    ...
    test("removes a dangling mapping without force, since no checkout lives there any more", () => {
        const root = scratch();
        const gone = join(scratch(), "moved-away");
        writeFileSync(
            configPathFor(root),
            SafeJSON.stringify({ compilerOptions: { paths: { [`${PACKAGE_NAME}/*`]: [join(gone, "*")] } } })
        );

        expect(unlinkUtilsPackage({ root }).outcome).toBe("removed");
        expect(existsSync(configPathFor(root))).toBe(false);
    });
...
describe("resolution through an ancestor config", () => {
    test("a FRESH bun process resolves the bare specifier only once the mapping exists", async () => {
        ...
        const run = async (): Promise<number> => {
            ...
            // `--no-install`: with no node_modules here, auto-install would fetch the package from
            // the registry, so the result would depend on the network, not on the mapping.
            const proc = Bun.spawn(["bun", "--no-install", probe], {
                cwd: deep,
                env: process.env,
                stdout: "pipe",
                stderr: "pipe",
...
describe("shadowing", () => {
    ...
    test("refuses a config that inherits its paths through extends, and leaves it untouched", () => {
        const root = scratch();
        const project = nest(root, "project");
        const child = '{ "extends": "./tsconfig.base.json" }';
        writeFileSync(
            join(project, "tsconfig.base.json"),
            '{ "compilerOptions": { "paths": { "@app/*": ["./src/*"] } } }'
        );
        writeFileSync(configPathFor(project), child);

        const result = linkUtilsPackage({ root: project });

        expect(result.outcome).toBe("inherits-paths");
        expect(readFileSync(configPathFor(project), "utf8")).toBe(child);
    });

    test("still merges into an extending config that sets its own paths", () => {
        const root = scratch();
        const project = nest(root, "project");
        writeFileSync(
            configPathFor(project),
            '{ "extends": "./tsconfig.base.json", "compilerOptions": { "paths": { "@app/*": ["./src/*"] } } }'
        );

        expect(linkUtilsPackage({ root: project }).outcome).toBe("merged");
    });
    ...
    test("🛑 a nearer config really does break resolution, and installing into it fixes it", async () => {
        ...
        const run = async (): Promise<number> => {
            ...
            // `--no-install` keeps the registry out of it; only the mapping may decide.
            const proc = Bun.spawn(["bun", "--no-install", probe], {
                cwd: project,
                env: process.env,
                stdout: "pipe",
