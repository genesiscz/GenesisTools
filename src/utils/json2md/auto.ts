...
/**
 * A key as display text. Keys are data exactly as values are, and they land in headings,
 * `<summary>`, definition terms and table headers, so they get the same escaping: a key such as
 * `<img onerror=…>` or `# x` must not become markup.
 */
function keyText(key: string, opts: Resolved): string {
    const cased = applyHeaderCase(key, opts.keyCase);

    return opts.escapeValues ? escapeInline(cased) : cased;
}

function definitionEntries(value: Record<string, unknown>, opts: Resolved): DefinitionEntry[] {
    return Object.entries(value).map(([key, item]) => ({
        term: keyText(key, opts),
...
function keyValueRows(value: Record<string, unknown>, opts: Resolved): Row[] {
    return Object.entries(value).map(([key, item]) => ({
        Field: keyText(key, opts),
        Value: text(item, opts),
    }));
}

/** Table options for the auto path: data-derived cells AND headers go through the same escaping. */
function autoTableOptions(opts: Resolved): TableOptions {
    return {
        headerCase: opts.keyCase,
        toCellText: ({ value }) => text(value, opts),
        toHeaderTitle: ({ title }) => (opts.escapeValues ? escapeInline(title) : title),
    };
}
