...
function formatAttributes(stamp: Stamp): string {
    ...
    if (stamp.generator) {
        parts.push(`generator=${stamp.generator}`);
    }
...
export function stampMarkdown(body: string, meta: Omit<Stamp, "version" | "content"> = {}): string {
    const clean = stripStamp(body).body;
    const stamp: Stamp = { version: STAMP_VERSION, content: hashText(clean), ...meta };
