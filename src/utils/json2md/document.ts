...
/**
 * GitHub's anchor rule: lowercase, drop punctuation except `-` and `_`, and turn EACH space into
 * a hyphen without collapsing a run. `## user_login` anchors as `#user_login`, and the auto path
 * uses raw snake_case keys as headings, so dropping `_` broke most of its contents links.
 */
