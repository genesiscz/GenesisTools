...
import { displayWidth, escapeCell, padToWidth, truncateToWidth } from "./escape";
...
function renderCell<T extends Row>(column: ResolvedColumn<T>, row: T, index: number, options: TableOptions<T>): string {
    ...
    if (options.toCellText) {
        text = options.toCellText({ key: column.key, value: raw, row, index });
    }
    ...
    return escapeCell(text, options.lineBreak ?? "strip");
}
    ...
    for (const paragraph of value.split(/\r\n|\r|\n/)) {
        ...
        for (const word of paragraph.split(/\s+/)) {
            ...
            while (stringLength(rest) > max) {
                const head = truncateToWidth(rest, { max, strategy: "truncateEnd", stringLength, ellipsis: "" });
                lines.push(head);
