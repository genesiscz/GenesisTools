...
describe("edge cases the first review found", () => {
    test("a key is data too, so it is escaped wherever it becomes a title", () => {
        const markdown = jsonToMarkdown({ "<img src=x onerror=alert(1)>": { a: 1 }, "# x": 1, plain: 2 });

        expect(markdown).not.toContain("<img");
        expect(markdown).toContain("&lt;img");
        expect(markdown).not.toMatch(/^# x/m);

        const table = jsonToMarkdown([{ "<b>k</b>": 1 }, { "<b>k</b>": 2 }]);

        expect(table).not.toContain("<b>");
    });

    test("keys stay raw when escaping is turned off on purpose", () => {
        expect(jsonToMarkdown({ "**bold**": { a: 1 } }, { escapeValues: false })).toContain("**bold**");
    });

    test("the declared error codes are the ones actually thrown", async () => {
        expect(() => selectValue({ a: 1 }, "$[?(", { dialect: "jsonpath" })).toThrow(Json2mdError);

        try {
            selectValue({ a: 1 }, "$[?(", { dialect: "jsonpath" });
        } catch (error) {
            expect(error instanceof Json2mdError ? error.code : "").toBe("SELECT_FAILED");
        }

        const dir = await mkdtemp(join(tmpdir(), "json2md-codes-"));
        const missing = defineDocument({ data: "./nope.json", render: () => [] });

        await expect(writeDocument(join(dir, "doc.ts"), missing)).rejects.toMatchObject({ code: "DATA_NOT_FOUND" });

        await Bun.write(join(dir, "plain.ts"), "export default { nothing: true };\n");
        await expect(loadDocumentModule(join(dir, "plain.ts"))).rejects.toMatchObject({ code: "NO_DOCUMENT_EXPORT" });
    });

    test("a build refuses an unstamped or newer-format file, and --force still overwrites", async () => {
        const dir = await mkdtemp(join(tmpdir(), "json2md-refuse-"));
        const modulePath = join(dir, "doc.ts");
        const outPath = join(dir, "doc.md");

        await Bun.write(join(dir, "doc.json"), SafeJSONStringify({ items: [{ id: 1 }] }));
        await Bun.write(outPath, "# Written by a person\n");

        const definition = defineDocument<{ items: Array<{ id: number }> }>({
            data: "./doc.json",
            render: (d) => [{ table: { rows: d.items } }],
        });
        const unstamped = await writeDocument(modulePath, definition);

        expect(unstamped.outcome).toBe("refused");
        expect(await Bun.file(outPath).text()).toBe("# Written by a person\n");

        const future = `body\n\n<!-- json2md:stamp v${STAMP_VERSION + 1} content=sha256:00 -->\n`;

        await Bun.write(outPath, future);
        expect((await writeDocument(modulePath, definition)).outcome).toBe("refused");
        expect(await Bun.file(outPath).text()).toBe(future);

        expect((await writeDocument(modulePath, definition, { force: true })).outcome).toBe("written");
    });

    test("a heading slug keeps underscores and turns each space into a hyphen", () => {
        expect(slugifyHeading("user_login")).toBe("user_login");
        expect(slugifyHeading("a  b")).toBe("a--b");
    });

    test("a combining mark past the Thai block measures zero columns", () => {
        expect(displayWidth("a᷀")).toBe(1);
        expect(displayWidth("a᪰")).toBe(1);
    });

    test("a multi-line string in a front-matter list keeps its value", () => {
        expect(toYaml({ tags: ["a\nb", "c\td"] })).toBe('tags:\n  - "a\\nb"\n  - "c\\td"');
    });

    test("data hashing tells a string from a number, and a comma from a second key", () => {
        expect(hashData({ a: 1 })).not.toBe(hashData({ a: "1" }));
        expect(hashData({ a: "1,b:2" })).not.toBe(hashData({ a: 1, b: 2 }));
    });

    test("a generator path with a space survives the stamp", () => {
        const stamped = stampMarkdown("# T\n", { generator: "My Reports/doc.ts" });

        expect(stripStamp(stamped).stamp?.generator).toBe("My Reports/doc.ts");
    });

    test("a freshly stamped body is clean whatever its line endings", () => {
        for (const body of ["# T\r\n\r\nline\r\n", "# T\n\nno trailing newline", "# T\n\nseveral\n\n\n"]) {
            expect(checkMarkdown({ current: stampMarkdown(body) }).verdict).toBe("clean");
        }
    });

    test("toCellText receives what format already made of the value", () => {
        const markdown = renderTable([{ n: 5 }], {
            columns: [{ key: "n", format: (value) => `#${String(value)}` }],
            toCellText: ({ text }) => `[${text}]`,
        });

        expect(markdown).toContain("[#5]");
    });

    test("overflow wrap survives escaping with the default line-break strategy", () => {
        const markdown = renderTable([{ text: "one two three four" }], {
            columns: [{ key: "text", maxWidth: 8, overflow: "wrap" }],
        });

        expect(markdown).toContain("<br>");
    });

    test("wrapping a glyph wider than the budget terminates", () => {
        expect(wrapToWidth("日本", 1)).toEqual(["日", "本"]);
    });
});

describe("edge cases the third review found", () => {
    test("a nested ordered list starts at its parent's content column", () => {
        expect(render([{ ol: [{ text: "a", children: ["b"] }] }])).toBe("1. a\n   1. b");
        expect(render([{ ul: [{ text: "a", children: ["b"] }] }])).toBe("- a\n  - b");
    });

    test("a block marker is escaped on every line of a value, not only the first", () => {
        const markdown = jsonToMarkdown({ note: "ok\n# Injected\n- item\n~~~" });

        expect(markdown).not.toMatch(/^\s*# Injected/m);
        expect(markdown).not.toMatch(/^\s*- item/m);
        expect(markdown).not.toMatch(/^\s*~~~/m);
    });

    test("check reports a document that was never generated as missing", async () => {
        const dir = await mkdtemp(join(tmpdir(), "json2md-missing-"));

        await Bun.write(join(dir, "doc.json"), SafeJSONStringify({ items: [] }));

        const check = await checkDocument(
            join(dir, "doc.ts"),
            defineDocument({ data: "./doc.json", render: () => [] })
        );
