...
async function readData(path: string): Promise<unknown> {
    ...
    if (!(await file.exists())) {
        throw new Json2mdError("DATA_NOT_FOUND", `json2md: data file not found: ${path}`);
    }
 ...
 *
 * 🛑 Every verdict except `stale` and `clean` is a refusal, not a warning. Overwriting is the
 * one outcome that cannot be undone from the repository, because the edit exists nowhere else.
 */
export async function writeDocument<T>(
    ...
    // Only `stale` is known to be safe. `unstamped` cannot rule out a hand edit (a hand-written
    // file at the output path, or an `unstamped: true` document a person keeps editing), and
    // `unsupported` is a newer format this build would downgrade. Both need `--force`.
    if (check.verdict !== "stale" && !options.force) {
...
export interface DocumentCheck extends CheckResult {
    outPath: string;
    /** `false` when the output was never generated or was deleted; `check` counts that as a failure. */
    exists: boolean;
