...
export function SeedOperation({
  ...
  useEffect(() => {
    ...
    ds.initialize()
      .then(() => {
        if (!cancelled) dispatch({ type: 'CONNECTED', dataSource: ds });
      })
      .catch((err: unknown) => {
        if (!cancelled) {
          dispatch({
  ...
  // ── Phase 2: Analyze ───────────────────────────────────────────────────
  useEffect(() => {
    if (state.phase !== 'analyzing' || !state.dataSource) return;
    ...
    async function analyze() {
      try {
        ...
        if (analyzeMethod) {
          ...
          if (!cancelled) dispatch({ type: 'SKIP_ANALYSIS' });
          return;
        }

        if (!cancelled) dispatch({ type: 'ANALYZED', results });
      } catch (err: unknown) {
        if (!cancelled) {
          dispatch({
            type: 'ERROR',
  ...
  // ── Phase 3: Execute ──────────────────────────────────────────────────
  useEffect(() => {
    if (state.phase !== 'executing' || !state.dataSource) return;
    ...
    async function execute() {
      ...
      const entitySteps: StepProps[] = state.analysisResults.length > 0
  ...
  function getConfirmTier(): 'safe' | 'moderate' | 'destructive' {
    if (confirmTier) return confirmTier;

    // Determine from analysis results risk
    const maxRisk = state.analysisResults.reduce<string>((max, r) => {
      if (r.risk === 'destructive') return 'destructive';
      if (r.risk === 'new' && max !== 'destructive') return 'new';
      return max;
    }, 'safe');

    if (maxRisk === 'destructive') return 'destructive';
    if (maxRisk === 'new') return 'moderate';
    return 'safe';
  }
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      {state.phase === 'connecting' && (
        <ProgressSteps
          steps={[{ label: 'Connecting to database', status: 'running' }]}
        />
      )}
        ...
        <>
          ...
          {state.analysisResults.length > 0 && (
            <>
              ...
              <Warnings
                warnings={state.analysisResults.flatMap((r) => r.warnings)}
              />
      ...
      {/* Executing phase */}
      {state.phase === 'executing' && state.steps.length > 0 && (
      ...
      {state.phase === 'done' && (
        <>
          {resultData.length > 0 && (
            <Table columns={resultColumns} data={resultData} />
          )}
          <Box>
            <Text color="green">
