...
function ModerateConfirm({
  ...
  useInput((input, key) => {
    if (resolved || autoConfirm) return;
  ...
  if (autoConfirm || resolved) return null;

  return (
    <Box flexDirection="column" gap={0}>
      {message && <Text>{message}</Text>}
      <Text>
        Continue? (<Text bold color="green">Y</Text>/n)
      </Text>
    </Box>
  );
}
...
function DestructiveConfirm({
  ...
  return (
    <Box flexDirection="column" gap={0}>
      ...
      <Box marginTop={1}>
        <Text>
          Type &apos;<Text bold color="red">{confirmText}</Text>&apos; to confirm:{' '}
        </Text>
        <TextInput
          value={inputValue}
