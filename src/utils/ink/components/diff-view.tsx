...
export function DiffView({ results, showSkips = false, maxRecords = 20 }: DiffViewProps) {
    ...
    return (
        <Box flexDirection="column" gap={1}>
            {results.map((result) => (
                <EntityDiff key={result.entity} result={result} showSkips={showSkips} maxRecords={maxRecords} />
            ))}
...
function EntityDiff({ result, showSkips, maxRecords }: EntityDiffProps) {
    ...
    const visibleRecords = showSkips ? records : records.filter((r) => r.action !== "skip");

    // Truncate if needed
    const truncated = maxRecords > 0 && visibleRecords.length > maxRecords;
    const displayRecords = truncated ? visibleRecords.slice(0, maxRecords) : visibleRecords;
    ...
    return (
        <Box flexDirection="column">
            ...
            <Box flexDirection="column" paddingLeft={2}>
                ...
                {truncated && <Text dimColor> ... and {remaining} more</Text>}
            </Box>
        </Box>
    );
}
...
function DiffSummary({ results }: { results: AnalysisResult[] }) {
    ...
    if (totals.skip > 0) {
        parts.push(
            <Text key="skip" dimColor>
                ={totals.skip} unchanged
            </Text>
        );
    }
