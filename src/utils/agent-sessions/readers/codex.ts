        ...
        try {
            ...
            if (columns.includes("updated_at_ordinal")) {
                const rows = database
                    ...
                    .all() as Array<{ thread_id: string; count: number; ordinal: number; revision: number }>;
                ...
                for (const row of database
                    ...
                    .iterate() as Iterable<{ thread_id: string; rollout_ordinal: number; item_json: string }>) {
...
export async function readCodexProjectionFingerprint(
    source: NativeSessionSource<"codex">,
    options: HistoryReadOptions & { nativeId?: string; projection?: CodexProjectionIndex } = {}
    ...
    if (options.projection) {
        const indexed = options.projection.get(nativeId);
        return indexed && indexed.length > 0 ? SafeJSON.stringify(indexed, { strict: true }) : undefined;
    }
