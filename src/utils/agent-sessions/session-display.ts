...
export function printAmbiguousSessions(candidates: SessionDisplayItem[], shown = AMBIGUOUS_ROWS_SHOWN): void {
    ...
    // stderr, not stdout. This prints on the FAILURE path, immediately before the caller throws,
    // and `out.result` / `out.print` are the only writers to stdout. A scripted caller was being
    // handed rendered box-drawing where it expected a machine result.
    out.printlnErr(table.toString());

    if (candidates.length > shown) {
        out.printlnErr(pc.dim(`… and ${candidates.length - shown} more; narrow the query to see them.`));
    }
}
