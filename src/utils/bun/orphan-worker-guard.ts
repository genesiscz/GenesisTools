...
/** How far in the future a heartbeat may sit and still count: timestamp precision, not clock drift. */
const BEAT_CLOCK_SLACK_MS = 1000;
...
function isLiveWatchdog(notePath: string, pid: number, pollSeconds: number): boolean {
    ...
    try {
        // A beat from the future (the clock was set back since it was written) proves nothing
        // about the watchdog now, so it counts as stale rather than as fresh forever. The slack
        // is not optional: `mtimeMs` keeps sub-millisecond digits and `Date.now()` does not, so
        // a beat written a moment ago reads as slightly negative (1986 of 2000 fresh writes,
        // measured 2026-09-24), and a zero bound would reject every just-installed watchdog.
        const ageMs = Date.now() - statSync(beatPathFor(notePath)).mtimeMs;

        return ageMs > -BEAT_CLOCK_SLACK_MS && ageMs < pollSeconds * 3000;
    } catch {
...
export function buildWatchdogScript(args: {
    ...
    return [
        ...
        "while :; do",
        // `true`, not `:`: a failed redirection on the special built-in `:` EXITS a non-interactive
        // sh (measured: rc=1, the loop never reaches its checks), so an unwritable note directory
        // would kill every watchdog on its first poll. On the regular built-in it is only an error.
        ...(args.beatPath ? [`  true > ${shQuote(args.beatPath)} 2>/dev/null`] : []),
        '  if ! kill -0 "$self"; then',
        "    exit 0",
        "  fi",
