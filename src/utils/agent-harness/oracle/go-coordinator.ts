...
/** How long a TS → Go request (internals, a setter) may wait for its answer before the run fails. */
...
class GoCoordinator implements Coordinator {
    ...
    /** The first failure of a setter the twin fired without awaiting; the next `settled()` throws it. */
    private setterFailure: Error | null = null;
    ...
    run(signal: AbortSignal): Promise<void> {
        ...
        // Decode as one UTF-8 stream: a pipe chunk can end inside a multi-byte character (the
        // preamble's em dash is three bytes), and decoding each chunk alone turns both halves into U+FFFD.
        child.stdout?.setEncoding("utf8");
        child.stdout?.on("data", (chunk: string) => this.onData(chunk));

        // Wakes the two forwarding loops when the run ends: `signal` covers an abort, this covers a
        // run that settles or dies on its own, so no loop stays parked on an empty queue afterwards.
        const runEnded = new AbortController();
        const forwarding = AbortSignal.any([signal, runEnded.signal]);
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            void this.forwardInputs(forwarding);
            void this.forwardUpdates(forwarding);
        }).then(
            () => undefined,
            (error: unknown) => {
    ...
    async settled(): Promise<void> {
        ...
        if (this.setterFailure) {
            const failure = this.setterFailure;
            this.setterFailure = null;
            throw failure;
        }
    ...
    /**
     * A white-box setter the twin calls synchronously, so nothing awaits it. Its failure (a bridge
     * that stopped answering) is kept and thrown by the next `settled()`, which every twin awaits,
     * instead of surfacing as an unhandled rejection against whichever test runs at that moment.
     */
    private fireSetter(op: string, params: unknown): void {
        this.request(op, params).catch((error: unknown) => {
            this.setterFailure ??= error instanceof Error ? error : new Error(`${op}: ${String(error)}`);
        });
    }

    /**
     * A TS → Go op the bridge answers (internals and the white-box setters). Resolves undefined
     * when the run has ended, since the bridge process is gone. A bridge that stays alive but never
     * answers rejects after `REQUEST_TIMEOUT_MS`, so the twin fails with the op named instead of
     * hanging until the test runner's timeout.
     */
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                if (this.requests.delete(id)) {
                    reject(
                        new Error(
                            `oracle bridge did not answer "${op}" (request ${id}) within ${REQUEST_TIMEOUT_MS} ms`
                        )
                    );
                }
            }, REQUEST_TIMEOUT_MS);
    ...
    private internals(): CoordinatorInternals {
        ...
        return {
            ...
            setCurrentTurnType(type: TurnType) {
                ...
                self.fireSetter("setCurrentTurnType", { Type: type });
            },
            ...
            set callModel(value: boolean) {
                ...
                self.fireSetter("setCallModel", { Value: value });
            },
            graceToolCallKeys: () =>
