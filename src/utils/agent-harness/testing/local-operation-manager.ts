...
export class LocalOperationManager implements Manager {
    ...
    private startShell(operation: Operation): void {
        ...
        // A spawn that fails (missing shell, missing cwd) emits `error`, often with no `exit` after
        // it. Without a listener that is an uncaught exception, and the operation never settles.
        child.on("error", (error) => {
            ...
            state.Result = {
                ...
                Err: error.message,
                OutSize: out.length,
                ErrSize: error.message.length,
                ExitCode: -1,
            };
            this.updateQueue.push({ ...operation, Status: "failed", State: SafeJSON.stringify(state) });
        });
