...
export type LinkOutcome =
    ...
    | "occupied"
    /**
     * The config `extends` another one and sets no `paths` of its own. A `paths` written here
     * would REPLACE the inherited map, so every alias the project gets through `extends` would
     * stop resolving. Never written.
     */
    | "inherits-paths";
...
export function linkUtilsPackage(options: LinkOptions = {}): LinkResult {
    ...
    // TypeScript and Bun merge `compilerOptions` from an `extends` chain option by option, and a
    // child `paths` replaces the inherited one whole. Writing our two keys into a child that
    // inherits its `paths` would silently cut off every alias the base config defines.
    if (config.extends !== undefined && compilerOptions.paths === undefined) {
        return { outcome: "inherits-paths", configPath, target, resolves: false };
    }
...
export function unlinkUtilsPackage(options: LinkOptions = {}): UnlinkResult {
    ...
    // A mapping to a path that no longer exists is stale, not another install: `link` repairs it
    // without --force for the same reason, since nothing can depend on a missing checkout.
    if (existing !== utilsPackageDir() && existsSync(existing) && !force) {
