...
export const CHILD_DEADLINE_SCRIPT =
    'my $ms=shift @ARGV; my $sec=int(($ms+999)/1000); $sec=1 if $sec<1; my $pid=fork(); die "fork: $!\\n" unless defined $pid; if($pid==0){exec {$ARGV[0]} @ARGV; exit 127} $SIG{ALRM}=sub{kill 9,$pid; $SIG{ALRM}=sub{exit 124}; alarm 1; waitpid($pid,0); exit 124}; alarm $sec; waitpid($pid,0); alarm 0; my $sig=$?&127; exit $sig?128+$sig:($?>>8);';
