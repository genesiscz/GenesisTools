...
describe("SNAPSHOT_OPS", () => {
    ...
    // The dashboard card prints "fetched <ago>" straight off this field, so a replayed row
    // stamped with the failed round's clock read "fetched just now" above hours-old bars.
    test("a replayed row keeps the fetch time of the data it is replaying", async () => {
        const fetchedAt = "2026-09-06T09:00:00.000Z";
        const store: CacheStore = new Map();
        store.set("snapshots:grok-sub", {
            fetchedAt: Date.now() - 120_000,
            accounts: [{ ...snapshot("grok-sub", "work", 33), fetchedAt }],
        });

        const get = makeGet(
            "grok-sub",
            storeDeps(store, async () => [errored("grok-sub", "work", "session expired")])
        );

        const [work] = await get({ force: true });

        expect(work.fetchedAt).toBe(fetchedAt);
        expect(work.limits[0].percentUsed).toBe(33);
    });

    // Negative control: a row that really was fetched keeps its own, new clock.
    test("a successful row keeps its own fetch time", async () => {
        const store: CacheStore = new Map();
        store.set("snapshots:grok-sub", {
            fetchedAt: Date.now() - 120_000,
            accounts: [{ ...snapshot("grok-sub", "work", 33), fetchedAt: "2026-09-06T09:00:00.000Z" }],
        });

        const fresh = snapshot("grok-sub", "work", 44);
        const get = makeGet(
            "grok-sub",
            storeDeps(store, async () => [fresh])
        );

        const [work] = await get({ force: true });

        expect(work.fetchedAt).toBe(fresh.fetchedAt);
        expect(work.stale).toBeUndefined();
    });

    // When the network is down every account fails at once. The last good payload must
    // survive that round, or the whole dashboard blanks instead of ageing.
    test("a round in which EVERY account fails still serves the last good data", async () => {
        const lastGoodAt = Date.now() - 120_000;
        const store: CacheStore = new Map();
        store.set("snapshots:openai-sub", {
            fetchedAt: lastGoodAt,
        ...
        const get = makeGet(
            "openai-sub",
            storeDeps(store, async () => [
                errored("openai-sub", "work", "Unable to connect"),
                errored("openai-sub", "personal", "Unable to connect"),
            ])
        );

        const result = await get({ force: true });

        expect(result.map((r) => r.limits[0]?.percentUsed)).toEqual([33, 44]);
        expect(result.every((r) => r.stale !== undefined)).toBe(true);
        // And the cache keeps it, so the NEXT failed round replays it too.
        expect(store.get("snapshots:openai-sub")?.accounts.every((a) => a.limits.length > 0)).toBe(true);
    });
