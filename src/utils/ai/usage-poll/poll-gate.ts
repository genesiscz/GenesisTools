import { isUpstreamOutageStatus } from "@genesiscz/utils/ai/http-auth";
...
export interface GateEntry {
    ...
    failures: number;
    /**
     * Consecutive failures that were not this account's doing (nothing reached the
     * provider, or the provider answered with an outage status), counted on their own
     * ladder. Sharing `failures` let a five-minute wifi drop pre-load the ladder, so the
     * first routine 429 after recovery earned the 6h block instead of none.
     */
    ...
    reason: string;
    /**
     * Epoch ms of the failure that set `blockedUntil`, so a repair can be recognised.
     *
     * OPTIONAL, and absent in every gate file written before this field existed. A missing
     * stamp simply means the release rule below cannot fire, which is the pre-existing
     * behaviour: the block stands until it expires.
     */
    failedAt?: number;
}
...
/**
 * Error NAMES that mean the same thing as the codes above, for the failures that carry no
 * string `code` at all.
 *
 * `AbortSignal.timeout()` is the trap. Verified against Bun 1.3.13: a `fetch` that hits its
 * deadline rejects with a `DOMException` whose `name` is `TimeoutError` and whose `code` is
 * the NUMBER 23, so the string-code set above never matched it and every 20s stall on the
 * anthropic usage endpoint was filed as that account's own fault. Three of them in a row
 * bought the account a two-hour block while the network was already back.
 */
const TRANSPORT_ERROR_NAMES = new Set([
    // Local lock contention: the request was never sent, and the lock is shared by
    // every account, so it is no more this account's fault than a dead network.
    "LockTimeoutError",
    // `AbortSignal.timeout()` on `fetch` (anthropic `fetchUsage`, the grok OIDC refresh).
    "TimeoutError",
    // A deadline the caller imposed through `AbortSignal.any([signal, deadline])`.
    "AbortError",
]);
...
/**
 * True when the failure says nothing about THIS account, so it belongs on the short
 * ladder: either no request reached the provider (a dead network, a lock timeout, a
 * deadline), or the provider answered with an outage status that every account in the
 * round got at the same moment.
 */
export function isTransportFailure(err: unknown): boolean {
    if (typeof err === "object" && err !== null) {
        const { code, name, cause, statusCode } = err as {
            code?: unknown;
            name?: unknown;
            cause?: unknown;
            statusCode?: unknown;
        ...
        // `UpstreamStatusError` and `RetryableApiError` both carry it; only the outage
        // half of the range qualifies, so a 401 or a 429 still earns the long ladder.
        if (typeof statusCode === "number" && isUpstreamOutageStatus(statusCode)) {
            return true;
        }
...
/**
 * True when the credential this account's plugin reads was replaced AFTER the failure that
 * blocked it, so the block is about a problem the user has already fixed.
 *
 * Anthropic clears its own gate from `tools claude login`. Codex and grok log in through
 * their own CLIs, which we cannot hook, so the only evidence a re-login happened is that
 * the vendor auth file moved. Without this rule a fixed account stayed dark for the rest
 * of its backoff, up to six hours, while the dashboard replayed the old error.
 */
export function credentialMovedSince(entry: GateEntry | undefined, credentialStamp: number | undefined): boolean {
    if (!entry || entry.failedAt === undefined || credentialStamp === undefined) {
        return false;
    }

    return credentialStamp > entry.failedAt;
}

/**
 * The blocking entry for an account, or null when it is due to be polled.
 *
 * `credentialStamp` is the plugin's answer to "when was this account's credential last
 * written"; pass it and a re-login releases the block immediately.
 */
export function blockedEntry(gate: PollGate, account: string, now: number, credentialStamp?: number): GateEntry | null {
    ...
    if (credentialMovedSince(entry, credentialStamp)) {
        return null;
    }
