...
/** Plugin id every gate key and every recorded row for this provider carries. */
export const ANTHROPIC_SUB = "anthropic-sub";
 ...
 */
export class PollSuppressedError extends PollSuppressed {
...
export async function fetchUsage(
    ...
    if (!res.ok) {
        ...
        // The status rides on the error so the poll gate can tell an Anthropic outage
        // (every account gets the same 5xx in the same round) from a dead account.
        throw new UpstreamStatusError(res.status, `Usage API ${res.status}: ${body.slice(0, 200)}`);
    }
...
export interface PollAccountArgs {
    ...
    orgBlocked?: ReadonlySet<string>;
    /**
     * Diagnosis only. The refresh grant is single-use, so a probe reads the stored token,
     * never rotates it, and never takes the 429 "rotate and retry" unlock. It also skips
 ...
 */
export async function pollAccount(args: PollAccountArgs): Promise<AccountUsage> {
    const { account, config, gate, now, signal, probe } = args;
    const tag = `[usage:${account.name}]`;

    const blocked = blockedEntry(gate, account.name, now);
    // A probe never re-reads the profile: `refreshSubscriptionProfile` and
    // `revalidateStalePlan` both WRITE the plan fields back into the config.
    const anchorDue = !probe && isAnchorDue(account, now);
    ...
    try {
        ...
        // A probe stops here: the unlock below SPENDS the single-use refresh grant, and
        // diagnosing an account must never be the thing that rotates its credential.
        if (!(err instanceof RetryableApiError) || probe || args.orgBlocked?.has(account.name)) {
...
export async function fetchAllAccountsUsage(opts: FetchAllAccountsOptions = {}): Promise<AccountUsage[]> {
    ...
    const now = Date.now();
    const storedGate = await loadPollGate(ANTHROPIC_SUB);
