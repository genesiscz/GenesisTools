...
export function mapRateLimits(result: CodexRateLimitsResult | null | undefined): {
    ...
    // Which plan-wide windows the top-level block actually emitted. "Did it emit anything?" is a
    // different question and answering it per BLOCK lost windows both ways: testing whether
    // `rateLimits` was PRESENT discarded the account's only window (a block carrying `planType`
    // and `limitId` with null primary/secondary emits nothing) and reported it as logged out,
    // while testing `limits.length > 0` discarded the scoped `primary` whenever the top level
    // emitted `secondary` alone. The skip below is therefore per window, not per block.
    const emitted = new Set<WindowKey>();
    ...
    for (const [limitId, scoped] of Object.entries(result?.rateLimitsByLimitId ?? {})) {
        ...
        const isDefault = limitId === defaultLimitId;
        const scope: WindowScope = { limitId, limitName: scoped.limitName ?? limitId };

        for (const key of WINDOW_KEYS) {
            // The plan-wide pool is the only one the top-level block can repeat, and only for
            // the windows it actually emitted; a per-model pool appears nowhere else.
            if (isDefault && emitted.has(key)) {
                continue;
            }
