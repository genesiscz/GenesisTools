...
describe("AIXAIProvider", () => {
    test("has correct type", () => {
        const provider = new AIXAIProvider();
        expect(provider.type).toBe("xai");
    });

    test("supports transcribe and tts only", () => {
        const provider = new AIXAIProvider();
        expect(provider.supports("transcribe")).toBe(true);
    ...
    test.skipIf(!process.env.X_AI_API_KEY)(
        ...
        async () => {
            const provider = new AIXAIProvider({ forceFreshVoices: true });
    ...
    test.skipIf(!process.env.X_AI_API_KEY)("listVoices() second call hits cache (requires X_AI_API_KEY)", async () => {
        ...
        try {
            const cached = new AIXAIProvider();
