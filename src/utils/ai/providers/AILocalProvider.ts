...
/**
 * Adapter over the transformers.js runtime: this class owns the task shapes
 * (whisper streaming, anti-hallucination thresholds, batch reshaping), the
 * runtime owns pipeline loading, caching and the ONNX error-recovery paths.
 */
    ...
    implements AITranscriptionProvider, AITranslationProvider, AISummarizationProvider, AIEmbeddingProvider
{
    ...
    private runtime = new TransformersJsRuntime();
    ...
    private getPipeline(task: string, model: string, onProgress?: OnProgress): Promise<PipelineInstance> {
        return this.runtime.getPipeline(task, model, onProgress);
    }
