...
export async function evaluateState({
    ...
    logger.debug(
        { model: modelName, questionCount: Object.keys(request.questions).length, zeroDataRetention },
    ...
    logger.debug({ model: modelName, usage: result.usage }, "Jev evaluation complete");
    return {
        model: modelName,
