...
/**
 * `title` and `detail` are always literals from this file. Nothing from the request
 * reaches the page, so an authorization code or a provider-supplied error string can
 * never be reflected into the browser.
 *
 * `brand` is the exception: it arrives from the caller, so its two fields are escaped.
 * Today every caller passes a constant, and the escaping is what keeps that from being
 * a precondition the next caller has to know about.
 */
function page(title: string, detail: string, brand?: CallbackBrand): string {
    const app = brand ? escapeHtml(brand.app) : "";
    const header = brand ? `<div class="mark">${markSvg()}</div><p class="app">${app}</p>` : "";
    const footer = brand ? `<footer>${app} · ${escapeHtml(brand.product)}</footer>` : "";
