...
import { type GateDeps, requestAccountAccess } from "./request";
...
function harness(decision: ApprovalDecision): Harness {
    ...
    state.deps = {
        ...
        resolveToken: async (_provider, entry) => {
            state.resolves++;
            ...
            return { accessToken: `tok-${entry.name}`, expiresAt: 2_000_000, kind: "long-lived" };
        },
    };
    return state;
}
...
describe("requestAccountAccess", () => {
    ...
    test("the window is told which token kind it approves, and the result carries it", async () => {
        ...
        expect(bob.tokenKind).toBe("long-lived");
