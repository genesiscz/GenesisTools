        ...
        info: (msg: unknown) => {
            ...
            const text =
                typeof msg === "object" && msg !== null && "msg" in (msg as Record<string, unknown>)
                    ...
                    : String(msg);
            // This used to be `askUI().logFetching(...)`, which made a util
            // import ask's UI layer — the one hard boundary violation in the
            // repo. `out.log.step` renders the same clack step on stderr.
            out.log.step(text);
        },
