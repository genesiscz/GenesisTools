...
export interface ApplyLoginOutcomeInput {
    ...
    name: string;
    /**
     * The immutable id of the account being written, when the caller already
     * resolved one. Two accounts may share a name, so a name-only write from
     * `login-secondary` landed the grant on the first namesake, whichever
     * provider it belonged to, and a provider switch there deleted that
     * stranger's vault entries (PR #360 review t4). With an id the target is
...
export async function applyLoginOutcome(input: ApplyLoginOutcomeInput): Promise<ApplyLoginOutcomeResult> {
    ...
    return store.withLock(async (config) => {
        const existing =
            input.id === undefined
                ? config.accounts.find((entry) => entry.name === input.name)
                : config.accounts.find((entry) => entry.id === input.id);
