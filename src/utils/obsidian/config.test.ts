...
describe("vault root in the shared plugin config", () => {
    ...
    it("never overwrites a corrupt shared config, by migration or by setVaultRoot", () => {
        const { plugin, legacy, vault } = sandbox();

        write(plugin, "{ other plugins' sections, but broken");
        write(legacy, { vaultRoot: vault });
        resolveVaultRoot(plugin, legacy);

        expect(readFileSync(plugin, "utf8")).toBe("{ other plugins' sections, but broken");
        expect(existsSync(legacy)).toBe(true);
        expect(() => setVaultRoot(vault, plugin)).toThrow("refusing to overwrite");
        expect(readFileSync(plugin, "utf8")).toBe("{ other plugins' sections, but broken");
    });

    it("never replaces an earlier same-day archive when a legacy file migrates again", () => {
        const { plugin, legacy, vault } = sandbox();
        const earlier = `${legacy}.migrated-${new Date().toISOString().slice(0, 10)}`;
