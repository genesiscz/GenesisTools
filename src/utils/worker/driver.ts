 ...
 * `interruptTurn` on a backend whose turn IS a signalled process answers with what the signal
 * actually reached, so the caller never reports success for a turn that had already ended.
 */
export type WorkerVerbOutcome =
    | { kind: "turn"; report: WorkerTurnReport }
    | { kind: "ack"; result: unknown }
    | { kind: "signalled"; pids: number[] };
