...
export const DASHBOARDS = {
    ...
    jev: {
        ...
        bindHost: "127.0.0.1",
        strictPort: false,
        launch: "tools jev dashboard",
        portOverride: { flag: "--port" },
        tech: "vite",
        auth: "none",
        matchProcess: matchGenesisTool("src/jev/dashboard", "jev dashboard"),
    },
    "claude-history": {
        key: "claude-history",
    ...
    spotify: {
        ...
        bindHost: "127.0.0.1",
        strictPort: true,
        launch: "tools spotify ui",
        portOverride: { env: "SPOTIFY_UI_PORT" },
        tech: "vite+tanstack-start",
        auth: "none",
        note: "Reads local files only (streaming-history export + harvested library). Loopback-bound: no auth, and the payloads are one person's whole listening history. Port sourced from this registry.",
        matchProcess: matchGenesisTool("spotify/ui", "spotify ui", "src/spotify/ui"),
    },
    monitor: {
        key: "monitor",
        name: "Monitor",
        ...
        bindHost: "127.0.0.1",
        strictPort: true,
        launch: "tools monitor ui",
        portOverride: null,
        tech: "vite+tanstack-router",
        auth: "none",
        note: "Vite proxies /api to the monitor server (9878). Port sourced from this registry.",
        matchProcess: matchGenesisTool("monitor/ui", "monitor ui", "src/monitor/ui"),
    },
    "debugging-master": {
...
export const WEB_SERVICES = {
    ...
    "monitor-server": {
        key: "monitor-server",
        name: "Monitor Server",
        description: "Watcher scheduler + HTTP/WebSocket API for `tools monitor` (Bun).",
        port: 9878,
        launch: "tools monitor server up",
        portOverride: { flag: "--port" },
        serviceKind: "http-api",
        note: "Default in src/monitor/lib/server/app.ts.",
        matchProcess: matchGenesisTool("monitor/lib/server", "src/monitor/lib/server"),
    },
    "ai-proxy": {
        key: "ai-proxy",
        name: "AI Proxy",
