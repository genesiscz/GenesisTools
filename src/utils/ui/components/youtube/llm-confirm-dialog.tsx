...
export interface LlmConfirmDialogProps {
    ...
    /** Default provider (e.g. "claude") that will be used unless overridden. */
    defaultProvider?: string;
    /** Default model id (e.g. "claude-haiku-4-5") that will be used unless overridden. */
    defaultModel?: string;
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <Input id="llm-provider" placeholder={defaultProvider ?? "default"} value={provider} onChange={(event) => setProvider(event.currentTarget.value)} />
                        </div>
                        <div>
                            <label htmlFor="llm-model" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Model</label>
                            <Input id="llm-model" placeholder={defaultModel ?? "default"} value={model} onChange={(event) => setModel(event.currentTarget.value)} />
                        </div>
                    </div>
