...
describe("Phase 2 live-smoke (RUN_LIVE_SMOKE=1)", () => {
    for (const origin of PHASE_2_SHOPS) {
        describe(origin, () => {
            it_("listCategories returns at least 1 category", async () => {
            ...
            it_("listCategory yields at least 1 product on the first category", async () => {
                const client = ShopRegistry.get().forShop(origin);
                expect(client).toBeDefined();
                if (!client) {
                    return;
    ...
    describe("AlbertClient persisted-query freshness", () => {
        it_("LeftHandNavigationBar does not return PersistedQueryNotFound", async () => {
