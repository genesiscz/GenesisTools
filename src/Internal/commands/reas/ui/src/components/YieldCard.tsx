        ...
        <Card className="border-white/5">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between rounded-md border border-white/5 bg-white/[0.02] px-3 py-2">
                    ...
                    <span className="text-sm font-mono font-bold text-foreground">{paybackYears.toFixed(1)} years</span>
                </div>
                ...
                <div className="rounded-md border border-white/5 bg-white/[0.02] px-3 py-2 space-y-1.5">
                    <span className="text-[10px] font-mono uppercase tracking-wider text-gray-600">
                        At Market Price ({(atMarketPrice.price / 1_000_000).toFixed(1)}M CZK)
                    </span>
                    <div className="flex items-center gap-4 text-xs font-mono">
                        <span className="text-gray-500">
                            Gross:{" "}
                            <span className={yieldColor(atMarketPrice.grossYield)}>
                                {formatYield(atMarketPrice.grossYield)}
                            </span>
                        </span>
                        <span className="text-gray-500">
                            Net:{" "}
                            <span className={yieldColor(atMarketPrice.netYield)}>
                                {formatYield(atMarketPrice.netYield)}
                            </span>
                        </span>
                        <span className="text-gray-500">
                            Payback: <span className="text-foreground">{atMarketPrice.paybackYears.toFixed(1)}y</span>
                        </span>
                    </div>
                </div>
                ...
                {benchmarks.length > 0 && (
                    <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {benchmarks.map((b) => {
                                const diff = netYield - b.annualReturn;
                                return (
                                    <Badge
                                        key={b.name}
                                        variant="outline"
...
function YieldMetric({ label, value }: { label: string; value: number }) {
    return (
