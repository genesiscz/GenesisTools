...
export const grokLauncher: AgentLauncher = {
    ...
    async preflight({ flags, account }) {
        ...
        const result = await runSession({
            name: legacy.name,
            cwd: legacy.cwd,
            // `tools grok run <account> --name x` used to drop the positional account in silence,
            // so an account that exists nowhere was accepted and the turn billed the ambient
            // credential. `runSession` refuses it, the same way the `spawn` door's `--account` is.
