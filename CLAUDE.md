...
**Before adding a test file, read `docs/test-runtime.md`.** The ubuntu CI step is a fixed budget that the suite has already outgrown twice. Measured there: `--parallel` implies `--isolate`, so **a new test FILE costs ~368 ms of runner time (~92 ms of wall) before it asserts anything**, while a new test in an existing file costs only its own runtime. The three patterns that make one file expensive, each with a measured example: a real wall-clock wait (`device-flow.test.ts`, 9.6 s of 9.75 s asleep), a child process per assertion (`merged.test.ts`, 1359 `git` spawns for 33 tests), and an on-demand package install inside a test (`chunker.test.ts`, 9836 ms of `bun add`). `scripts/ci/test-runtime-guard.ts` enforces a per-file ceiling on CI; make the test cheap rather than raising the ceiling.
