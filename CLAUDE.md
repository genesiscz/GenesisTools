...
- **Curated static registry**: `src/utils/ai/models/registry.ts` — `MODEL_REGISTRY: CanonicalModel[]` (id, provider, family, displayName, contextWindow, thinking, list `pricing`, aliases, `flags.supports1m`/`native1m`, `cli` label) with `byProvider()` / `byId()` / `aliasMapFor()` / `pricingFor()`. Anthropic models live ONLY here; `src/utils/ai/anthropic/models.ts` (`ANTHROPIC_SUB_STATIC_CATALOG`, alias map, `inferAnthropicContextWindow`), `src/claude/lib/models.ts` (`CLAUDE_MODELS`), and `src/ai-spend/lib/pricing.ts` (`DEFAULT_PRICING`) are derived views over it. Grok (`src/utils/ai/grok/models.ts`) and Codex/WHAM (`src/utils/ai/openai/sub-models.ts`) stay authoritative in their own files — their records carry provider-specific fields (probe status, speed, visibility, modalities).
...
- **Deliberate exception**: `src/ai-proxy/lib/billing/pricing.ts` keeps a small STATIC table — it's the ai-proxy client-ledger invoicing source of truth (deterministic, offline). Don't "deduplicate" it into the dynamic path or into `MODEL_REGISTRY`, and don't copy it anywhere else; its rates may intentionally differ from list prices. Cost is booked at WRITE time (`recordClientUsage()` in `src/ai-proxy/lib/usage/client-ledger.ts` adds `estimateCostUsd(...)` into the month total), so a later rate edit never rewrites past invoices — readers (`tools ai-proxy clients usage`) print the stored `cost_usd` and never recompute. `pricing.test.ts` asserts every priced prefix still names a model in `MODEL_REGISTRY` / the grok / Codex catalogs, but never asserts rate equality.
