...
Run the githubPR script with markdown output:

```bash
bun scripts/githubPR.ts <pr-number-or-url> -g --md [-u if flag present]
```
...
4. Follow project coding patterns from `.claude/docs/coding-patterns.md`

**Important:**
- Fix threads one by one, validating each fix works
- If a fix is unclear, ask the user for clarification
- Run PHPStan on modified files: `vendor/bin/phpstan analyze <modified-files>`
...
fix(invoices): address code review issues for voucher order invoices

Fixes review comments from PR #137:
- Fixed JSON column query performance issue
...
1. Run: bun scripts/githubPR.ts 137 -g --md -u
...
6. Fix each thread, run PHPStan
7. Commit: "fix(invoices): address code review issues..."
...
4. **Run PHPStan** - validate fixes don't break types
