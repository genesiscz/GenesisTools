...
Research & rationale live in the author's local vault note (not committed here).
...
**Paths** come from `~/.genesis-tools/claude/fable/config.json` (`packPath` = pack repo in the notes vault, own git; `sessionsMirrorPath` = frozen legacy mirror, machine-local). The CLI resolves them itself — you only need `packPath` for the editorial merge and git steps:
...
- Model choice per stage lives in config.json `models` (mine/filterBare/judge/eval) — full ai-proxy ids like `<account>/claude-sub/claude-sonnet-5`; override per-run with the CLI flags, never by editing code.
...
If it reports missing config, STOP and ask the user for `packPath` (the pack folder inside the notes vault) before continuing. `$FABLE/README.md` is versioned — do not regenerate it.
