...
/// These pin the PRODUCTION `sharedIdentifierSummary` in SnapshotSupport, which both hints are
/// built from. A copy of the rule here used to pass while the two hints disagreed.
final class SharedIdentifierHintTests: XCTestCase {
    func testBothHintsCountTheOtherSharedIdentifiers() {
        let rows: [[String: Any]] = Array(repeating: ["AXIdentifier": "worst"], count: 6)
            + Array(repeating: ["AXIdentifier": "also"], count: 5)
        let hint = sharedIdentifierHintText(["worst": 6, "also": 5])

        XCTAssertEqual(hint?.contains("1 other identifier is shared this way."), true)
        XCTAssertEqual(sharedIdentifierRowHint([IdentifierWindow(index: 0, title: "Main", rows: rows)]), hint)
    }
