...
final class WindowEventTests: XCTestCase {
    ...
    func testDragPinsTheFrameThatMatchesItsChosenTarget() {
        // dragVerifyTarget above only proves WHICH target a step names. This proves what
        // that choice pins, which is the half the regression actually lived in: the window
        // bounds once the drag is under way, the captured element frame before it.
        let windowBounds = CGRect(x: 0, y: 0, width: 800, height: 600)
        let elementFrame = CGRect(x: 90, y: 190, width: 40, height: 20)

        XCTAssertEqual(
            WindowEventFactory.expectedVerifyFrame(target: .window, windowBounds: windowBounds,
                                                   elementFrame: elementFrame),
            windowBounds, "a step under way pins the window, never the element that is moving")
        XCTAssertEqual(
            WindowEventFactory.expectedVerifyFrame(target: .element, windowBounds: windowBounds,
                                                   elementFrame: elementFrame),
            elementFrame, "the step before mouse-down still pins the captured element")
    }
