        ...
        for key in ["AXIdentifier", "AXTitle", "AXDescription", "AXSubrole", "AXValue", "AXEnabled", "AXFocused", "AXSelected", "AXSelectedText", "AXSelectedTextRange"] {
            if let value = axAttribute(element, key) {
                if key == "AXSelectedTextRange", CFGetTypeID(value) == AXValueGetTypeID() {
                    ...
                    // `as!` is deliberate: Swift REFUSES `as?` to a CoreFoundation type
                    // ("conditional downcast ... will always succeed" is an error, not a
                    // warning) and its own fix-it names the CFGetTypeID comparison guarding
                    // this line. The type-ID check IS the checked cast here.
