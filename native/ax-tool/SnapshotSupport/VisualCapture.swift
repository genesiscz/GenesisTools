...
func pruneExpiredVisualClaims(in root: URL, now: Date = Date()) throws {
    let entries = try FileManager.default.contentsOfDirectory(at: root, includingPropertiesForKeys: nil)
    for file in entries.prefix(4096) {
        guard file.pathExtension == "used",
              let id = UUID(uuidString: file.deletingPathExtension().lastPathComponent),
              file.lastPathComponent == id.uuidString + ".used" else { continue }
        var info = stat()
        guard lstat(file.path, &info) == 0, info.st_uid == getuid(),
              info.st_mode & S_IFMT == S_IFREG, info.st_mode & 0o077 == 0,
              now.timeIntervalSince1970 - Double(info.st_mtimespec.tv_sec) > 600 else { continue }
        if unlink(file.path) != 0 && errno != ENOENT {
            throw VisualCaptureError.invalid("could not prune expired visual claim (errno \(errno))")
        }
    }
}
...
public func consumeVisualCapture(_ capture: VisualCaptureIdentity, directory: URL? = nil) throws {
    ...
    try pruneExpiredVisualClaims(in: root)
