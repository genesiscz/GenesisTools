...
public func sharedIdentifierRowHint(_ windows: [IdentifierWindow]) -> String? {
    ...
    return "\(count) elements in this app all report the identifier \"\(id)\". SwiftUI propagates a"
        + " container's .accessibilityIdentifier to every descendant, which shadows per-control"
        + " identifiers; .accessibilityElement(children: .contain) on the root restores them."
}
