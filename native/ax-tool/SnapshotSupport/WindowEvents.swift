...
public final class WindowEventFactory {
    ...
    /// The frame a verify step must pin for a given target: the window's CG bounds once a drag
    /// is under way, the element's captured frame otherwise.
    ///
    /// This selection IS the drag regression. `dragVerifyTarget` only decides which target a
    /// step names; what that choice pins lived inline in a nested function, so it could be
    /// reverted with every test still green. It is a free function to keep both branches
    /// assertable.
    public static func expectedVerifyFrame(target: SnapshotVerifyTarget, windowBounds: CGRect,
                                           elementFrame: CGRect) -> CGRect {
        switch target {
        case .window: return windowBounds
        case .element: return elementFrame
        }
    }
