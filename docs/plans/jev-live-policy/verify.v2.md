# tools jev screen + verify v2 — more templates and PR body

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## More templates

`license`, `public-api-break`, `macos-tcc`, `test-accounts`.
Each is a closed question set in `templates.ts`.

## PR body

`tools jev verify --claims pr-body.md --against src/` is the
documented flow. v2 adds `--only-changed` using `git diff --name-only`.

## Dashboard

A Verify strip in Control Lab is optional and out of scope if it
fights the design system. CLI is the product.

## Still

No generated review comment. The output object is the API.
