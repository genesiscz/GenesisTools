# tools jev listen v2 — speculative dispatch and Lab tail

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

v1 proves the pipe. The bookmarked demo feels instant because likely
act payloads are already built. v2 turns the top-3 cache into a live
prefetch and makes the Control Lab Listen tab a real surface.

## Speculative click-ahead

Store `{ snapshot, payloads[3], builtAt }` from the last distribution.
On the next tick, if the winner is still in the set and the token is
identical, dispatch the prebuilt payload. If the tree changed, drop
the cache. If the winner left the set, rebuild. Prefetch never invents
an id that was not in the last Jev answer.

## Questions in one call

| id | type | purpose |
|---|---|---|
| verb | choice | observed ids + chrome verbs + abstain |
| terminal | boolean | complete command? |
| correction | boolean | retract previous intent? |

Corrections invalidate prefetch and do not dispatch the previous winner.

## Lab API

Local only, same `validLocalRequest` rules:
...
One session. A second start is 409. Start does not open a browser mic;
it starts the CLI or attaches to a JSONL fifo.

## Auto surface

`--surface auto` prefers CDP for page content and AX for chrome when
both exist. Candidate ids are prefixed `ax:` / `cdp:`.

## Wake-word handoff

Accept an already-open `LiveSttSession`. Wake-word owns capture; listen
owns policy.

## Tests

Prefetch hit skips a second see. Snapshot change misses. Correction
drops the winner. Lab rejects a second session. Auto surface prefixes
ids. Partial to fixture decision display stays under 100 ms.
