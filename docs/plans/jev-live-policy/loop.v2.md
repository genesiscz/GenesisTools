# tools jev loop v2 — auto surface and listen/watch reuse

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Auto surface

`--surface auto` merges AX chrome + CDP page as in listen v2.
Listen and watch call `runGoalLoop` instead of private loops.

## Reupdates

Smarter wait: use `awaitCondition` from phase 2 when `--wait`
is set. Recovery remedies from assist apply here too.

## Browser complexity

Named frames, same-origin only eval, no `page.evaluate` of model
text. Clicks are uid-only. Fill values come from `--data` or
listen's closed phrase list, never from free Jev text.

## Tests

Listen fixture uses the loop. Watch fixture uses the loop.
Auto merge prefixes ids. Model-supplied CSS selector is rejected.
