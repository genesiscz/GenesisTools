# tools jev compact v2 — richer traces and streaming

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

v1 is a file filter. v2 understands agent session JSONL from
`~/.genesis-tools/claude-history` style traces and can stream.

## Streaming

`--follow` reads JSONL as it grows (file watch, deadline on idle).
Each new tool result is decided immediately. The decision table is
appended, not rewritten, so a consumer can tail it.

## Session sources

`--source claude|codex|grok|jsonl` uses the shared
`src/utils/agent-sessions` readers. Compaction still never writes
those databases. It only reads and prints.

## Token-aware keep

`--keep` may be a token budget (`--keep-tokens 8000`) using the
existing token estimators. Structural layer uses char/4 as a stand-in
when the estimator is missing; document that.

## Pairing

v2 pairs `tool_use` with `tool_result` across interleaved rows so a
drop cannot orphan a result. Tests include a shuffled pair.

## Still not a summarizer

No rewrite of user/assistant. No generated recap. If a caller wants
a recap, that is a different tool.
