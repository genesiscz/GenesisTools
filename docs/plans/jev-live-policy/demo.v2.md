# tools jev control demo v2 — recorded reel

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Record

On macOS with permissions, `--record` writes `demo.mp4` beside
`trace.json` using the existing capture runner. The overlay is on
so clicks are visible. The summary still refuses success without
exact readback even if the video looks great.

## Lab

Control Lab Demo reel page lists the fixture names, runs them
through the local API, shows the trace. No user's Mail.

## Build-in-public

The trace schema is stable: `{demo, startedAt, events[], readback, ok}`.
That is the artifact, not a new subsystem.
