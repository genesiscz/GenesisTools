# tools jev loop v1 — goal-driven see/act for AX and browser

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

The user wants the see/act multistep loop with a controlling goal
for both browser and general computer use. Assist already loops AX.
This feature is the shared `GoalLoop` plus a browser driver so
listen/watch/assist/demo call one thing.

## Contract

```
tools jev loop --goal "archive the selected newsletter" --app Mail
tools jev loop --goal "open the first result" --browser --port 9222
tools jev control loop   # alias
```

Bounded: app/window or CDP target, time, requests, action attempts.
Re-see after every act. Fan-out observe each step. Hold on stale
token. Exact readback wins.
...
export interface GoalSurface {
  ...
  act(snapshot: SurfaceSnapshot, candidate: SurfaceCandidate): Promise<{ ok: boolean; error?: string }>;
  overlay?(candidate: SurfaceCandidate): Promise<void>;
}
```

AX surface wraps `NativeControlDriver`. Browser surface wraps
chrome-devtools MCP: `take_snapshot` → candidates, `click` /
`fill` on uid. No invented selectors.

## Multistep reupdates

After each act, see again. If `done`, stop verified. If `wait`,
use the existing semantic wait helper when available, else one
250 ms bounded sleep and see. If `blocked`, stop. If `risk=high`,
stop unless `--yes`.

## Computer use

`--computer-use` attaches the existing computer-use session
(`src/control/lib/computer-use/session.ts`) as a third surface
kind `cu`. v1 may only expose observe+press verbs already in that
session. No Sky/Bonsai.

## Library

```
src/jev/lib/loop/surface.ts
src/jev/lib/loop/ax.ts
src/jev/lib/loop/browser.ts
src/jev/lib/loop/run.ts
src/jev/commands/loop.ts
src/chrome-devtools/lib/page-snapshot.ts
```

`page-snapshot.ts` is the expansion of chrome-devtools: a typed
list of clickable nodes from MCP `take_snapshot` or CDP
`DOM.getDocument` + `Accessibility.getFullAXTree`, mapped to
`SurfaceCandidate`. Keep it in chrome-devtools so jev does not
reimplement CDP.

## Tests

AX fixture loop verifies on done. Browser fixture (mocked MCP)
clicks uid `1_2` once. Stale navigation id holds. High risk
without `--yes` stops. Computer-use kind is rejected on Linux
with a clear message if the native session cannot start, not a
crash. Help lists `--browser` and `--app` as mutually exclusive
unless `--surface auto` (v2).
