# Jev wake word v2 — stream handoff and native face

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Goal

Say the phrase, the Listen strip lights up, the first command word
after the phrase is not clipped, and nothing left the machine before
the trigger.

## Stream handoff

Wake-word owns capture. On trigger it emits a PCM or `LiveSttSession`
handoff. Listen v2 consumes it. Closing listen returns the source.

Test: fixture PCM with wake word ending at t=1.00s and "click export"
at t=1.05s. Listen must see "click export".

## Native face

Run as GenesisTools.app `--wake` so TCC follows
`com.genesiscz.genesistools`. `tools jev wake status` prints pid and
start time. Stale binaries are reaped like `--rpc`.

## Engine

Local keyword spotter only. Cloud spotting is forbidden. Porcupine
stays behind `WakeWordEngine` if licensed later.

## Frontmost routing

After trigger, one Jev choice: which of {frontmost, last two listen
targets} to bind. Abstain keeps frontmost.

## Safety

Still off by default. Marker file. No pre-trigger upload. Diagnostics
