# tools jev watch v2 — delta see and Lab strip

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Delta see

Use existing `snapshot-diff` / `--since` so unchanged trees do not
pay a full encode. Jev state includes `{changed, unchangedCount}`.

## OCR blocks

When phase 2 visual grounding is present, include OCR text blocks
in the state. Still text, still one-use pixel evidence for any
click that used OCR.

## Lab strip

Hz, in-flight, last distribution, last refusal, sparkline of
decision ms. Tweetable, still exact-readback honest.

## Parallel fixtures

Optional: 8 AppKit fixture windows in live-smoke, chart latency.
Not a product command. A script under `src/control/scripts/`.
