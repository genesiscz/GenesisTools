# gt:github-pr compact hook v1 — spec only, do not implement

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

The user wants thread expansion in `/gt:github-pr` to become a Jev
keep/drop, not another LLM summary. This PR must not hook it. This
file is the contract so a later PR can do it in one place.

## Current pain

`/gt:github-pr` expands review threads into the agent context. Long
PRs drown the model. A second LLM summary loses verbs and ids that
the agent needs to act.

## Hook shape

```
tools github review <n> --llm
  → JSONL thread messages
  → tools jev compact - --keep 0.5 --pin 8
  → skill reads stdout JSONL
```

The skill grows one optional flag `--compact` defaulting off in the
first hook PR. This spec's PR only documents it.

## What must not happen

- Do not call Jev from the skill via a hidden path that bypasses
  `tools jev compact`.
- Do not summarize thread bodies.
- Do not drop the user's latest review comment (pin).
- Do not write to the GitHub API as a side effect of compact.

## Mapping

| Thread object | Compact role |
|---|---|
| review comment body | assistant-equivalent text: keep |
| file hunk | tool result: truncatable |
| outdated thread | tool result: droppable |
| resolved thread | tool result: droppable unless `--keep-resolved` |
| latest user reply | pinned |

## Skill copy (for the later PR)

Add to the skill: "If `--compact`, pipe review JSONL through
`tools jev compact -` and use the compacted JSONL as the thread
expansion. If compact returns `unchanged`, use the original."

## Tests the later PR must add

A fixture PR thread with 40 outdated hunks and 2 latest comments
keeps the 2 comments and drops or truncates the hunks. This PR adds
no skill edits.

## Acceptance for this PR

This file exists. No `plugins/**/github-pr` edits. No skill hook.
