# Speculative click-ahead v1 — cache only

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

Voice and watch both need this. v1 is the cache type and the
invalidation rules, used by listen v1 as record-only.
...
export interface PrefetchCache {
  ...
  items: Array<{
    id: string;
    probability: number;
    payload: { element: number; action: "press" | "set" | "chrome"; chrome?: string };
  }>;
}
```

`buildPrefetch(distribution, snapshot, candidates)` keeps top 3
admitted-or-not ids that exist in `candidates`.
`matchPrefetch(cache, snapshot, winnerId)` returns the payload
or null.

## Invalidation

Null if snapshot differs, age > 2000 ms, winner not in items,
or candidates no longer contain that element index.

## Tests

Top-3 order. Snapshot mismatch. Age expiry with injected now().
Missing winner. Chrome verb payload.
