# tools jev route v1 — System-One front door to the CLI catalogue

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

Jev cannot answer "what's the weather". It can pick `tools weather`
versus `tools github pr` versus abstain. This is how `tools jev`
becomes the front door to the checkout's CLIs without pretending to
be ChatGPT.
...
→ { tool: "github", argv: ["review", "409", "-u"], p: 0.86, destructive: false }
→ prints: tools github review 409 -u
```

Default is print. `--run` executes.

## Catalogue

State = utterance + compact catalogue of this checkout:

- `name` (tool directory)
- `oneLine` from README first paragraph (`discoverTools`)
- `commands[]` with `path`, `description`, `argHint`, `destructive`

v1 builds the catalogue by walking `src/*/index.ts(x)` plus a static
destructive set (`control act`, `git push`, `jenkins`, `osascript`,
`chrome-devtools rm-cookie`, `wake enable`). Subcommand names come from
a conservative source scan of `.command("name")` in `commands/` and
`index.ts`. The walker must not execute tool entrypoints.

Cache the catalogue per `(git rev-parse HEAD, src mtime)` under
`~/.genesis-tools/jev/catalogue.json`. `--refresh` rebuilds.

## Questions in one call

| id | type | purpose |
|---|---|---|
| tool | choice | tool names + abstain |
| command | choice | subcommands of the chosen tool + abstain |
| destructive | boolean | would this mutate durable state? |
| confirm | boolean | should a human confirm? |

If `destructive` or the static set says yes, `--run` requires `--yes`
or a TTY confirm. The extra noul gate is the `confirm` boolean at
`p >= 0.8`.

v1 may ask tool and command in one request by flattening
`github.review`, `github.pr`, … as choice criteria, plus `abstain`.
That keeps one RTT. If the flattened set exceeds 80 options, first
choose the tool, then the command (two requests, still one command
invocation). Prefer flatten when `<= 80`.
...
`--limit` is how many suggestions to print (default 3). The winner is
first. Others are listed with probabilities when the distribution
exists.

## Library

```
src/jev/lib/route/catalogue.ts
src/jev/lib/route/router.ts
src/jev/lib/route/destructive.ts
src/jev/commands/route.ts
```

## Execution

`--run` spawns `bun <tool-entry> ...argv` the same way the `tools`
launcher does, inheriting stdio. It does not `eval` a string. argv is
taken from a small argv encoder: known flags from the catalogue hint,
plus integers/ids copied from the utterance when they match
`/^\d+$/` or a conservative PR/issue pattern. v1 does not invent
flags Jev did not pick. If the command needs required flags the
utterance did not supply, print the command and the missing flags;
do not run.

## Tests

Catalogue includes `jev` and `control` and marks `control act`
destructive. Router with a fixture evaluator picking `github.review`
prints the command and does not spawn. `--run` without `--yes` on a
destructive tool exits 1. Flattened criteria stay under 80 or split.
Unknown utterance → abstain, exit 1. Help lists `--run` as opt-in.
