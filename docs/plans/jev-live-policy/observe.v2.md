# tools jev control observe v2 — assist default fan-out

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

v1 proves the packet. v2 makes assist System-One: every step is
one fan-out, then code.

## Assist default

`--fanout` defaults on. `--no-fanout` restores the serial chooser
for replay comparison. Replay fixtures gain expected fan-out
answers.

## Extra questions

`recovery` choice over the existing remedy inventory when the last
act refused. `rebind` boolean for workflow plans. These stay inside
the same request when the last step produced a refusal.

## Dashboard

Control Lab gains an Observe panel showing the six meters for the
selected fixture. Compare serial vs fan-out like the existing
chooser compare.

## Tests

Assist default path sends the fan-out question map. `--no-fanout`
sends only `target`. Recovery question appears only after a refusal.
