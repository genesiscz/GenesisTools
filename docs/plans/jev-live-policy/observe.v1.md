# tools jev control observe v1 — fan-out on one see

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

Jev wants one state, many isolated questions. A `see` dump is a
perfect prefix. Today `resolveIntent` asks one `target` choice.
`assist` then maybe judges. That is two or more RTTs and a weaker
policy. Observe asks the parallel set in one request, then code
branches.

## CLI

```
tools jev control observe
  --app <name>
  --window-id <id>
  --goal <text>
  --snapshot-file <file>
  --expect <text>
  --exact-id <id> --exact-value <text>
```

Also `tools control observe` because the registry is shared.

## Questions (one request)

| id | type | purpose | criteria |
|---|---|---|---|
| target | choice | which element | candidate ids + abstain + reobserve |
| verb | choice | press / set / scroll / abstain | closed set |
| done | boolean | is the goal already true | |
| blocked | boolean | dialog / occluder / wrong window | |
| risk | score | irreversible / send / delete | low, medium, high |
| wait | boolean | should we wait instead of acting | |

State is the existing observation encoding (`observedEvidence` +
`candidatesFor` for press and set). Scroll is listed only when a
scrollable row exists; otherwise the criterion is omitted, not faked.

## Code branches

```
if done && !blocked → status verified (still require exact if provided)
if blocked → escalate / stop
if wait → return {status:"wait"} without act
if verb==abstain or target==abstain → stop
if risk==high → escalate unless --yes
else → selected candidate + verb
```

Exact readback, when supplied, overrides `done`.

## Library

`observeTask` in `src/control/lib/decision/observe.ts`.
`assistTask` v1 calls `observeTask` each step instead of
`chooseCandidate` + a later `judgeOutcome` when `--fanout` is set.
Default `--fanout` is on for `observe`, off for `assist` in v1 so
existing assist tests stay stable. v2 flips assist default.

## Multistep

`observe --loop` (v1 yes, bounded) re-sees after each branch that
does not stop. `--max-steps` default 1 for observe, 8 for assist.
Reupdates: if `wait` or `reobserve`, sleep up to 250 ms (existing
assist behaviour) and see again. Shared budget.

## Tests

One evaluator call contains all six question ids. `done=true`
returns verified without act. `blocked=true` does not act.
`risk=high` without `--yes` does not act. Exact mismatch overrides
semantic `done`. Snapshot-file path does not call native. Assist
`--fanout` uses observeTask (spy on evaluate input.questions).
