# tools jev watch v1 — bounded high-Hz policy on one window

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

Not a game. A bounded high-Hz policy on a real window. Subway
Surfers DNA, honest shape: tiny verb set, fresh state, exact
readback.
...
Default `--hz 4`, `--max-seconds 15`.

## Loop

1. `see --since` cheap delta when the driver supports it; else see.
2. Encode changed rows + OCR blocks as text (OCR optional; empty is
   fine in v1).
3. Fan-out: `done` / `verb` / `abstain` (observe subset).
4. Act only on exact id/value after a fresh token.
5. Stop on done, budget, or SIGINT.

Do not ship 50 parallel games. If a brag is needed, the demo spec
charts decision latency on the existing live-smoke harness.

## Watch strip

Control Lab: Hz, in-flight request, last distribution, last refusal.
Tails JSONL from the CLI. v1 can be CLI-only if the Lab time is
better spent on listen.

## Tests

Fake driver that becomes "done" on step 3. Watch stops with
verified. Hz cap: a fixture clock shows at most 10 observes in 1 s
of simulated time (inject now()). SIGINT stops. Stale token holds.
`--hz 0` rejected. `--hz 11` rejected.
