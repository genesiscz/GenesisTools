# tools jev route v2 — semantic suggest and commander graph

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

v1 is a door with a coarse catalogue. v2 ranks by semantic meaning
across all commander paths and can fill safe argv from isolated
questions.

## Catalogue v2

Walk commander programs without running actions: dynamically import
`registerX(program)` where a safe `register*` export exists, then
read `program.commands`. Tools without a registrar stay on the v1
scan. Each option records `{ long, required, enumValues, destructive }`.

## Argv filling

After tool+command, extra choice/score questions fill required
options from a shortlist extracted from the utterance (numbers,
flags already present, file paths that exist). Jev never invents a
path that does not exist on disk. Missing required options still
print, not run.

## Semantic suggest

`tools jev route --suggest <text>` prints the top 10 catalogue rows
by a Jev score question over batches of 20 names (shared prefix =
utterance). This is the "all repos commander commands auto-suggested
by semantic meaning" demand.

## Aliases

Honour `src/aliases` and README usage lines as extra criteria
descriptions so `pr 409` lands on `github review` not `git`.

## Tests

Registrar walk does not invoke action handlers. Suggest batches at
most 20 names. Non-existent path is not filled. Alias description
is present on `github` rows.
