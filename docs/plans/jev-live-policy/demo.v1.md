# tools jev control demo v1 — fixture reel with traces

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

The bookmarks are videos. The proofs are JSON. Demo commands must
use the AppKit fixture, record via existing capture, write the
decision trace next to an optional mp4, and refuse to call it
success if exact readback did not confirm.

## CLI

```
tools jev control demo [listen|watch|route|compact|observe|verify|loop|voice]
  --fixture
  --record
  --out <dir>
```

`voice` is an alias of `listen`. Default `--fixture`. Refuses
`--app Mail` without `--i-mean-it` (and even then the README says
don't).

## Behaviour

Each demo name loads a checked-in JSONL / snapshot under
`src/jev/lib/demo/fixtures/`. It runs the corresponding lib with
a fixture evaluator and fixture STT/driver. It writes
`trace.json` and `summary.json` to `--out` (default a temp dir
printed on stderr).

`--record` calls the existing capture helper only when the
platform can. On Linux CI, `--record` prints "capture skipped"
and still writes the trace. Success is the trace, not the mp4.

Success criteria: `summary.ok === true` only when exact readback
confirmed, or the demo is a non-act demo (route/compact/verify)
that produced the expected fixture object.

## Tests

Each demo name returns a summary. `listen` dry-run on fixture
transcript does not call native. `compact` demo reduces the
checked-in session below keep or reports unchanged honestly.
Unknown name exits 1 with the list. `--app Mail` without the
break-glass flag exits 1.
