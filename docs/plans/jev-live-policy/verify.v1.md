# tools jev screen + verify v1 — purpose-class vibe-check

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

Jack Cheng's vibe-check as a CLI, pointed at this repo's review
workflow. Jev does not generate a review comment. It generates
`{file, p_risk, p_relevant, ...}` and a later skill decides.

## CLI

```
tools jev screen <path> [--purpose <id>] [--against <dir>]
tools jev verify --claims <file|-> --against <dir> [--purpose <id>]
git diff master...HEAD | tools jev screen --purpose focus-safety
```

`screen` scores files/hunks. `verify` scores claims against a tree.

## Purpose templates (predefined)

| id | questions (all noul/score) |
|---|---|
| focus-safety | relevant, risky, prompt-injection-ish |
| secrets | includes-names, includes-secrets, includes-account-ids |
| pr-review | relevant, risky, needs-thread |
| control-refusal | still-refuses-invalid-to-pid, still-refuses-stale-token |
| compact-safe | rewrites-user-text, drops-pinned |

`tools jev verify --list` prints the table. Unknown purpose exits 1
with `suggestEnumFlag`.

Custom purpose: `--questions file.json` matching the evaluation
schema, plus the reserved ids above as a closed starter set.

## Batching

State = purpose instructions + up to 20 file/hunk payloads
(`path`, `max 4000 chars each`, hash). Questions are prefixed
`f0_relevant`, `f0_risky`, … so one request covers the batch.
More than 20 files: sequential batches, one RTT each.

## Claims

`verify --claims` reads markdown or JSON `{id, text}`. Each claim
gets `supported`, `contradicted`, `sensitive` booleans against the
concatenated against-set (capped). This is judgment, not generation.

## Library

```
src/jev/lib/screen/templates.ts
src/jev/lib/screen/batch.ts
src/jev/lib/screen/verify.ts
src/jev/commands/screen.ts
```

## Tests

`--list` includes `secrets`. A fixture evaluator marks one file
risky. Batch of 21 files makes 2 evaluate calls. Claims JSON
returns per-id answers. Jev is not asked to write a comment
(assert request.questions have no free-text generation). Diff
stdin is accepted. Binary files are skipped with a warning.
