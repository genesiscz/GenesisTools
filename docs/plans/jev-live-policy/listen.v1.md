# tools jev listen v1 — live STT into a typed Jev chooser

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Why

The bookmarked voice-browser demo is: encode visible actions as text,
classify a partial transcript in 100–300 ms, dispatch a deterministic
tool. GenesisTools already has `see`/`act`, snapshot tokens, refusal,
and the Cua overlay. The missing pipe is live speech-to-text that feeds
`resolveIntent` on every partial, then acts only when the token is still
fresh and the probability clears the gate.

## Non-goals

No wake word in the runtime (own spec). No always-on daemon. No free
typing, coordinates, or shell. No second AI model. No pixel-in Jev.
No Mail/Messages as the default target.

## Contract

`mic or transcript stream → partial text → Jev choice over the current
see()/CDP verb set → act only if admitted and the snapshot token is live.`
...
tools jev listen
  ...
  --port <n>
  --match <substr>
  --stt [deepgram|xai|openai|fixture]
  --account <id>
  --model <ref>
  --goal <text>
  --gate <0.5-1>
...
`--stt` is an enumerated flag declared as `--stt [value]`. TTY with no
value prompts; non-TTY or invalid value prints `suggestEnumFlag` and
exits 1.

Non-TTY without `--transcript` exits 1. Non-TTY defaults to dry-run
unless `--force-act`.

Ctrl+C dies. One foreground session, one app or one tab.

## Library

```
src/utils/ai/stt/types.ts
src/utils/ai/stt/resolve.ts
src/utils/ai/stt/providers/{deepgram,xai,openai,fixture}.ts
src/utils/ai/stt/index.ts
src/jev/lib/listen/pipeline.ts
src/jev/lib/listen/verbs.ts
src/jev/lib/listen/prefetch.ts
...
```ts
export type SttProviderId = "deepgram" | "xai" | "openai" | "fixture";
export interface LiveTranscriptEvent {
  kind: "partial" | "final" | "speech_start" | "speech_end" | "error";
  text: string;
  isFinal: boolean;
  confidence?: number;
  startedAtMs: number;
...
`openLiveStt` binds through AI accounts. Missing credentials print the
login command. xAI already has `transcript.partial` on a websocket.
Deepgram uses the lockfile SDK. OpenAI uses the existing transcribe
binding; v1 may chunk PCM if streaming is missing, but events stay
the same.

## State machine

`idle → hearing → deciding → (hold | would | act | abstain) → hearing`

On every partial: refresh see if snapshot older than 750 ms; encode
candidates plus chrome verbs `back|next_tab|prev_tab|close_tab|reload|abstain`;
ask Jev `verb` (choice) and optional `terminal` (boolean); admit;
dispatch only on final, silence, or high-confidence terminal verb;
hold if the token changed.

v1 prefetch only records top-3 ids. It does not dispatch-ahead.

## Browser path

`--browser` uses chrome-devtools snapshot nodes `{ uid, role, name }`.
Act clicks that uid. Hold if navigation id changed. Overlay runs
against the OS window when the target is Chrome/Brave.

`ListenSurface` is `{ see, act, snapshotId, scopeLabel }`.

## Control Lab

v1: a strip that tails CLI JSONL. No browser microphone.

## Tests

Fixture STT emits partial then final. Below-gate partial does not act.
Final above gate on a fresh token acts once. Token change holds.
Dry-run never calls act. Help lists `--stt` values. Non-TTY without
transcript exits 1.
