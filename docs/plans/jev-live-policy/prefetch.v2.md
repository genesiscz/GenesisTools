# Speculative click-ahead v2 — dispatch path

Status: contract for `cursor/jev-live-policy-d0bc` on top of PR #409.

## Dispatch

Listen v2 and watch v2 call `matchPrefetch` before `see` when the
last snapshot is still believed live. A hit still re-checks the
token on the act call (the native driver already does). A miss
falls through to see+decide.

## Tests

Hit does not call see (spy). Miss calls see. Act still refuses a
forged payload whose snapshot was swapped.
...
- Extra acceptance 57: unclassified callers of new guards are bugs; document every call site in the same commit.
- Extra acceptance 57.1: remaining budget is checked before every paid request and every act.
- Extra acceptance 57.2: cancellation restores the terminal and releases the mic, CDP client, and overlay.
- Extra acceptance 58: unclassified callers of new guards are bugs; document every call site in the same commit.
- Extra acceptance 58.1: remaining budget is checked before every paid request and every act.
- Extra acceptance 58.2: cancellation restores the terminal and releases the mic, CDP client, and overlay.
- Extra acceptance 59: unclassified callers of new guards are bugs; document every call site in the same commit.
- Extra acceptance 59.1: remaining budget is checked before every paid request and every act.
- Extra acceptance 59.2: cancellation restores the terminal and releases the mic, CDP client, and overlay.
